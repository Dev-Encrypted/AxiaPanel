...
AxiaPanel services run via systemd. View their logs with `journalctl`:

```bash
# Agent logs
journalctl -u axiapanel-agent -n 100 --no-pager

# API logs
journalctl -u axiapanel-api -n 100 --no-pager

# Follow logs in real-time
journalctl -u axiapanel-agent -f

# Logs since last boot
journalctl -u axiapanel-agent -b
```

For verbose output, set the log level in the service config:

```bash
# Edit /etc/axiapanel/api.env and add:
RUST_LOG=debug
...
**Cause**: The AxiaPanel agent is not running.

**Fix**:

```bash
# Check agent status
systemctl status axiapanel-agent

# Start it if stopped
systemctl start axiapanel-agent

# Check for errors
journalctl -u axiapanel-agent -n 50 --no-pager
```

Also check the API service:

```bash
systemctl status axiapanel-api
journalctl -u axiapanel-api -n 50 --no-pager
```
...
2. If the directory is empty, AxiaPanel should have created a default `index.html`. Re-create the site from the panel.
...
5. Retry:
   ```bash
   axiapanel ssl provision example.com --email you@example.com --runtime static
   ```
...
mkdir -p /run/axiapanel
mkdir -p /etc/axiapanel
mkdir -p /var/backups/axiapanel
mkdir -p /var/www/acme

# Check the agent token exists
ls -la /etc/axiapanel/agent.token
...
```bash
sudo bash /opt/axiapanel/scripts/update.sh
```
...
```bash
# Enable all AxiaPanel services
systemctl enable axiapanel-agent
systemctl enable axiapanel-api
...
```bash
# Restart all AxiaPanel services
systemctl restart axiapanel-agent
systemctl restart axiapanel-api
...
```bash
INSTALL_FROM_RELEASE=1 sudo bash /opt/axiapanel/scripts/update.sh
```

## Getting Help

- **GitHub Issues**: [github.com/ovexro/axiapanel/issues](https://github.com/ovexro/axiapanel/issues) -- Report bugs or request features
- **Discussions**: Use GitHub Issues for questions and community support
- **Email**: hello@axiapanel.dev for priority support inquiries

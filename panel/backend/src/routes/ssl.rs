...
pub async fn provision(
    ...
    if site.status != "active" {
        return Err(err(
            ...
            "Site deve estar ativo antes de provisionar SSL",
        ));
    }

    if site.ssl_enabled {
        return Err(err(StatusCode::CONFLICT, "SSL já está habilitado"));
    }
    ...
    if recent_count >= 10 {
        return Err(err(
            ...
            "Limite de taxa: máximo 10 certificados SSL por hora. Tente novamente mais tarde.",
        ));
    }
...
pub async fn provision_dns01(
    ...
    if site.status != "active" {
        return Err(err(StatusCode::BAD_REQUEST, "Site deve estar ativo"));
    }
    ...
    let zone = zones.into_iter()
        ...
        .ok_or_else(|| err(
            StatusCode::PRECONDITION_FAILED,
            "Nenhuma zona DNS do Cloudflare encontrada para este domínio. Adicione-a primeiro no gerenciamento de DNS.",
        ))?;

    let cf_zone_id = zone.cf_zone_id.as_deref()
        .ok_or_else(|| err(StatusCode::PRECONDITION_FAILED, "Zona não possui zone ID do Cloudflare"))?;
    let cf_api_token = zone.cf_api_token.as_deref()
        .ok_or_else(|| err(StatusCode::PRECONDITION_FAILED, "Zona não possui token de API do Cloudflare"))?;
...
pub async fn renew(
    ...
    if !site.ssl_enabled {
        return Err(err(StatusCode::BAD_REQUEST, "SSL não está habilitado para este site"));
    }

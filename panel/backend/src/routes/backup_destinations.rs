...
pub async fn create(
    ...
    if !["s3", "sftp"].contains(&body.dtype.as_str()) {
        return Err(err(StatusCode::BAD_REQUEST, "Tipo deve ser s3 ou sftp"));
    }
    if body.name.trim().is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Nome é obrigatório"));
    }
...
pub async fn update(
    ...
    .await
    .map_err(|e| internal_error("update backup_destinations", e))?
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Destino não encontrado"))?;
...
pub async fn remove(
    ...
    if deleted.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Destino não encontrado"));
    }
    ...
    if dependent_count.0 > 0 {
        resp["warning"] = serde_json::json!(format!(
            "{} agendamento(s) de backup usavam este destino e agora estão sem destino",
            dependent_count.0
        ));
    }

...
pub async fn create(
    ...
    if body.name.is_empty() || body.name.len() > 63 {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de banco de dados inválido"));
    }
    if !body
        .name
        .chars()
        .all(|c| c.is_ascii_alphanumeric() || c == '_')
    {
        return Err(err(
            ...
            "O nome do banco de dados deve ser alfanumérico com underscores",
        ));
    }
    ...
    if existing.is_some() {
        return Err(err(StatusCode::CONFLICT, "Nome de banco de dados já existe para este site"));
    }
    ...
    if !["postgres", "mysql", "mariadb"].contains(&engine) {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Engine deve ser postgres, mysql ou mariadb",
        ));
    }
    ...
    .map_err(|e| {
        if e.to_string().contains("unique") || e.to_string().contains("duplicate") {
            err(StatusCode::CONFLICT, "Conflito de port ou nome de banco de dados, tente novamente")
        } else {
...
pub async fn credentials(
    ...
    let (name, engine, password_enc, port, container_id) =
        row.ok_or_else(|| err(StatusCode::NOT_FOUND, "Banco de dados não encontrado"))?;
...
pub async fn remove(
    ...
    let (_, name, container_id) = db.ok_or_else(|| err(StatusCode::NOT_FOUND, "Banco de dados não encontrado"))?;
...
pub async fn table_schema(
    ...
    {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de tabela inválido"));
    }
...
pub async fn query(
    ...
    if body.sql.trim().is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Query vazia"));
    }
    if body.sql.len() > 10_000 {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Query muito longa (máx. 10KB)",
        ));
    }
...
async fn find_available_port(state: &AppState, engine: &str) -> Result<i32, ApiError> {
    ...
    row.map(|(p,)| p).ok_or_else(|| err(
        ...
        "Sem ports disponíveis para banco de dados",
    ))
}
...
pub async fn pitr_config(
    ...
    let _db: (String,) = sqlx::query_as("SELECT name FROM databases WHERE id = $1 AND site_id IN (SELECT id FROM sites WHERE user_id = $2)")
        ...
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "Banco de dados não encontrado"))?;
...
pub async fn update_pitr_config(
    ...
    if enabled {
        let config_sql = match engine.as_str() {
            ...
            _ => return Err(err(StatusCode::BAD_REQUEST, "Engine não suportado para PITR")),
        };
...
pub async fn pitr_restore(
    ...
    let _parsed = chrono::DateTime::parse_from_rfc3339(target_time)
        .map_err(|_| err(StatusCode::BAD_REQUEST, "Formato de timestamp inválido (use ISO 8601)"))?;
    ...
    if config.map(|(e,)| e) != Some(true) {
        return Err(err(StatusCode::BAD_REQUEST, "PITR não está habilitado para este banco de dados"));
    }

...
pub async fn create(
    ...
    if req.title.is_empty() || req.title.len() > 200 {
        return Err(err(StatusCode::BAD_REQUEST, "Título deve ter entre 1 e 200 caracteres"));
    }

    let status = req.status.as_deref().unwrap_or("investigating");
    if !VALID_STATUSES.contains(&status) {
        return Err(err(StatusCode::BAD_REQUEST, "Status inválido"));
    }

    let severity = req.severity.as_deref().unwrap_or("major");
    if !VALID_SEVERITIES.contains(&severity) {
        return Err(err(StatusCode::BAD_REQUEST, "Severidade inválida"));
    }
...
pub async fn get_one(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Incidente não encontrado"))?;
...
pub async fn update(
    ...
    if existing.is_none() {
        return Err(err(StatusCode::NOT_FOUND, "Incidente não encontrado"));
    }
...
pub async fn create_component(
    ...
    if req.name.is_empty() || req.name.len() > 100 {
        return Err(err(StatusCode::BAD_REQUEST, "Nome deve ter entre 1 e 100 caracteres"));
    }
...
pub async fn delete_component(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Componente não encontrado"));
    }
...
pub async fn subscribe(
    ...
    if req.email.is_empty() || !req.email.contains('@') || req.email.len() > 255 {
        return Err(err(StatusCode::BAD_REQUEST, "Endereço de e-mail inválido"));
    }
    ...
    Ok(Json(serde_json::json!({ "ok": true, "message": "Inscrito nas atualizações de status" })))
}
...
pub async fn public_status_page(
    ...
    if !enabled {
        return Err(err(StatusCode::NOT_FOUND, "Página de status desabilitada"));
    }

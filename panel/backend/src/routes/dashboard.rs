...
pub async fn intelligence(
    ...
    if stale_backups > 0 {
        recommendations.push(serde_json::json!({
            "severity": "warning",
            "message": format!("{} site(s) sem backup nos últimos 7 dias", stale_backups),
            "action": "backup",
        }));
    }
    if scan_crits > 0 {
        recommendations.push(serde_json::json!({
            "severity": "critical",
            "message": format!("Scan de segurança encontrou {} vulnerabilidade(s) crítica(s)", scan_crits),
            "action": "security",
        }));
    }
    if scan_warns > 0 {
        recommendations.push(serde_json::json!({
            "severity": "warning",
            "message": format!("Scan de segurança encontrou {} aviso(s)", scan_warns),
            "action": "security",
        }));
    }
    if open_incidents > 0 {
        recommendations.push(serde_json::json!({
            ...
            "message": format!("{} incidente(s) ativo(s) requerem atenção", open_incidents),
            "action": "incidents",
        }));
    }
    for ssl in &ssl_countdowns {
        ...
        if days <= 14 {
            recommendations.push(serde_json::json!({
                ...
                "message": format!("Certificado SSL de {} expira em {} dia(s)", domain, days),
                "action": "ssl",
            }));
        }
    }
    if firing_count > 0 {
        recommendations.push(serde_json::json!({
            "severity": "critical",
            "message": format!("{} alerta(s) atualmente disparando", firing_count),
            "action": "alerts",
        }));
    }
    if diag_critical > 0 {
        recommendations.push(serde_json::json!({
            "severity": "critical",
            "message": format!("{} problema(s) crítico(s) de diagnóstico detectado(s)", diag_critical),
            "action": "diagnostics",
        }));
    }
...
pub async fn timeline(
    ...
    for (critical, warning, created_at) in &scans {
        events.push(serde_json::json!({
            "type": "security",
            "detail": format!("{} crítico(s), {} aviso(s)", critical, warning),
            "target": "Scan de Segurança",
            "created_at": created_at.to_rfc3339(),
        }));
    }

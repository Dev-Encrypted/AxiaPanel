...
pub async fn create_checkout(
    ...
    let stripe_key = state.config.stripe_secret_key.as_ref()
        .ok_or_else(|| err(StatusCode::SERVICE_UNAVAILABLE, "Billing não configurado"))?;

    let plan = body.plan.to_lowercase();
    if !["starter", "pro", "agency"].contains(&plan.as_str()) {
        return Err(err(StatusCode::BAD_REQUEST, "Plano inválido"));
    }
    ...
    let customer_id = if let Some(cid) = user.0 {
        ...
        let cid = body["id"].as_str()
            .ok_or_else(|| err(StatusCode::BAD_GATEWAY, "Stripe: customer id ausente"))?
            .to_string();
    ...
    let price_id = price_row
        .map(|r| r.0)
        .ok_or_else(|| err(StatusCode::SERVICE_UNAVAILABLE, &format!("Preço não configurado para o plano {plan}. Defina '{price_key}' nas configurações.")))?;
    ...
    let url = session["url"].as_str()
        .ok_or_else(|| err(StatusCode::BAD_GATEWAY, "Stripe: URL de checkout ausente"))?;
...
pub async fn customer_portal(
    ...
    let cid = customer_id
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Nenhuma conta de billing encontrada"))?
        .0;
    ...
    let url = session["url"].as_str()
        .ok_or_else(|| err(StatusCode::BAD_GATEWAY, "Stripe: URL do portal ausente"))?;
...
pub async fn webhook(
    ...
    let webhook_secret = state.config.stripe_webhook_secret.as_ref()
        .ok_or_else(|| err(StatusCode::SERVICE_UNAVAILABLE, "Webhook não configurado"))?;

    // Verify signature
    let sig = headers
        .get("stripe-signature")
        .and_then(|v| v.to_str().ok())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Cabeçalho stripe-signature ausente"))?;

    let payload = std::str::from_utf8(&body)
        .map_err(|_| err(StatusCode::BAD_REQUEST, "Payload inválido"))?;
    ...
    if timestamp.is_empty() || signature.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Formato de assinatura inválido"));
    }

    // Validate timestamp freshness (reject events older than 5 minutes)
    let ts: i64 = timestamp.parse()
        .map_err(|_| err(StatusCode::BAD_REQUEST, "Timestamp inválido"))?;
    let now = chrono::Utc::now().timestamp();
    if (now - ts).abs() > 300 {
        return Err(err(StatusCode::BAD_REQUEST, "Evento de webhook muito antigo"));
    }
    ...
    let mut mac = HmacSha256::new_from_slice(webhook_secret.as_bytes())
        .map_err(|_| err(StatusCode::INTERNAL_SERVER_ERROR, "Erro de HMAC"))?;
    mac.update(signed_payload.as_bytes());
    let expected = hex::encode(mac.finalize().into_bytes());

    if expected.as_bytes().ct_eq(signature.as_bytes()).unwrap_u8() != 1 {
        return Err(err(StatusCode::BAD_REQUEST, "Assinatura inválida"));
    }

    // Parse event
    let event: serde_json::Value = serde_json::from_str(payload)
        .map_err(|_| err(StatusCode::BAD_REQUEST, "JSON inválido"))?;

...
pub async fn remove(
    ...
    if server.is_local {
        return Err(err(StatusCode::BAD_REQUEST, "Não é possível excluir o servidor local"));
    }
...
pub async fn rotate_cert_pin(
    ...
    Ok(Json(serde_json::json!({
        "ok": true,
        "message": "Pin do certificado limpo — o próximo checkin do agente irá recapturar a fingerprint (TOFU).",
    })))
}
...
pub async fn rotate_token(
    ...
    let new_token = resp
        .get("new_token")
        .and_then(|v| v.as_str())
        .ok_or_else(|| err(StatusCode::INTERNAL_SERVER_ERROR, "Agente não retornou novo token"))?;
    ...
    Ok(Json(serde_json::json!({
        "success": true,
        "message": "Token do agente rotacionado com sucesso"
    })))
}

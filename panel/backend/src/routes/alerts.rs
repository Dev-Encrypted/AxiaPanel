...
pub async fn acknowledge(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Alerta não encontrado ou já tratado"));
    }
...
pub async fn resolve(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Alerta não encontrado ou já resolvido"));
    }
...
async fn upsert_rules(
    ...
    if let Some(ref url) = body.notify_webhook_url {
        if !url.is_empty() {
            if let Err(e) = crate::helpers::validate_url_not_internal(url).await {
                return Err(err(StatusCode::BAD_REQUEST, &format!("URL de webhook inválida: {}", e)));
            }
        }
    }
    if let Some(ref url) = body.notify_slack_url {
        if !url.is_empty() {
            if let Err(e) = crate::helpers::validate_url_not_internal(url).await {
                return Err(err(StatusCode::BAD_REQUEST, &format!("URL do Slack inválida: {}", e)));
            }
        }
    }
    if let Some(ref url) = body.notify_discord_url {
        if !url.is_empty() {
            if let Err(e) = crate::helpers::validate_url_not_internal(url).await {
                return Err(err(StatusCode::BAD_REQUEST, &format!("URL do Discord inválida: {}", e)));
            }
        }
    }

...
pub async fn analyze(
    ...
    if path.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Caminho do backup é obrigatório"));
    }
    ...
    if !["auto", "cpanel", "plesk", "hestiacp"].contains(&source) {
        return Err(err(
            ...
            "Source deve ser auto, cpanel, plesk ou hestiacp",
        ));
    }
...
pub async fn get_one(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Migração não encontrada"))?;
...
pub async fn import(
    ...
    if migration.status != "analyzed" {
        return Err(err(
            StatusCode::BAD_REQUEST,
            &format!("Migração está '{}', esperado 'analyzed'", migration.status),
        ));
    }
    ...
    if sites.is_empty() && databases.is_empty() {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Selecione ao menos um site ou banco de dados para importar",
        ));
    }
    ...
    let agent_migration_id = migration
        ...
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .ok_or_else(|| err(StatusCode::INTERNAL_SERVER_ERROR, "Inventário da migração sem ID do agent"))?;
...
pub async fn progress(
    ...
    if exists.is_none() {
        return Err(err(StatusCode::NOT_FOUND, "Migração não encontrada"));
    }
    ...
    let rx = rx.ok_or_else(|| err(StatusCode::NOT_FOUND, "Nenhuma importação ativa para esta migração"))?;

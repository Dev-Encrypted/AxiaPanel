...
pub async fn protect_all(
    ...
    if let Some((existing_id,)) = existing {
        return Err(err(StatusCode::CONFLICT,
            &format!("Política '{}' já existe (id: {})", policy_name, existing_id)));
    }
...
pub async fn delete_db_backup(
    ...
    let backup = backup.ok_or_else(|| err(StatusCode::NOT_FOUND, "Backup não encontrado"))?;
    ...
    if backup.filename.contains('/') || backup.filename.contains("..") || backup.filename.contains('\0') {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de arquivo de backup inválido"));
    }
...
pub async fn restore_db_backup(
    ...
    let (_engine, user, password_enc) =
        creds.ok_or_else(|| err(StatusCode::NOT_FOUND, "Banco de dados não encontrado"))?;
    ...
    let encryption_key: Option<String> = if backup.encrypted {
        ...
        Some(key.ok_or_else(|| err(StatusCode::BAD_REQUEST, "Backup criptografado, mas nenhuma chave de criptografia encontrada"))?)
    } else {
        None
    };
...
pub async fn create_volume_backup(
    ...
    if req.container_name.contains('/') || req.container_name.contains("..") || req.container_name.contains('\0') || req.container_name.len() > 128 {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de container inválido"));
    }
    if req.volume_name.contains('/') || req.volume_name.contains("..") || req.volume_name.contains('\0') || req.volume_name.len() > 128 {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de volume inválido"));
    }
...
pub async fn restore_volume_backup(
    ...
    let backup = backup.ok_or_else(|| err(StatusCode::NOT_FOUND, "Backup de volume não encontrado"))?;

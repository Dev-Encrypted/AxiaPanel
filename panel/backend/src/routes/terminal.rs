...
pub async fn ws_token(
    ...
    if q.site_id.is_none() && claims.role != "admin" {
        return Err(err(
            ...
            "Acesso de admin necessário para o terminal do servidor",
        ));
    }
    ...
    if q.site_id.is_none() {
        ...
        if disabled.map(|r| r.0 == "true").unwrap_or(false) {
            return Err(err(StatusCode::FORBIDDEN, "O terminal do servidor está desabilitado"));
        }
    }
    ...
    let domain = if let Some(ref sid) = q.site_id {
        ...
        match row {
            Some((d,)) => Some(d),
            None => return Err(err(StatusCode::FORBIDDEN, "Site não encontrado ou não pertence a você")),
        }
    } else {
        None
    };
...
pub async fn share_output(
    ...
    if content.is_empty() || content.len() > 500_000 {
        return Err(err(
            ...
            "Conteúdo obrigatório (máximo 500KB)",
        ));
    }
...
pub async fn revoke_share(
    ...
    if id.len() != 12 || !id.chars().all(|c| c.is_ascii_hexdigit()) {
        return Err(err(StatusCode::BAD_REQUEST, "ID de compartilhamento inválido"));
    }
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Compartilhamento não encontrado ou já expirado"));
    }
...
pub async fn view_shared(
    ...
    let raw = content
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "Compartilhamento expirado ou não encontrado"))?
        .0;

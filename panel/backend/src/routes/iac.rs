...
pub async fn create_token(
    ...
    if count.0 >= 10 {
        return Err(err(StatusCode::BAD_REQUEST, "Máximo de 10 tokens IaC por usuário"));
    }
...
pub async fn delete_token(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Token não encontrado"));
    }
...
pub async fn create_autoscale(
    ...
    let cid = body.container_id.as_deref().ok_or_else(|| err(StatusCode::BAD_REQUEST, "container_id é obrigatório"))?;
...
pub async fn update_autoscale(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Regra não encontrada"));
    }

...
pub async fn dashboard(
    ...
    if claims.role != "reseller" {
        return Err(err(
            ...
            "Este endpoint é apenas para revendedores (admin: use /api/resellers/{id})",
        ));
    }
    ...
    let profile = profile.ok_or_else(|| err(StatusCode::NOT_FOUND, "Perfil de revendedor não encontrado"))?;
...
pub async fn create_user(
    ...
    if claims.role != "reseller" {
        return Err(err(
            StatusCode::FORBIDDEN,
            "Apenas revendedores podem criar usuários por este endpoint",
        ));
    }
    ...
    let quota = quota.ok_or_else(|| err(StatusCode::NOT_FOUND, "Perfil de revendedor não encontrado"))?;

    if let Some(max) = quota.1 {
        if quota.0 >= max {
            return Err(err(
                StatusCode::FORBIDDEN,
                "Cota de usuários excedida — faça upgrade do seu plano de revendedor",
            ));
        }
    }
    ...
    if body.email.is_empty() || body.email.len() > 254 || !body.email.contains('@') {
        return Err(err(StatusCode::BAD_REQUEST, "E-mail inválido"));
    }
    ...
    if existing.is_some() {
        return Err(err(StatusCode::CONFLICT, "E-mail já cadastrado"));
    }
...
pub async fn update_user(
    ...
    let user = user.ok_or_else(|| err(StatusCode::NOT_FOUND, "Usuário não encontrado"))?;
    ...
    if let Some(max) = max {
        if used >= max {
            return Err(err(
                StatusCode::FORBIDDEN,
                &format!(
                    "Cota do revendedor excedida para {resource} ({used}/{max})"
                ),
            ));
        }
    }

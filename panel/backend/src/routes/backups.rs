...
pub async fn create(
    ...
    Ok((StatusCode::ACCEPTED, Json(serde_json::json!({
        "backup_id": backup_id,
        "message": "Criação de backup iniciada",
    }))))
}
...
pub async fn restore(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Backup não encontrado"))?;
...
pub async fn restic_restore(
    ...
    if snapshot_id.len() < 6 || !snapshot_id.chars().all(|c| c.is_ascii_hexdigit()) {
        return Err(err(StatusCode::BAD_REQUEST, "ID de snapshot inválido"));
    }

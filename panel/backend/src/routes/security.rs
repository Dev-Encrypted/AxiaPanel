...
pub async fn add_firewall_rule(
    ...
    if !["tcp", "udp", "tcp/udp"].contains(&proto.as_str()) {
        return Err(err(StatusCode::BAD_REQUEST, "Protocolo deve ser tcp, udp ou tcp/udp"));
    }
    let action = body.action.unwrap_or_else(|| "allow".to_string());
    if !["allow", "deny", "reject"].contains(&action.as_str()) {
        return Err(err(StatusCode::BAD_REQUEST, "Ação deve ser allow, deny ou reject"));
    }
...
pub async fn fail2ban_banned(
    ...
    if jail.is_empty() || !jail.chars().all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_') {
        return Err(err(StatusCode::BAD_REQUEST, "Nome do jail inválido"));
    }
...
/// POST /api/security/panel-jail/setup — Create AxiaPanel Fail2Ban jail.
...
<html><head><meta charset="utf-8"><title>AxiaPanel Security Report</title>
<style>
...
<h1>AxiaPanel Security Report</h1>
...
<div class="footer">
<p>Generated by AxiaPanel Security Scanner</p>
</div>
</body></html>"#,
...
pub async fn recordings_list(
    ...
    let dir = "/var/lib/axiapanel/recordings";

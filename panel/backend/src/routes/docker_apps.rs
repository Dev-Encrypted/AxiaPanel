...
pub async fn deploy(
    ...
    if !is_valid_name(&body.name) {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de app inválido"));
    }

    if body.port == 0 {
        return Err(err(StatusCode::BAD_REQUEST, "Porta deve estar entre 1 e 65535"));
    }

    // Validate env vars: max 50 vars, max 4KB per value
    if let Some(ref env) = body.env {
        if env.len() > 50 {
            return Err(err(StatusCode::BAD_REQUEST, "Muitas variáveis de ambiente (máx 50)"));
        }
        for (key, value) in env {
            if key.is_empty() || key.len() > 255 {
                return Err(err(StatusCode::BAD_REQUEST, "Nome de variável de ambiente inválido"));
            }
            if value.len() > 4096 {
                return Err(err(StatusCode::BAD_REQUEST, "Valor da variável de ambiente muito grande (máx 4KB)"));
            }
        }
    }
    ...
    if let Some((max_containers, max_memory, max_cpu, allowed_images)) = &policy {
        ...
        if let Ok(apps_json) = agent.get("/apps").await {
            if let Some(apps) = apps_json.as_array() {
                if apps.len() >= *max_containers as usize {
                    return Err(err(StatusCode::FORBIDDEN, &format!("Limite de containers atingido ({max_containers})")));
                }
            }
        }
        ...
        if let Some(mem) = body.memory_mb {
            if mem as i64 > *max_memory {
                return Err(err(StatusCode::FORBIDDEN, &format!("Memória excede o limite da política ({max_memory}MB)")));
            }
        }
        ...
        if let Some(cpu) = body.cpu_percent {
            if cpu as i32 > *max_cpu {
                return Err(err(StatusCode::FORBIDDEN, &format!("CPU excede o limite da política ({max_cpu}%)")));
            }
        }
        ...
        if let Some(allowed) = allowed_images {
            if !allowed.is_empty() {
                ...
                if !allowed_list.iter().any(|a| body.template_id.contains(a) || a == &"*") {
                    return Err(err(StatusCode::FORBIDDEN, "Imagem não está na lista permitida"));
                }
            }
        }
    }
...
pub async fn deploy_log(
    ...
    if claims.role != "admin" {
        ...
        if owners.get(&deploy_id) != Some(&claims.sub) {
            return Err(err(StatusCode::FORBIDDEN, "Este deploy não pertence a você"));
        }
    }
...
pub async fn stop_app(
    ...
    if !is_valid_container_id(&container_id) {
        return Err(err(StatusCode::BAD_REQUEST, "ID de container inválido"));
    }
...
pub async fn update_image(
    ...
    if image.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "image é obrigatório (ex: postgres:17)"));
    }
    ...
    if image.len() > 256 || image.contains(' ') || image.contains('\0') {
        return Err(err(StatusCode::BAD_REQUEST, "Formato de imagem inválido"));
    }
...
pub async fn update_limits(
    ...
    if memory_mb.is_none() && cpu_percent.is_none() {
        return Err(err(StatusCode::BAD_REQUEST, "Pelo menos um entre memory_mb ou cpu_percent é obrigatório"));
    }

    if let Some(mem) = memory_mb {
        if mem < 4 || mem > 65536 {
            return Err(err(StatusCode::BAD_REQUEST, "memory_mb deve estar entre 4 e 65536"));
        }
    }

    if let Some(cpu) = cpu_percent {
        if cpu == 0 || cpu > 10000 {
            return Err(err(StatusCode::BAD_REQUEST, "cpu_percent deve estar entre 1 e 10000"));
        }
    }
...
pub async fn compose_validate(
    ...
    let yaml = body["yaml"]
        .as_str()
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Campo 'yaml' ausente"))?;

    if yaml.len() > 65536 {
        return Err(err(StatusCode::BAD_REQUEST, "YAML muito grande (máx 64KB)"));
    }
...
pub async fn create_policy(
    ...
    let user_id = body.user_id.ok_or_else(|| err(StatusCode::BAD_REQUEST, "user_id é obrigatório"))?;
    ...
    if let Some(ref imgs) = body.allowed_images {
        if imgs.len() > 4096 {
            return Err(err(StatusCode::BAD_REQUEST, "allowed_images muito longo"));
        }
    }
...
pub async fn update_policy(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Política não encontrada para este usuário"));
    }
...
pub async fn delete_policy(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Política não encontrada"));
    }

...
pub async fn generate_app(
    ...
    let image = resolve_app_image(&agent, &name)
        .await?
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "Aplicativo não encontrado ou sem image"))?;
...
pub async fn download_app_sbom(
    ...
    let spdx = row
        .map(|r| r.0)
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "Nenhum SBOM gerado para esta image ainda"))?;
...
pub async fn get_image_sbom(
    ...
    let (spdx, generated_at) = row
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "Nenhum SBOM armazenado para esta image"))?;

...
pub async fn trigger(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Nenhuma configuração de deploy encontrada"))?;
    ...
    tokio::spawn(async move {
        ...
        emit("deploy", "Executando deploy", "in_progress", None);

        match execute_deploy(&db, &agent, id, &domain_clone, &config, "manual").await {
            Ok(log) => {
                let ok = log.status == "success";
                emit("deploy", "Executando deploy", if ok { "done" } else { "error" },
                    log.output.as_ref().map(|o| o.chars().take(500).collect()));
                emit("complete",
                    if ok { "Deploy concluído" } else { "Falha no deploy" },
            ...
            Err((_status, body)) => {
                ...
                emit("deploy", "Executando deploy", "error", Some(msg));
...
pub async fn webhook(
    ...
    Ok(Json(serde_json::json!({ "ok": true, "message": "Deploy disparado" })))
}
...
async fn execute_deploy(
    ...
    if success {
        notifications::notify_panel(db, user_id, &format!("Deploy concluído: {}", domain), "Deploy do site concluído com sucesso", "info", "deploy", None).await;
    } else {
        notifications::notify_panel(db, user_id, &format!("Falha no deploy: {}", domain), output, "critical", "deploy", None).await;
    }
...
pub async fn rollback_release(
    ...
    .bind(id)
    .bind(&release_id)
    .bind(format!("Rollback para a release {release_id}"))
    .execute(&state.db)
    .await
    .ok();

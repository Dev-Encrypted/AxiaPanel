...
pub async fn create(
    ...
    if !matches!(monitor_type, "http" | "tcp" | "ping" | "heartbeat") {
        return Err(err(StatusCode::BAD_REQUEST, "monitor_type deve ser 'http', 'tcp', 'ping' ou 'heartbeat'"));
    }

    let url = body.url.trim();
    if monitor_type == "http" {
        if url.is_empty() || (!url.starts_with("http://") && !url.starts_with("https://")) {
            return Err(err(StatusCode::BAD_REQUEST, "URL deve começar com http:// ou https://"));
        }
        // SSRF protection: block internal URLs
        if let Err(e) = crate::helpers::validate_url_not_internal(url).await {
            return Err(err(StatusCode::BAD_REQUEST, &format!("URL do monitor inválida: {}", e)));
        }
    } else if url.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Host/URL é obrigatório"));
    }

    let name = body.name.trim();
    if name.is_empty() || name.len() > 100 {
        return Err(err(StatusCode::BAD_REQUEST, "Nome deve ter 1-100 caracteres"));
    }
    ...
    if count.0 >= 50 {
        return Err(err(StatusCode::BAD_REQUEST, "Limite de monitores atingido (50)"));
    }
...
pub async fn update(
    ...
    if existing.is_none() {
        return Err(err(StatusCode::NOT_FOUND, "Monitor não encontrado"));
    }
...
pub async fn force_check(
    ...
    Ok(Json(serde_json::json!({ "ok": true, "message": "A verificação será executada em até 60 segundos" })))
}
...
pub async fn status_page(
    ...
    if enabled.map(|(v,)| v).unwrap_or_else(|| "false".to_string()) != "true" {
        return Err(err(StatusCode::NOT_FOUND, "Página de status não habilitada"));
    }
...
pub async fn create_maintenance(
    ...
    if starts_at.is_empty() || ends_at.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "starts_at e ends_at são obrigatórios"));
    }
...
pub async fn heartbeat(
    ...
    let monitor = monitor.ok_or_else(|| err(StatusCode::NOT_FOUND, "Monitor não encontrado"))?;

    // Verify heartbeat token
    if monitor.2.as_deref() != Some(&token) {
        return Err(err(StatusCode::UNAUTHORIZED, "Token de heartbeat inválido"));
    }

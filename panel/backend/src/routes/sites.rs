...
pub async fn create(
    ...
    if security_hardening::is_locked_down(&state.db).await {
        return Err(err(StatusCode::SERVICE_UNAVAILABLE, "Sistema em modo de bloqueio"));
    }
    ...
    // Feature 3: Rate limit site creation (max N per user per hour)
    {
        ...
        if recent.0 >= max_sites {
            ...
            return Err(err(StatusCode::TOO_MANY_REQUESTS,
                &format!("Limite de criação de sites: máx. {max_sites} sites por hora")));
        }
    }

    // Validate domain format
    if !is_valid_domain(&body.domain) {
        return Err(err(StatusCode::BAD_REQUEST, "Formato de domínio inválido"));
    }

    // Block reserved panel domains
    let reserved = ["axiapanel.dev", "panel.example.com", "docs.axiapanel.dev"];
    let domain_lower = body.domain.to_lowercase();
    if reserved.iter().any(|r| domain_lower == *r || domain_lower.ends_with(&format!(".{r}"))) {
        return Err(err(StatusCode::FORBIDDEN, "Este domínio é reservado e não pode ser usado"));
    }
    ...
    if !["static", "php", "proxy", "node", "python"].contains(&runtime) {
        return Err(err(
            ...
            "Runtime deve ser static, php, proxy, node ou python",
        ));
    }
    ...
    if runtime == "proxy" && body.proxy_port.is_none() {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "proxy_port é obrigatório para runtime proxy",
        ));
    }
    ...
    if (runtime == "node" || runtime == "python") && body.app_command.as_ref().map_or(true, |c| c.trim().is_empty()) {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "app_command é obrigatório para runtime node/python",
        ));
    }
    ...
    if let Some(ref cmd) = body.app_command {
        if cmd.contains('\n') || cmd.contains('\r') || cmd.contains('\0') {
            return Err(err(StatusCode::BAD_REQUEST, "app_command não pode conter quebras de linha ou bytes nulos"));
        }
        let forbidden = ['`', '$', '|', ';', '&', '<', '>', '\\', '!', '{', '}'];
        if cmd.chars().any(|c| forbidden.contains(&c)) {
            return Err(err(StatusCode::BAD_REQUEST, "app_command contém caracteres proibidos"));
        }
        if cmd.contains("..") {
            return Err(err(StatusCode::BAD_REQUEST, "app_command não pode conter '..'"));
        }
        if cmd.len() > 1024 {
            return Err(err(StatusCode::BAD_REQUEST, "app_command muito longo"));
        }
        ...
        if runtime == "node" {
            ...
            if !valid {
                return Err(err(StatusCode::BAD_REQUEST,
                    "app_command para node deve começar com node/npm/npx/yarn/pnpm ou ser apenas o nome de um arquivo"));
            }
            ...
            if !valid {
                return Err(err(StatusCode::BAD_REQUEST,
                    "app_command para python deve começar com python/gunicorn/uvicorn/flask/django ou ser apenas o nome de um arquivo"));
            }
        }
    }
    ...
    if let Some(ref preset) = body.php_preset {
        if !["generic", "laravel", "wordpress", "drupal", "joomla", "symfony", "codeigniter", "magento"].contains(&preset.as_str()) {
            return Err(err(
                StatusCode::BAD_REQUEST,
                "php_preset deve ser um dos seguintes: generic, laravel, wordpress, drupal, joomla, symfony, codeigniter, magento",
            ));
        }
    }
    ...
    if existing.is_some() {
        return Err(err(StatusCode::CONFLICT, "Domínio já existe"));
    }
    ...
    if git_conflict.is_some() {
        return Err(err(StatusCode::CONFLICT, "Domínio já em uso por um deploy via git"));
    }
    ...
    if let Some((Some(rid),)) = user_reseller {
        ...
        if allowed.is_none() {
            return Err(err(StatusCode::FORBIDDEN, "Este servidor não está alocado à sua conta de revendedor"));
        }
    }

    // Insert site with status "creating" inside a transaction
    let mut tx = state.db.begin().await
        .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, &format!("Falha ao iniciar transação: {e}")))?;
    ...
    .map_err(|e| {
        ...
        if msg.contains("duplicate key") || msg.contains("unique") {
            err(StatusCode::CONFLICT, "Domínio já existe")
        } else {
    ...
    match agent.put(&agent_path, agent_body).await {
        Ok(_) => {
            ...
            tx.commit().await
                .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, &format!("Falha ao confirmar transação: {e}")))?;
            ...
            // See: https://github.com/ovexro/axiapanel/issues/XX
...
pub async fn provision_log(
    ...
    if exists.is_none() {
        return Err(err(StatusCode::NOT_FOUND, "Site não encontrado"));
    }
    ...
    let rx = rx.ok_or_else(|| err(StatusCode::NOT_FOUND, "Nenhum provisionamento ativo para este site"))?;
...
pub async fn get_one(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Site não encontrado"))?;
...
pub async fn php_install(
    ...
    if claims.role != "admin" {
        return Err(err(StatusCode::FORBIDDEN, "Apenas administradores"));
    }
...
pub async fn update_limits(
    ...
    if let Some(rl) = body.rate_limit {
        if rl < 1 || rl > 10000 {
            return Err(err(StatusCode::BAD_REQUEST, "Limite de taxa deve estar entre 1 e 10000"));
        }
    }
    let max_upload = body.max_upload_mb.unwrap_or(site.max_upload_mb);
    if max_upload < 1 || max_upload > 10240 {
        return Err(err(StatusCode::BAD_REQUEST, "Upload máximo deve estar entre 1 e 10240 MB"));
    }
    let php_memory = body.php_memory_mb.unwrap_or(site.php_memory_mb);
    if php_memory < 32 || php_memory > 4096 {
        return Err(err(StatusCode::BAD_REQUEST, "Memória PHP deve estar entre 32 e 4096 MB"));
    }
    let php_workers = body.php_max_workers.unwrap_or(site.php_max_workers);
    if php_workers < 1 || php_workers > 100 {
        return Err(err(StatusCode::BAD_REQUEST, "Workers PHP devem estar entre 1 e 100"));
    }
...
pub async fn add_redirect(
    ...
    if !body.source.starts_with('/') || body.source.contains(|c: char| matches!(c, ';' | '|' | '&' | '$' | '`' | '\'' | '"' | '\\' | '\n' | '\r' | '\0')) {
        return Err(err(StatusCode::BAD_REQUEST, "Origem de redirecionamento inválida: deve começar com / e não conter metacaracteres de shell"));
    }
    ...
    if !(body.target.starts_with("http://") || body.target.starts_with("https://") || body.target.starts_with('/')) {
        return Err(err(StatusCode::BAD_REQUEST, "Destino de redirecionamento inválido: deve ser uma URL (http/https) ou caminho (começa com /)"));
    }
    if body.target.contains(|c: char| matches!(c, ';' | '|' | '&' | '$' | '`' | '\'' | '"' | '\\' | '\n' | '\r' | '\0')) {
        return Err(err(StatusCode::BAD_REQUEST, "Destino de redirecionamento inválido: contém metacaracteres de shell"));
    }
...
pub async fn add_password_protect(
    ...
    if body.path.contains("..") || body.path.contains(|c: char| matches!(c, ';' | '|' | '&' | '$' | '`' | '\'' | '"' | '\\' | '\n' | '\r' | '\0')) {
        return Err(err(StatusCode::BAD_REQUEST, "Caminho inválido: não pode conter '..' ou metacaracteres de shell"));
    }
    // Validate username: alphanumeric + underscore/hyphen only
    if body.username.is_empty() || !body.username.chars().all(|c| c.is_alphanumeric() || c == '_' || c == '-') {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de usuário inválido: deve ser alfanumérico (underscores e hífens permitidos)"));
    }
...
pub async fn add_alias(
    ...
    if !is_valid_domain(&body.alias) {
        return Err(err(StatusCode::BAD_REQUEST, "Alias inválido: deve ser um nome de domínio válido"));
    }
...
pub async fn health_summary(
    ...
    let (domain, ssl_enabled, ssl_expiry) = site.ok_or_else(|| err(StatusCode::NOT_FOUND, "Site não encontrado"))?;
...
pub async fn clone_site(
    ...
    let target_domain = body.get("domain").and_then(|v| v.as_str())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Domínio de destino obrigatório"))?;

    if !is_valid_domain(target_domain) {
        return Err(err(StatusCode::BAD_REQUEST, "Formato de domínio de destino inválido"));
    }
    ...
    let source = source.ok_or_else(|| err(StatusCode::NOT_FOUND, "Site de origem não encontrado"))?;
    ...
    .map_err(|e| {
        if e.to_string().contains("duplicate") || e.to_string().contains("unique") {
            err(StatusCode::CONFLICT, "Já existe um site com este domínio")
        } else {
...
pub async fn rename_domain(
    ...
    if new_domain == site.domain {
        return Err(err(StatusCode::BAD_REQUEST, "O novo domínio é o mesmo que o atual"));
    }
...
pub async fn toggle_enabled(
    ...
    let enabled = body.get("enabled")
        ...
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Campo booleano 'enabled' ausente"))?;
...
pub async fn toggle_fastcgi_cache(
    ...
    if site.runtime != "php" {
        return Err(err(StatusCode::BAD_REQUEST, "Cache FastCGI está disponível apenas para sites PHP"));
    }
...
pub async fn purge_fastcgi_cache(
    ...
    if !site.fastcgi_cache {
        return Err(err(StatusCode::BAD_REQUEST, "Cache FastCGI não está habilitado para este site"));
    }
...
pub async fn toggle_redis_cache(
    ...
    if site.runtime != "php" {
        return Err(err(StatusCode::BAD_REQUEST, "Cache de objetos Redis está disponível apenas para sites PHP"));
    }
    ...
    let redis_db = if enabled {
        ...
        (1..=15).find(|n| !used_dbs.contains(n))
            .ok_or_else(|| err(StatusCode::CONFLICT, "Todos os slots de DB Redis (1-15) estão em uso"))?
    } else {
        0
    };
...
pub async fn purge_redis_cache(
    ...
    if !site.redis_cache {
        return Err(err(StatusCode::BAD_REQUEST, "Cache Redis não está habilitado para este site"));
    }
...
pub async fn toggle_waf(
    ...
    let enabled = body.get("enabled").and_then(|v| v.as_bool())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Booleano 'enabled' ausente"))?;
    let mode = body.get("mode").and_then(|v| v.as_str()).unwrap_or("detection");

    if mode != "detection" && mode != "prevention" {
        return Err(err(StatusCode::BAD_REQUEST, "Modo deve ser 'detection' ou 'prevention'"));
    }
...
pub async fn optimize_images(
    ...
    if format != "webp" && format != "avif" {
        return Err(err(StatusCode::BAD_REQUEST, "Formato deve ser 'webp' ou 'avif'"));
    }
...
pub async fn update_security_headers(
    ...
    if let Some(ref csp_val) = csp {
        if csp_val.len() > 4096 {
            return Err(err(StatusCode::BAD_REQUEST, "Política CSP deve ter menos de 4KB"));
        }
        if csp_val.contains('\n') || csp_val.contains('\r') || csp_val.contains('\0') {
            return Err(err(StatusCode::BAD_REQUEST, "Política CSP contém caracteres inválidos"));
        }
    }
    if let Some(ref perms_val) = perms {
        if perms_val.len() > 2048 {
            return Err(err(StatusCode::BAD_REQUEST, "Permissions-Policy deve ter menos de 2KB"));
        }
    }
...
pub async fn toggle_bot_protection(
    ...
    if !["off", "rate-limit", "challenge", "block"].contains(&mode) {
        return Err(err(StatusCode::BAD_REQUEST, "Modo deve ser 'off', 'rate-limit', 'challenge' ou 'block'"));
    }

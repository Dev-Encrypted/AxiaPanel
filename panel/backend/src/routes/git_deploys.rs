...
pub async fn create(
    ...
    if !is_valid_name(&body.name) {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de deploy inválido"));
    }

    if body.repo_url.trim().is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "URL do repositório é obrigatória"));
    }
    ...
    let host_port = (7000..=7999)
        ...
        .ok_or_else(|| err(StatusCode::CONFLICT, "Sem ports disponíveis no intervalo 7000-7999"))?;
    ...
    if build_context.contains("..") || build_context.starts_with('/') {
        return Err(err(StatusCode::BAD_REQUEST, "build_context não pode conter '..' nem começar com '/'"));
    }
    ...
    if let Some(ref domain) = body.domain {
        if !domain.is_empty() {
            ...
            if site_conflict.is_some() {
                return Err(err(StatusCode::CONFLICT, "Domínio já em uso por um site"));
            }
            ...
            if git_conflict.is_some() {
                return Err(err(StatusCode::CONFLICT, "Domínio já em uso por outro git deployment"));
            }
        }
    }
    ...
    .map_err(|e| {
        if e.to_string().contains("duplicate key") {
            err(StatusCode::CONFLICT, "Já existe um deploy com este nome")
        } else {
...
pub async fn get_one(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Git deploy não encontrado"))?;
...
pub async fn update(
    ...
    if existing.is_none() {
        return Err(err(StatusCode::NOT_FOUND, "Git deploy não encontrado"));
    }
...
pub async fn remove(
    ...
    let (name, _domain) = deploy.ok_or_else(|| err(StatusCode::NOT_FOUND, "Git deploy não encontrado"))?;
...
pub async fn deploy(
    ...
    if active_incidents.0 > 0 {
        return Err(err(StatusCode::CONFLICT,
            "Deploy bloqueado: incidente crítico/major ativo em andamento. Resolva o incidente primeiro."));
    }
    ...
    if config.deploy_protected {
        ...
        notifications::notify_panel(
            &state.db, None,
            "Aprovação de deploy necessária",
            &format!("Deploy para {} requer aprovação", config.name),
            "warning", "deploy", Some("/git-deploys"),
        ).await;

        return Ok((StatusCode::ACCEPTED, Json(serde_json::json!({
            "status": "pending_approval",
            "message": "Deploy requer aprovação de outro admin",
        }))));
    }
    ...
    if active.0 > 0 {
        return Err(err(StatusCode::CONFLICT, "Deploy já em andamento"));
    }
...
pub async fn deploy_log(
    ...
    // Verify the caller owns this deploy (or is admin)
    {
        ...
        if let Some(&owner_id) = owners.get(&deploy_id) {
            if claims.sub != owner_id && claims.role != "admin" {
                return Err(err(StatusCode::FORBIDDEN, "Acesso negado"));
            }
        }
    ...
    let rx = rx.ok_or_else(|| err(StatusCode::NOT_FOUND, "Nenhum deploy ativo"))?;
...
pub async fn rollback(
    ...
    .await
    .map_err(|e| internal_error("rollback", e))?
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Entrada de histórico não encontrada"))?;
    ...
    tokio::spawn(async move {
        ...
        emit("deploy", "Revertendo container", "in_progress", None);
        ...
        match agent.post_long("/git/deploy", Some(deploy_body), 120).await {
            Ok(result) => {
                ...
                if blue_green {
                    emit("deploy", "Revertendo container", "done", Some("Troca sem downtime".into()));
                } else {
                    emit("deploy", "Revertendo container", "done", None);
                }
                emit("complete", "Rollback concluído", "done", None);
                ...
                .bind(format!("Rollback para {}", &rollback_commit[..7.min(rollback_commit.len())]))
                .bind(&rollback_image)
                .bind(format!("Rollback para a image {rollback_image}"))
                .bind(duration_ms)
                .execute(&db)
                .await
                {
                ...
                // Panel notification
                notifications::notify_panel(&db, Some(user_id), &format!("Rollback concluído: {}", deploy_name), &format!("Rollback para {}", &rollback_commit[..7.min(rollback_commit.len())]), "info", "deploy", Some("/git-deploys")).await;
            }
            Err(e) => {
                emit("deploy", "Revertendo container", "error", Some(format!("{e}")));
                emit("complete", "Falha no rollback", "error", None);
                ...
                // Panel notification
                notifications::notify_panel(&db, Some(user_id), &format!("Falha no rollback: {}", deploy_name), &format!("{e}"), "critical", "deploy", Some("/git-deploys")).await;
            }
        }
...
pub async fn keygen(
    ...
    let (name,) = deploy.ok_or_else(|| err(StatusCode::NOT_FOUND, "Git deploy não encontrado"))?;
...
pub async fn webhook(
    ...
    if !content_type.is_empty() && !content_type.contains("application/json") {
        return Err(err(StatusCode::BAD_REQUEST, "Content-Type deve ser application/json"));
    }
    ...
    // Rate limit: max 10 attempts per deploy per hour
    {
        ...
        if entry.0 >= 10 {
            return Err(err(StatusCode::TOO_MANY_REQUESTS, "Limite de requisições excedido. Tente novamente mais tarde."));
        }
    }
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Webhook inválido"))?;
    ...
    if provided_hash != stored_hash {
        ...
        return Err(err(StatusCode::NOT_FOUND, "Webhook inválido"));
    }

    if !config.auto_deploy {
        return Err(err(StatusCode::BAD_REQUEST, "Auto-deploy não está habilitado para este projeto"));
    }
    ...
    if active_incidents.0 > 0 {
        ...
        return Ok(Json(serde_json::json!({ "ok": false, "message": "Deploy ignorado: incidente ativo" })));
    }
    ...
    if active.0 > 0 {
        return Ok(Json(serde_json::json!({ "ok": false, "message": "Deploy já em andamento, ignorando" })));
    }
...
fn spawn_deploy_task(
    ...
    tokio::spawn(async move {
        ...
        if let Some(ref domain) = config.domain {
            emit("backup", "Backup pré-deploy", "in_progress", None);
            let _ = agent.post(
                &format!("/backups/{}/create", domain),
                Some(serde_json::json!({"reason": "pre-deploy"})),
            ).await;
            emit("backup", "Backup pré-deploy", "done", None);
        ...
        let (commit_hash, commit_message) = match &clone_result {
            ...
            Err(e) => {
                emit("clone", "Clonando repositório", "error", Some(format!("{e}")));
                emit("complete", "Falha no deploy", "error", None);
        ...
        if is_compose {
            ...
            emit("compose", "Fazendo deploy com Docker Compose", "in_progress", None);
            ...
            match agent.post_long("/apps/compose/deploy", Some(serde_json::json!({
                ...
                Ok(_) => {
                    emit("compose", "Docker Compose implantado", "done", None);
                    emit("complete", "Deploy concluído (Compose)", "done", None);
                ...
                Err(e) => {
                    emit("compose", "Falha no deploy do Docker Compose", "error", Some(format!("{e}")));
                    emit("complete", "Falha no deploy", "error", None);
                    let duration_ms = started.elapsed().as_millis() as i32;
        ...
        emit("detect", "Detectando método de build", "in_progress", None);
        match agent.post_long("/git/nixpacks-build", Some(serde_json::json!({
            ...
            Ok(result) => {
                ...
                emit("detect", "Compilado com Nixpacks", "done", None);
            ...
            Err(_) => {
                ...
                match agent.post("/git/auto-detect", Some(serde_json::json!({
                    ...
                    Ok(result) => {
                        ...
                        if auto {
                            emit("detect", "Tipo de projeto detectado automaticamente", "done", None);
                            ...
                            emit("detect", "Usando Dockerfile existente", "done", None);
                    ...
                    Err(e) => {
                        emit("detect", "Dockerfile não encontrado e detecção automática falhou", "error", Some(format!("{e}")));
                        emit("complete", "Falha no deploy", "error", None);
                        let duration_ms = started.elapsed().as_millis() as i32;
                        if let Err(db_err) = sqlx::query("INSERT INTO git_deploy_history (git_deploy_id, commit_hash, commit_message, image_tag, status, output, triggered_by, duration_ms) VALUES ($1, $2, $3, '', 'failed', $4, $5, $6)")
        ...
        if let Some(ref cmd) = config.pre_build_cmd {
            if !cmd.trim().is_empty() {
                emit("pre_build", "Executando hook pre-build", "in_progress", None);
                match agent.post_long("/git/pre-build-hook", Some(serde_json::json!({ "name": config.name, "command": cmd })), 330).await {
                    Ok(result) => {
                        let success = result.get("success").and_then(|v| v.as_bool()).unwrap_or(false);
                        if success {
                            emit("pre_build", "Executando hook pre-build", "done", None);
                        } else {
                            let output = result.get("output").and_then(|v| v.as_str()).unwrap_or("");
                            emit("pre_build", "Falha no hook pre-build", "error", Some(output.to_string()));
                        }
                    }
                    Err(e) => {
                        emit("pre_build", "Falha no hook pre-build", "error", Some(format!("{e}")));
                    }
                }
            }
        }
        ...
        let image_tag = if let Some(tag) = nixpacks_image {
            emit("build", "Image construída pelo Nixpacks", "done", None);
            tag
        } else {
        emit("build", "Construindo image Docker", "in_progress", None);
        ...
        match agent.post_long("/git/build", Some(build_body), 660).await {
            Ok(result) => {
                emit("build", "Construindo image Docker", "done", None);
                result.get("image_tag").and_then(|v| v.as_str()).unwrap_or("unknown").to_string()
            }
            Err(e) => {
                emit("build", "Construindo image Docker", "error", Some(format!("{e}")));
                emit("complete", "Falha no deploy", "error", None);
        ...
        // Step 3: Deploy
        emit("deploy", "Implantando container", "in_progress", None);
        ...
        match agent.post_long("/git/deploy", Some(deploy_body), 120).await {
            Ok(result) => {
                let blue_green = result.get("blue_green").and_then(|v| v.as_bool()).unwrap_or(false);
                if blue_green {
                    emit("deploy", "Implantando container", "done", Some("Troca sem downtime".into()));
                } else {
                    emit("deploy", "Implantando container", "done", None);
                }
                emit("complete", "Deploy concluído", "done", None);
                ...
                // Panel notification
                notifications::notify_panel(&db, Some(user_id), &format!("Deploy concluído: {}", deploy_name), &format!("Commit: {}", commit_hash), "info", "deploy", Some("/git-deploys")).await;

                // Post-deploy hook
                if let Some(ref cmd) = config.post_deploy_cmd {
                    if !cmd.trim().is_empty() {
                        emit("post_deploy", "Executando hook post-deploy", "in_progress", None);
                        match agent.post_long("/git/hook", Some(serde_json::json!({ "name": config.name, "command": cmd })), 330).await {
                            Ok(result) => {
                                let success = result.get("success").and_then(|v| v.as_bool()).unwrap_or(false);
                                let output = result.get("output").and_then(|v| v.as_str()).unwrap_or("");
                                if success {
                                    emit("post_deploy", "Hook post-deploy concluído", "done", None);
                                } else {
                                    emit("post_deploy", "Falha no hook post-deploy", "error", Some(output.to_string()));
                                }
                            }
                            Err(e) => {
                                emit("post_deploy", "Falha no hook post-deploy", "error", Some(format!("{e}")));
                            }
                        }
                    }
                }
                        ...
                        .build()
                    {
                        match client.get(&check_url).send().await {
                            Ok(resp) => {
                                ...
                                if status_code >= 500 {
                                    ...
                                    notifications::notify_panel(&db, Some(user_id),
                                        &format!("Aviso de deploy: {} retornando HTTP {}", domain, status_code),
                                        &format!("Deploy bem-sucedido, mas o site está retornando HTTP {}. Verifique os logs da aplicação.", status_code),
                            ...
                            Err(e) => {
                                ...
                                notifications::notify_panel(&db, Some(user_id),
                                    &format!("Aviso de deploy: {} inacessível", domain),
                                    &format!("Deploy bem-sucedido, mas o site não está respondendo: {}", e),
                                    "warning", "deploy", Some("/git-deploys")).await;
                            }
                        }
                    }
                                ...
                                Err(_) => {
                                    ...
                                    if let Some((prev_image, prev_commit)) = prev {
                                        ...
                                        if monitor_agent.post_long("/git/deploy", Some(rollback_body), 120).await.is_ok() {
                                            ...
                                            // Panel notification
                                            notifications::notify_panel(&monitor_db, Some(monitor_user), &format!("Auto-rollback: {}", monitor_name), "Deploy falhou, rollback para a versão anterior realizado", "warning", "deploy", Some("/git-deploys")).await;
            ...
            Err(e) => {
                emit("deploy", "Implantando container", "error", Some(format!("{e}")));
                emit("complete", "Falha no deploy", "error", None);
                ...
                // Panel notification
                notifications::notify_panel(&db, Some(user_id), &format!("Falha no deploy: {}", deploy_name), &format!("{e}"), "critical", "deploy", Some("/git-deploys")).await;
...
pub async fn trigger_deploy_task(
    ...
    match agent.post_long("/git/deploy", Some(deploy_body), 120).await {
        ...
        Err(e) => {
            ...
            record_failed_history(&db, git_deploy_id, &commit_hash, &commit_message, &format!("Falha no deploy: {e}"), &triggered_by).await;
...
pub async fn schedule_deploy(
    ...
    let deploy_at = body.get("deploy_at")
        .and_then(|v| v.as_str())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "deploy_at é obrigatório (timestamp ISO 8601)"))?;

    let scheduled_at = chrono::DateTime::parse_from_rfc3339(deploy_at)
        .map(|dt| dt.with_timezone(&chrono::Utc))
        .map_err(|_| err(StatusCode::BAD_REQUEST, "Formato de deploy_at inválido — use ISO 8601 (ex.: 2026-03-23T02:00:00Z)"))?;

    if scheduled_at <= chrono::Utc::now() {
        return Err(err(StatusCode::BAD_REQUEST, "deploy_at deve estar no futuro"));
    }
...
pub async fn approve_deploy(
    ...
    let (deploy_id, requested_by, status) = row
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "Aprovação não encontrada"))?;

    if status != "pending" {
        return Err(err(StatusCode::CONFLICT, &format!("Aprovação já está com status: {status}")));
    }

    // Cannot approve your own deploy
    if requested_by == claims.sub {
        return Err(err(StatusCode::FORBIDDEN, "Não é possível aprovar sua própria solicitação de deploy"));
    }
    ...
    Ok((StatusCode::ACCEPTED, Json(serde_json::json!({
        ...
        "message": "Deploy aprovado e iniciado",
    }))))
}
...
pub async fn reject_deploy(
    ...
    let (status,) = row.ok_or_else(|| err(StatusCode::NOT_FOUND, "Aprovação não encontrada"))?;
    ...
    Ok(Json(serde_json::json!({
        "status": "rejected",
        "message": "Solicitação de deploy rejeitada",
    })))
}

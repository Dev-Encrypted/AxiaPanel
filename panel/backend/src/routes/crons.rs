...
pub async fn create(
    ...
    if body.command.trim().is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Comando é obrigatório"));
    }
    super::is_safe_shell_command(&body.command)
        .map_err(|e| err(StatusCode::BAD_REQUEST, &format!("Comando do cron: {e}")))?;
    if body.schedule.trim().is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Schedule é obrigatório"));
    }
    let parts: Vec<&str> = body.schedule.split_whitespace().collect();
    if parts.len() != 5 {
        return Err(err(StatusCode::BAD_REQUEST, "Schedule deve ter 5 campos (minuto hora dia mês dia-da-semana)"));
    }
...
pub async fn update(
    ...
    if existing.is_none() {
        return Err(err(StatusCode::NOT_FOUND, "Cron não encontrado"));
    }
    ...
    if let Some(ref schedule) = body.schedule {
        ...
        if parts.len() != 5 {
            return Err(err(StatusCode::BAD_REQUEST, "Schedule deve ter 5 campos"));
        }
    }
...
pub async fn run_now(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Cron não encontrado"))?;

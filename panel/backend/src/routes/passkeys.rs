...
pub async fn register_complete(
    ...
    let client_data_bytes = URL_SAFE_NO_PAD.decode(&body.response.client_data_json)
        .map_err(|_| err(StatusCode::BAD_REQUEST, "Codificação clientDataJSON inválida"))?;
    let client_data: serde_json::Value = serde_json::from_slice(&client_data_bytes)
        .map_err(|_| err(StatusCode::BAD_REQUEST, "clientDataJSON inválido"))?;

    // Verify type
    let cd_type = client_data.get("type").and_then(|v| v.as_str()).unwrap_or("");
    if cd_type != "webauthn.create" {
        return Err(err(StatusCode::BAD_REQUEST, "Tipo de cerimônia inválido"));
    }
    ...
    let challenge_data = challenge_data.ok_or_else(|| err(StatusCode::BAD_REQUEST, "Challenge desconhecido ou expirado"))?;

    // Verify it's a registration challenge for this user
    match &challenge_data.0 {
        ChallengeData::Registration { user_id, .. } => {
            if *user_id != claims.sub {
                return Err(err(StatusCode::BAD_REQUEST, "Usuário do challenge não confere"));
            }
        }
        _ => return Err(err(StatusCode::BAD_REQUEST, "Tipo de challenge incorreto")),
    }

    // Verify origin
    let cd_origin = client_data.get("origin").and_then(|v| v.as_str()).unwrap_or("");
    if cd_origin != rp_origin {
        return Err(err(StatusCode::BAD_REQUEST, "Origin não confere"));
    }

    // Decode attestationObject (CBOR)
    let att_obj_bytes = URL_SAFE_NO_PAD.decode(&body.response.attestation_object)
        .map_err(|_| err(StatusCode::BAD_REQUEST, "Codificação attestationObject inválida"))?;
    let att_obj: ciborium::Value = ciborium::de::from_reader(&att_obj_bytes[..])
        .map_err(|_| err(StatusCode::BAD_REQUEST, "CBOR de attestationObject inválido"))?;
    ...
    let auth_data_bytes = match &att_obj {
        ...
        _ => None,
    }.ok_or_else(|| err(StatusCode::BAD_REQUEST, "authData ausente na atestação"))?;

    // Verify rpIdHash
    let expected_rp_hash = Sha256::digest(rp_id.as_bytes());
    if auth_data_bytes.len() < 32 || auth_data_bytes[..32] != expected_rp_hash[..] {
        return Err(err(StatusCode::BAD_REQUEST, "Hash do RP ID não confere"));
    }

    // Verify user-present flag
    let flags = auth_data_bytes[32];
    if flags & 0x01 == 0 {
        return Err(err(StatusCode::BAD_REQUEST, "Usuário não presente"));
    }

    // Parse credential data
    let (credential_id, cose_key_cbor, aaguid) = parse_auth_data(&auth_data_bytes)
        .map_err(|e| { tracing::warn!("Passkey authData parse error: {e}"); err(StatusCode::BAD_REQUEST, "Dados de atestação inválidos") })?;

    // Verify the COSE key is a valid P-256 key
    parse_cose_p256_key(&cose_key_cbor)
        .map_err(|e| { tracing::warn!("Passkey invalid public key: {e}"); err(StatusCode::BAD_REQUEST, "Chave de credencial inválida") })?;
    ...
    if count.0 >= 10 {
        return Err(err(StatusCode::BAD_REQUEST, "Máximo de 10 passkeys por conta"));
    }
...
pub async fn auth_complete(
        ...
        .unwrap_or_else(|| "unknown".to_string());
    {
        ...
        if entry.len() >= 5 {
            return Err(err(StatusCode::TOO_MANY_REQUESTS, "Muitas tentativas de login. Tente novamente em 15 minutos."));
        }
    }
    ...
    let _challenge_data = challenge_data
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Challenge desconhecido ou expirado"))?;
    ...
    let (passkey_id, user_id, cose_key_cbor, stored_count) = passkey
        .ok_or_else(|| {
            ...
            err(StatusCode::UNAUTHORIZED, "Credencial desconhecida")
        })?;

    // Decode authenticator data
    let auth_data_bytes = URL_SAFE_NO_PAD.decode(&body.response.authenticator_data)
        .map_err(|_| err(StatusCode::BAD_REQUEST, "Codificação authenticatorData inválida"))?;
    ...
    if stored_count > 0 && new_count > 0 && new_count <= stored_count {
        ...
        return Err(err(StatusCode::UNAUTHORIZED, "Credencial pode ter sido clonada"));
    }
    ...
    let verifying_key = parse_cose_p256_key(&cose_key_cbor)
        .map_err(|e| { tracing::error!("Passkey stored key invalid: {e}"); err(StatusCode::INTERNAL_SERVER_ERROR, "Falha na autenticação") })?;

    let sig_bytes = URL_SAFE_NO_PAD.decode(&body.response.signature)
        .map_err(|_| err(StatusCode::BAD_REQUEST, "Codificação de assinatura inválida"))?;

    // WebAuthn signatures are DER-encoded, convert to fixed-size for p256
    let signature = Signature::from_der(&sig_bytes)
        .map_err(|_| err(StatusCode::UNAUTHORIZED, "Formato de assinatura inválido"))?;
    ...
    verifying_key.verify(&signed_data, &signature)
        .map_err(|_| {
            ...
            err(StatusCode::UNAUTHORIZED, "Falha na verificação da assinatura")
        })?;
    ...
    let user: crate::models::User = sqlx::query_as("SELECT * FROM users WHERE id = $1")
        ...
        .ok_or_else(|| err(StatusCode::UNAUTHORIZED, "Usuário não encontrado"))?;

    // Check user is not suspended
    if user.role == "suspended" {
        return Err(err(StatusCode::FORBIDDEN, "Conta suspensa"));
    }
    ...
    if let Ok(Some((approved,))) = sqlx::query_as::<_, (bool,)>(
        ...
        if !approved {
            return Err(err(StatusCode::FORBIDDEN, "Conta pendente de aprovação do administrador"));
        }
    }

    // Check lockdown
    if user.role != "admin" && crate::services::security_hardening::is_locked_down(&state.db).await {
        return Err(err(StatusCode::SERVICE_UNAVAILABLE, "Sistema em modo de lockdown"));
    }
...
pub async fn delete_passkey(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Passkey não encontrado"));
    }
...
pub async fn rename_passkey(
    ...
    if name.is_empty() || name.len() > 255 {
        return Err(err(StatusCode::BAD_REQUEST, "Nome deve ter entre 1 e 255 caracteres"));
    }

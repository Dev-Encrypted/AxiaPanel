...
pub async fn setup(
    ...
    if count.0 > 0 {
        return Err(err(StatusCode::FORBIDDEN, "Configuração inicial já concluída"));
    }

    // Validate input
    if body.email.is_empty() || body.email.len() > 254 || !body.email.contains('@') {
        return Err(err(StatusCode::BAD_REQUEST, "É necessário um endereço de e-mail válido"));
    }
    ...
    let user = user.ok_or_else(|| err(StatusCode::FORBIDDEN, "Configuração inicial já concluída"))?;
...
pub async fn login(
    ...
    if let Some((ips,)) = whitelist {
        if !ips.is_empty() {
            ...
            if !ips.split(',').any(|allowed| allowed.trim() == client_ip) {
                return Err(err(StatusCode::FORBIDDEN, "Acesso negado: IP não autorizado"));
            }
        }
    }
    ...
    let user = match user_opt {
        Some(u) => {
            let parsed = PasswordHash::new(&u.password_hash)
                .map_err(|_| err(StatusCode::INTERNAL_SERVER_ERROR, "Erro no hash de senha"))?;
            match Argon2::default().verify_password(body.password.as_bytes(), &parsed) {
                ...
                Err(_) => {
                    ...
                    return Err(err(StatusCode::UNAUTHORIZED, "Credenciais inválidas"));
                }
            }
        }
        None => {
    ...
    if !user.email_verified {
        ...
        if smtp_configured {
            return Err(err(StatusCode::FORBIDDEN, "E-mail não verificado. Verifique sua caixa de entrada."));
        }
    }

    // Feature 9: Block non-admin logins during lockdown
    if user.role != "admin" && security_hardening::is_locked_down(&state.db).await {
        return Err(err(StatusCode::SERVICE_UNAVAILABLE, "Sistema em modo de bloqueio. Apenas administradores podem fazer login."));
    }
    ...
    if let Ok(Some((approved,))) = sqlx::query_as::<_, (bool,)>(
        ...
        if !approved {
            return Err(err(StatusCode::FORBIDDEN, "Conta aguardando aprovação do administrador"));
        }
    }
...
pub async fn register(
    ...
    if security_hardening::is_locked_down(&state.db).await {
        return Err(err(StatusCode::SERVICE_UNAVAILABLE, "Sistema em modo de bloqueio. Cadastro desabilitado."));
    }
    ...
    let allowed = reg_enabled
        ...
        .unwrap_or(false);
    if !allowed {
        return Err(err(StatusCode::FORBIDDEN, "Cadastro está desabilitado"));
    }
    ...
    if body.password.len() < 8 {
        return Err(err(StatusCode::BAD_REQUEST, "A senha deve ter pelo menos 8 caracteres"));
    }
    ...
    .map_err(|e| {
        if e.to_string().contains("unique") {
            ...
            err(StatusCode::INTERNAL_SERVER_ERROR, "Falha no cadastro. Tente novamente.")
        } else {
...
pub async fn verify_email(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::BAD_REQUEST, "Token de verificação inválido ou expirado"));
    }
...
pub async fn reset_password(
    ...
    let user = user.ok_or_else(|| err(StatusCode::BAD_REQUEST, "Token de redefinição inválido ou expirado"))?;
...
pub async fn twofa_setup(
    ...
    if user.totp_enabled {
        return Err(err(StatusCode::BAD_REQUEST, "2FA já está habilitado"));
    }
    ...
    let encrypted_secret = crate::services::secrets_crypto::encrypt_credential(&secret_base32, &state.config.jwt_secret)
        .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, &format!("Falha na criptografia: {e}")))?;
    sqlx::query(
...
pub async fn twofa_enable(
    ...
    let secret_b32_enc = user.totp_secret.ok_or_else(|| {
        err(StatusCode::BAD_REQUEST, "Chame /api/auth/2fa/setup primeiro")
    })?;
    ...
    if !totp.check_current(&body.code).map_err(|e| internal_error("2FA enable", e))? {
        return Err(err(StatusCode::UNAUTHORIZED, "Código 2FA inválido"));
    }
...
pub async fn twofa_verify(
    ...
    .map_err(|_| err(StatusCode::UNAUTHORIZED, "Token 2FA inválido ou expirado"))?;

    if token_data.claims.purpose != "2fa" {
        return Err(err(StatusCode::UNAUTHORIZED, "Propósito de token inválido"));
    }
    ...
    // Rate limit: max 5 failed 2FA attempts per 5 minutes
    {
        ...
        if let Some((count, window_start)) = attempts.get(&user_id) {
            if now.duration_since(*window_start).as_secs() < 300 && *count >= 5 {
                ...
                return Err(err(
                    StatusCode::TOO_MANY_REQUESTS,
                    &format!("Muitas tentativas. Tente novamente em {mins} minutos."),
                ));
            }
        }
    }
    ...
    let secret_b32_enc = user.totp_secret.as_ref().ok_or_else(|| {
        err(StatusCode::INTERNAL_SERVER_ERROR, "Segredo 2FA ausente")
    })?;
...
async fn try_recovery_code(
    ...
    let hashed_codes: Vec<String> = serde_json::from_str(codes_json)
        .map_err(|_| err(StatusCode::INTERNAL_SERVER_ERROR, "Códigos de recuperação corrompidos"))?;
...
pub async fn twofa_disable(
    ...
    if !user.totp_enabled {
        return Err(err(StatusCode::BAD_REQUEST, "2FA não está habilitado"));
    }
...
pub async fn change_password(
    ...
    let current = body.get("current_password").and_then(|v| v.as_str())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Senha atual obrigatória"))?;
    let new_pass = body.get("new_password").and_then(|v| v.as_str())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Nova senha obrigatória"))?;
    ...
    let hash = user.ok_or_else(|| err(StatusCode::NOT_FOUND, "Usuário não encontrado"))?.0;

    // OAuth users have no password hash — they must use their OAuth provider
    if hash.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Usuários OAuth não podem alterar senhas. Use seu provedor OAuth."));
    }
    ...
    Argon2::default()
        ...
        .map_err(|_| err(StatusCode::UNAUTHORIZED, "Senha atual incorreta"))?;
...
pub async fn revoke_session(
    ...
    if let Some((jti,)) = session {
        ...
        Err(err(StatusCode::NOT_FOUND, "Sessão não encontrada"))
    }
}

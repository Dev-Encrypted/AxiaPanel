...
pub async fn get_one(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Stack não encontrado"))?;
...
pub async fn create(
    ...
    if body.name.trim().is_empty() || body.name.len() > 100 {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de stack inválido"));
    }
    if body.yaml.len() > 65536 {
        return Err(err(StatusCode::BAD_REQUEST, "YAML muito grande (máximo 64KB)"));
    }
...
pub async fn remove(
    ...
    let (_, name) = stack.ok_or_else(|| err(StatusCode::NOT_FOUND, "Stack não encontrado"))?;
...
pub async fn update(
    ...
    if exists.is_none() {
        return Err(err(StatusCode::NOT_FOUND, "Stack não encontrado"));
    }

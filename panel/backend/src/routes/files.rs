...
pub async fn list_dir(
    ...
    if rel_path != "." && !is_safe_relative_path(rel_path) {
        return Err(err(StatusCode::BAD_REQUEST, "Caminho inválido"));
    }
...
pub async fn read_file(
    ...
    if rel_path.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "path é obrigatório"));
    }
...
pub async fn create_entry(
    ...
    if !["file", "dir"].contains(&entry_type) {
        return Err(err(StatusCode::BAD_REQUEST, "type deve ser file ou dir"));
    }
...
pub async fn upload_file(
    ...
    if !is_safe_relative_path(&body.filename) && body.filename != "." {
        if body.filename.contains("..") || body.filename.contains('/') {
            return Err(err(StatusCode::BAD_REQUEST, "Nome de arquivo inválido"));
        }
    }
    ...
    if content_size > max_upload_bytes {
        return Err(err(StatusCode::PAYLOAD_TOO_LARGE,
            &format!("Arquivo muito grande: {}MB (máx 100MB)", content_size / (1024 * 1024))));
    }
    ...
    if dangerous_exts.iter().any(|ext| lower_name.ends_with(ext)) {
        return Err(err(StatusCode::BAD_REQUEST,
            "Tipo de arquivo não permitido (extensão perigosa)"));
    }

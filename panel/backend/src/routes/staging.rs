...
pub async fn create(
    ...
    if parent.status != "active" {
        return Err(err(StatusCode::BAD_REQUEST, "O site pai deve estar ativo"));
    }
    if parent.parent_site_id.is_some() {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Não é possível criar staging a partir de um site staging",
        ));
    }
    ...
    if existing.is_some() {
        return Err(err(
            StatusCode::CONFLICT,
            "Já existe um ambiente de staging para este site",
        ));
    }
    ...
    let staging_domain = match body.domain {
        Some(ref d) if !d.is_empty() => {
            if !is_valid_domain(d) {
                return Err(err(StatusCode::BAD_REQUEST, "Formato de domínio de staging inválido"));
            }
            d.clone()
        }
    ...
    if dup.is_some() {
        return Err(err(StatusCode::CONFLICT, "Domínio de staging já está em uso"));
    }
...
pub async fn sync_to_staging(
    ...
    let staging: Site =
        sqlx::query_as("SELECT * FROM sites WHERE parent_site_id = $1")
            ...
            .ok_or_else(|| err(StatusCode::NOT_FOUND, "Nenhum ambiente de staging encontrado"))?;

...
pub async fn update_config(
    ...
    if body.api_url.is_empty() || body.api_url.len() > 512 {
        return Err(err(StatusCode::BAD_REQUEST, "URL da API inválida"));
    }
    if body.api_identifier.is_empty() || body.api_identifier.len() > 255 {
        return Err(err(StatusCode::BAD_REQUEST, "Identificador de API inválido"));
    }
    if body.api_secret.is_empty() || body.api_secret.len() > 1024 {
        return Err(err(StatusCode::BAD_REQUEST, "API secret inválido"));
    }
...
pub async fn webhook(
    ...
    let (secret, auto_provision, auto_suspend, auto_terminate) = config
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "WHMCS não configurado"))?;
    ...
    match secret {
        Some(ref expected) => {
            ...
            if provided.as_bytes().ct_eq(expected.as_bytes()).unwrap_u8() != 1 {
                return Err(err(StatusCode::UNAUTHORIZED, "Webhook secret inválido"));
            }
        }
        None => {
            // Reject webhooks when no secret is configured (prevents open-access window)
            return Err(err(StatusCode::UNAUTHORIZED, "Webhook secret não configurado"));
        }
    }
...
pub async fn start_migration(
    ...
    if target.is_none() {
        return Err(err(StatusCode::NOT_FOUND, "Servidor de destino não encontrado"));
    }
    ...
    if source_server_id == body.target_server_id {
        return Err(err(StatusCode::BAD_REQUEST, "Servidores de origem e destino são os mesmos"));
    }
...
pub async fn migration_status(
    ...
    match mig {
        ...
        None => Err(err(StatusCode::NOT_FOUND, "Migração não encontrada")),
    }
}

...
pub async fn update_settings(
    ...
    if !valid_gate(&body.deploy_gate) {
        return Err(err(StatusCode::BAD_REQUEST, "deploy_gate inválido (none|critical|high|medium)"));
    }
    if !(1..=720).contains(&body.interval_hours) {
        return Err(err(StatusCode::BAD_REQUEST, "interval_hours deve estar entre 1 e 720"));
    }
...
pub async fn scan_app(
    ...
    let image = resolve_app_image(&agent, &name)
        .await?
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "App não encontrado ou sem image"))?;
        ...
        Some((critical, high, medium, low, unknown)) => {
            ...
            if exceeds_threshold(&gate, &result) {
                return Err(err(
                    ...
                    &format!(
                        "Deploy bloqueado pelo gate de scan de image ({gate}): {} crítica(s) / {} alta(s) / {} média(s) em {}",

...
pub async fn create(
    ...
    let (user_id, email, role) =
        user.ok_or_else(|| err(StatusCode::NOT_FOUND, "Usuário não encontrado"))?;

    if role == "admin" {
        return Err(err(StatusCode::BAD_REQUEST, "Não é possível promover um admin a revendedor"));
    }
    ...
    if role == "reseller" {
        let existing: Option<(Uuid,)> =
            sqlx::query_as("SELECT id FROM reseller_profiles WHERE user_id = $1")
                ...
                .map_err(|e| internal_error("create resellers", e))?;
        if existing.is_some() {
            return Err(err(StatusCode::CONFLICT, "Perfil de revendedor já existe"));
        }
    }
...
pub async fn get(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Perfil de revendedor não encontrado"))?;
...
pub async fn allocate_server(
    ...
    let server_name = server_name
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "Servidor não encontrado"))?
        .0;
...
pub async fn deallocate_server(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Alocação de servidor não encontrada"));
    }

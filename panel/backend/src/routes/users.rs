...
pub async fn create(
    ...
    if !["admin", "reseller", "user"].contains(&role) {
        return Err(err(StatusCode::BAD_REQUEST, "Função deve ser admin, reseller ou user"));
    }
...
pub async fn toggle_suspend(
    ...
    if id == claims.sub {
        return Err(err(StatusCode::BAD_REQUEST, "Não é possível suspender sua própria conta"));
    }
...
pub async fn remove(
    ...
    if id == claims.sub {
        return Err(err(StatusCode::BAD_REQUEST, "Não é possível excluir sua própria conta"));
    }

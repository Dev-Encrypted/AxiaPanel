...
pub async fn authorize(
    ...
    let provider = get_provider(&provider_name)
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Provedor OAuth desconhecido"))?;
    ...
    let client_id = client_id
        .map(|(v,)| v)
        .filter(|v| !v.is_empty())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, &format!("OAuth do {provider_name} não configurado")))?;
...
pub async fn callback(
    ...
    if let Some(ref error) = query.error {
        let desc = query.error_description.as_deref().unwrap_or("Erro desconhecido");
        return Err(err(StatusCode::BAD_REQUEST, &format!("Erro OAuth: {error} — {desc}")));
    }
    let code = query.code.as_ref()
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Código de autorização ausente"))?;
    ...
    // Validate CSRF state
    {
        ...
        match entry {
            ...
            _ => return Err(err(StatusCode::BAD_REQUEST, "Estado OAuth inválido ou expirado")),
        }
    }
    ...
    if client_id.is_empty() || client_secret_enc.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "OAuth não totalmente configurado"));
    }
    ...
    let token_resp = http.post(provider.token_url)
        ...
        .send()
        .await
        .map_err(|e| err(StatusCode::BAD_GATEWAY, &format!("Falha na troca do token: {e}")))?;

    let token_data: serde_json::Value = token_resp.json().await
        .map_err(|e| err(StatusCode::BAD_GATEWAY, &format!("Falha ao parsear token: {e}")))?;

    let access_token = token_data.get("access_token")
        .and_then(|v| v.as_str())
        .ok_or_else(|| err(StatusCode::BAD_GATEWAY, "access_token ausente na resposta OAuth"))?;
    ...
    let userinfo: serde_json::Value = userinfo_resp.json().await
        .map_err(|e| err(StatusCode::BAD_GATEWAY, &format!("Falha ao parsear userinfo: {e}")))?;
    ...
    let email = email.ok_or_else(|| err(StatusCode::BAD_GATEWAY, "Não foi possível obter o e-mail do provedor OAuth"))?;
    ...
    if oauth_id.is_empty() {
        return Err(err(StatusCode::BAD_GATEWAY, "Provedor OAuth não retornou ID de usuário"));
    }
        ...
        Some(mut u) => {
            ...
            if u.oauth_provider.is_none() && u.password_hash.is_empty() {
                ...
                return Err(err(StatusCode::CONFLICT,
                    "Já existe uma conta com este e-mail. Faça login com sua senha e vincule o OAuth nas Configurações."));
            }
            u
        }
        None => {
            ...
            if !auto_create {
                return Err(err(StatusCode::FORBIDDEN, "Auto-registro via OAuth desabilitado. Contate seu administrador."));
            }
            ...
            .map_err(|e| {
                if e.to_string().contains("duplicate key") {
                    err(StatusCode::CONFLICT, "E-mail já cadastrado")
                } else {
    ...
    if user.totp_enabled {
        ...
        .map_err(|e| err(StatusCode::INTERNAL_SERVER_ERROR, &format!("Falha ao codificar JWT: {e}")))?;

...
pub async fn create(
    ...
    if count.0 >= 10 {
        return Err(err(StatusCode::BAD_REQUEST, "Limite de equipes atingido (10)"));
    }
...
pub async fn remove(
    ...
    let team = team.ok_or_else(|| err(StatusCode::NOT_FOUND, "Equipe não encontrada ou você não é o proprietário"))?;
...
pub async fn invite(
    ...
    if member_role != "owner" && member_role != "admin" {
        return Err(err(StatusCode::FORBIDDEN, "Apenas proprietários e admins podem convidar membros"));
    }

    let email_addr = body.email.trim().to_lowercase();
    if email_addr.is_empty() || !email_addr.contains('@') {
        return Err(err(StatusCode::BAD_REQUEST, "E-mail válido é obrigatório"));
    }

    let role = body.role.as_deref().unwrap_or("viewer");
    if !["admin", "developer", "viewer"].contains(&role) {
        return Err(err(StatusCode::BAD_REQUEST, "O papel deve ser admin, developer ou viewer"));
    }
    ...
    if existing.is_some() {
        return Err(err(StatusCode::CONFLICT, "O usuário já é um membro"));
    }
    ...
    let _ = email::send_email(
        ...
        &format!(
            "<h2>Você foi convidado!</h2>\
             ...
             <p><a href=\"{invite_url}\" style=\"padding:10px 20px;background:#4F46E5;color:#fff;text-decoration:none;border-radius:6px\">Aceitar Convite</a></p>\
    ...
    Ok(Json(serde_json::json!({ "ok": true, "message": "Convite enviado" })))
}
...
pub async fn accept_invite(
    ...
    let (invite_id, team_id, _email, role) = invite
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Convite inválido ou expirado"))?;
...
pub async fn update_member(
    ...
    if caller_role != "owner" && caller_role != "admin" {
        return Err(err(StatusCode::FORBIDDEN, "Permissões insuficientes"));
    }
    ...
    let target_role = target.ok_or_else(|| err(StatusCode::NOT_FOUND, "Membro não encontrado"))?.0;
    if target_role == "owner" {
        return Err(err(StatusCode::FORBIDDEN, "Não é possível alterar o papel do proprietário"));
    }
...
pub async fn remove_member(
    ...
    let target_role = target.ok_or_else(|| err(StatusCode::NOT_FOUND, "Membro não encontrado"))?.0;
    if target_role == "owner" {
        return Err(err(StatusCode::FORBIDDEN, "Não é possível remover o proprietário da equipe"));
    }
...
async fn get_member_role(state: &AppState, team_id: Uuid, user_id: Uuid) -> Result<String, ApiError> {
    ...
    row.map(|r| r.0)
        .ok_or_else(|| err(StatusCode::FORBIDDEN, "Não é membro desta equipe"))
}

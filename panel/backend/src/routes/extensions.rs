...
pub async fn create(
    ...
    if name.is_empty() || name.len() > 100 {
        return Err(err(
            ...
            "Nome deve ter 1-100 caracteres",
        ));
    }
    ...
    Ok((
        ...
        Json(serde_json::json!({
            ...
            "message": "Salve estas credenciais — elas não serão exibidas novamente.",
        })),
    ))
}
...
pub async fn update(
    ...
    if existing.is_none() {
        return Err(err(StatusCode::NOT_FOUND, "Extensão não encontrada"));
    }
    ...
    if sets.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Nenhum campo para atualizar"));
    }
...
pub async fn remove(
    ...
    let ext_name = ext_info
        .map(|(n,)| n)
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "Extensão não encontrada"))?;
...
pub async fn test_webhook(
    ...
    let (name, webhook_url, webhook_secret) =
        ext.ok_or_else(|| err(StatusCode::NOT_FOUND, "Extensão não encontrada"))?;
    ...
    let signature = match HmacSha256::new_from_slice(webhook_secret.as_bytes()) {
        ...
        Err(_) => {
            return Err(err(StatusCode::INTERNAL_SERVER_ERROR, "Chave HMAC inválida — rotacione o segredo do webhook"));
        }
    };

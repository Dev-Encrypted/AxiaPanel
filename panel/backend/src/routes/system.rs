...
pub async fn updates_apply(
    ...
    tokio::spawn(async move {
        ...
        emit("update", "Aplicando atualizações do sistema", "in_progress", None);
            ...
            match ev_type {
                ...
                "done" => {
                    ...
                    let (update_status, complete_label, complete_status) = if success {
                        ("done", "Atualizações aplicadas", "done")
                    } else {
                        ("error", "Atualizações finalizadas com erros", "error")
                    };

                    for (step, label, status) in [
                        ("update", "Aplicando atualizações do sistema", update_status),
        ...
        match agent.post_long_ndjson("/system/updates/apply", Some(body), 300, emit_line).await {
            ...
            Err(e) => {
                emit("update", "Falha ao aplicar atualizações", "error", Some(format!("{e}")));
                emit("complete", "Atualização falhou", "error", None);
            }
        }
    ...
    Ok((StatusCode::ACCEPTED, Json(serde_json::json!({
        ...
        "message": "Atualizações iniciadas",
    }))))
}
...
async fn install_service_with_log(
    ...
    tokio::spawn(async move {
        ...
        emit("install", &format!("Instalando {svc}"), "in_progress", None);

        match agent.post(&path, None).await {
            Ok(_) => {
                emit("install", &format!("Instalando {svc}"), "done", None);
                emit("complete", &format!("{svc} instalado"), "done", None);
                activity::log_activity(
            ...
            Err(e) => {
                emit("install", &format!("Instalando {svc}"), "error", Some(format!("{e}")));
                emit("complete", "Instalação falhou", "error", None);
    ...
    Ok((StatusCode::ACCEPTED, Json(serde_json::json!({
        "install_id": install_id,
        "message": format!("Instalação de {service_name} iniciada"),
    }))))
}
...
pub async fn install_log(
    ...
    let rx = rx.ok_or_else(|| err(StatusCode::NOT_FOUND, "Nenhuma instalação ativa"))?;
...
pub async fn install_powerdns(
    ...
    tokio::spawn(async move {
        ...
        emit("install", "Instalando PowerDNS", "in_progress", None);
        ...
        match agent.post("/services/install/powerdns", None).await {
            Ok(result) => {
                ...
                emit("install", "Instalando PowerDNS", "done", None);
                emit("complete", "PowerDNS instalado", "done", None);
                activity::log_activity(
            ...
            Err(e) => {
                emit("install", "Instalando PowerDNS", "error", Some(format!("{e}")));
                emit("complete", "Instalação falhou", "error", None);
    ...
    Ok((StatusCode::ACCEPTED, Json(serde_json::json!({
        "install_id": install_id,
        "message": "Instalação do PowerDNS iniciada",
    }))))
}

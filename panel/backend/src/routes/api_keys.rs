...
pub async fn create(
    ...
    if count.0 >= 10 {
        return Err(err(StatusCode::BAD_REQUEST, "Máximo de 10 API keys por conta"));
    }
...
pub async fn revoke(
    ...
    let key_name = key_info
        .map(|(n,)| n)
        .ok_or_else(|| err(StatusCode::NOT_FOUND, "Chave de API não encontrada"))?;
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Chave de API não encontrada"));
    }
...
pub async fn rotate(
    ...
    Ok((
        ...
        Json(serde_json::json!({
            ...
            "prefix": prefix,
            "name": name,
            "message": "Chave rotacionada — salve a nova chave, a antiga agora é inválida.",
        })),
    ))
}

...
pub async fn mail_install(
    ...
    tokio::spawn(async move {
        ...
        emit("install", "Instalando servidor de e-mail", "in_progress", None);

        match agent.post("/mail/install", None).await {
            Ok(_) => {
                emit("install", "Instalando servidor de e-mail", "done", None);
                emit("complete", "Servidor de e-mail instalado", "done", None);
            ...
            Err(e) => {
                emit("install", "Instalando servidor de e-mail", "error", Some(format!("{e}")));
                emit("complete", "Falha na instalação", "error", None);
    ...
    Ok((StatusCode::ACCEPTED, Json(serde_json::json!({
        ...
        "message": "Instalação do servidor de e-mail iniciada",
    }))))
}
...
pub async fn mail_uninstall(
    ...
    tokio::spawn(async move {
        ...
        emit("uninstall", "Desinstalando servidor de e-mail", "in_progress", None);

        match agent.post("/mail/uninstall", None).await {
            Ok(_) => {
                emit("uninstall", "Desinstalando servidor de e-mail", "done", None);
                emit("complete", "Servidor de e-mail desinstalado", "done", None);
                activity::log_activity(
            ...
            Err(e) => {
                emit("uninstall", "Desinstalando servidor de e-mail", "error", Some(format!("{e}")));
                emit("complete", "Falha na desinstalação", "error", None);
    ...
    Ok((StatusCode::ACCEPTED, Json(serde_json::json!({
        "install_id": install_id,
        "message": "Desinstalação do servidor de e-mail iniciada",
    }))))
}
...
pub async fn create_domain(
    ...
    if !crate::routes::is_valid_domain(&domain) {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de domínio inválido"));
    }
...
pub async fn update_domain(
    ...
    let domain = domain.ok_or_else(|| err(StatusCode::NOT_FOUND, "Domínio não encontrado"))?;
    ...
    if let Some(catch_all) = &body.catch_all {
        if !catch_all.is_empty() {
            ...
            {
                return Err(err(StatusCode::BAD_REQUEST, "Endereço de e-mail catch-all inválido"));
            }
        }
...
pub async fn domain_dns(
    ...
    let (domain, selector, dkim_pub) = domain.ok_or_else(|| err(StatusCode::NOT_FOUND, "Domínio não encontrado"))?;
...
pub async fn create_account(
    ...
    if !email.contains('@') || !email.ends_with(&format!("@{}", domain.0)) {
        return Err(err(StatusCode::BAD_REQUEST, &format!("O e-mail deve terminar com @{}", domain.0)));
    }
    ...
    .map_err(|e| {
        if e.to_string().contains("duplicate") {
            err(StatusCode::CONFLICT, "Conta de e-mail já existe")
        } else {
...
pub async fn update_account(
    ...
    let account = account.ok_or_else(|| err(StatusCode::NOT_FOUND, "Conta não encontrada"))?;
    ...
    if let Some(forward) = &body.forward_to {
        if !forward.is_empty() {
            ...
            {
                return Err(err(StatusCode::BAD_REQUEST, "Endereço de e-mail de encaminhamento inválido"));
            }
        }
...
pub async fn create_alias(
    ...
    .map_err(|e| {
        if e.to_string().contains("duplicate") {
            err(StatusCode::CONFLICT, "Alias já existe")
        } else {
...
pub async fn blacklist_check(
    ...
    if ip.is_empty() {
        return Err(err(StatusCode::INTERNAL_SERVER_ERROR, "Falha na consulta de IP: não foi possível detectar o IP público"));
    }

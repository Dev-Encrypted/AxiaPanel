...
pub async fn dispatch(
    ...
    if !ALLOWED_ACTIONS.contains(&body.action.as_str()) {
        return Err(err(StatusCode::BAD_REQUEST, "Ação inválida"));
    }
    ...
    if exists.is_none() {
        return Err(err(StatusCode::NOT_FOUND, "Servidor não encontrado"));
    }
...
pub async fn command_status(
    ...
    let cmd = row.ok_or_else(|| err(StatusCode::NOT_FOUND, "Comando não encontrado"))?;

...
pub async fn system_logs(
    ...
    if !["nginx_access", "nginx_error", "syslog", "auth", "php_fpm"].contains(&log_type) {
        return Err(err(StatusCode::BAD_REQUEST, "Tipo de log inválido"));
    }
...
pub async fn site_logs(
    ...
    let (domain,) = row.ok_or_else(|| err(StatusCode::NOT_FOUND, "Site não encontrado"))?;

    let log_type = q.log_type.as_deref().unwrap_or("access");
    if !["access", "error"].contains(&log_type) {
        return Err(err(StatusCode::BAD_REQUEST, "Tipo de log de site inválido"));
    }
    let lines = q.lines.unwrap_or(100).max(1).min(1000);
...
pub async fn search_system_logs(
    ...
    if pattern.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Pattern é obrigatório"));
    }
...
pub async fn search_site_logs(
    ...
    let full_type = match log_type {
        ...
        _ => return Err(err(StatusCode::BAD_REQUEST, "Tipo de log de site inválido")),
    };
...
pub async fn stream_token(
    ...
    if q.site_id.is_none() && claims.role != "admin" {
        return Err(err(
            ...
            "Acesso de administrador necessário para streaming de logs do sistema",
        ));
    }
    ...
    if let Some(ref sid) = q.site_id {
        let site_id: uuid::Uuid = sid
            .parse()
            .map_err(|_| err(StatusCode::BAD_REQUEST, "site_id inválido"))?;
...
pub async fn service_logs(
    ...
    if service.is_empty() || !service.chars().all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_' || c == '.') {
        return Err(err(StatusCode::BAD_REQUEST, "Nome de serviço inválido"));
    }
    let lines = params
        .get("lines")
...
pub async fn check_errors(
    ...
    Ok(Json(serde_json::json!({
        ...
        "threshold": "Taxa de erro de 5% com mais de 10 erros",

...
fn get_encryption_key(jwt_secret: &str) -> String {
    std::env::var("SECRETS_ENCRYPTION_KEY").unwrap_or_else(|_| {
        ...
        hasher.update(b"axiapanel-secrets-encryption:");
...
pub async fn delete_vault(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Vault não encontrado"));
    }
...
pub async fn update_vault(
    ...
    let vault = vault.ok_or_else(|| err(StatusCode::NOT_FOUND, "Vault não encontrado"))?;
...
async fn verify_vault(state: &AppState, vault_id: Uuid, user_id: Uuid) -> Result<(), ApiError> {
    ...
    exists.map(|_| ()).ok_or_else(|| err(StatusCode::NOT_FOUND, "Vault não encontrado"))
}
...
pub async fn create_secret(
    ...
    if req.key.is_empty() || req.key.len() > 200 {
        return Err(err(StatusCode::BAD_REQUEST, "A chave deve ter de 1 a 200 caracteres"));
    }
    if req.value.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "O valor não pode ficar vazio"));
    }

    let secret_type = req.secret_type.as_deref().unwrap_or("env");
    if !VALID_TYPES.contains(&secret_type) {
        return Err(err(StatusCode::BAD_REQUEST, "secret_type inválido"));
    }
...
pub async fn update_secret(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Secret não encontrado"))?;
...
pub async fn delete_secret(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Secret não encontrado"));
    }
...
pub async fn inject_to_site(
    ...
    if rows.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Nenhum secret de auto-injeção neste vault"));
    }
...
pub async fn export_vault(
    ...
    // Export with encrypted values (portable — can be imported on another AxiaPanel with same key)
...
pub async fn import_vault(
    ...
    let secrets_arr = data.get("secrets").and_then(|v| v.as_array())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Array 'secrets' ausente"))?;

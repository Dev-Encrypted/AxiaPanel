...
pub async fn create_endpoint(
    ...
    if !["none", "hmac_sha256", "hmac_sha1"].contains(&verify_mode) {
        return Err(err(StatusCode::BAD_REQUEST, "verify_mode inválido"));
    }
...
pub async fn delete_endpoint(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Endpoint não encontrado"));
    }
...
pub async fn list_deliveries(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Endpoint não encontrado"))?;
...
pub async fn replay_delivery(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Entrega não encontrada"))?;
...
pub async fn create_route(
    ...
    if req.destination_url.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "destination_url obrigatória"));
    }
    ...
    if let Err(e) = crate::helpers::validate_url_not_internal(&req.destination_url).await {
        return Err(err(StatusCode::BAD_REQUEST, &format!("URL de destino inválida: {}", e)));
    }
...
pub async fn delete_route(
    ...
    if result.rows_affected() == 0 {
        return Err(err(StatusCode::NOT_FOUND, "Rota não encontrada"));
    }
...
pub async fn receive_webhook(
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Endpoint de webhook inválido"))?;
    ...
    if signature_valid == Some(false) {
        return Err(err(StatusCode::UNAUTHORIZED, "Assinatura de webhook inválida"));
    }

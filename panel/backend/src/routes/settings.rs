...
pub async fn update(
    ...
    for key in body.keys() {
        if !allowed_keys.contains(&key.as_str()) {
            return Err(err(StatusCode::BAD_REQUEST, &format!("Configuração desconhecida: {key}")));
        }
    }
    ...
    if let Some(url) = body.get("logo_url") {
        if !url.is_empty() && !url.starts_with("https://") && !url.starts_with("http://") && !url.starts_with("/") {
            return Err(err(StatusCode::BAD_REQUEST, "logo_url deve ser uma URL HTTP(S) ou caminho relativo"));
        }
    }
    ...
    if let Some(color) = body.get("accent_color") {
        if !color.is_empty() {
            ...
            if !valid {
                return Err(err(StatusCode::BAD_REQUEST, "accent_color deve ser uma cor hex válida (#rrggbb), rgb() ou hsl()"));
            }
        }
    }
...
pub async fn test_email(
    ...
    if to.is_empty() || !to.contains('@') {
        return Err(err(StatusCode::BAD_REQUEST, "Endereço de e-mail válido é obrigatório"));
    }
...
pub async fn test_webhook(
    ...
    let url = body.get("url").ok_or_else(|| err(StatusCode::BAD_REQUEST, "URL obrigatória"))?;
    let service = body.get("service").unwrap_or(&"webhook".to_string()).clone();

    if url.is_empty() || !url.starts_with("https://") {
        return Err(err(StatusCode::BAD_REQUEST, "URL de webhook inválida"));
    }
    ...
    let resp = client.post(url).json(&payload).send().await
        .map_err(|e| err(StatusCode::BAD_GATEWAY, &format!("Falha na requisição do webhook: {e}")))?;

    if !resp.status().is_success() {
        return Err(err(StatusCode::BAD_GATEWAY, &format!("Webhook retornou {}", resp.status())));
    }
...
pub async fn import_config(
    ...
    let settings_obj = body.get("settings").and_then(|s| s.as_object())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Formato inválido: objeto 'settings' ausente"))?;

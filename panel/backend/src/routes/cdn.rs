...
async fn get_zone(state: &AppState, zone_id: Uuid, user_id: Uuid) -> Result<CdnZone, ApiError> {
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Zona CDN não encontrada"))
}
...
pub async fn create_zone(
    ...
    if body.api_key.trim().is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "API key é obrigatória"));
    }

    let provider = body.provider.as_deref().unwrap_or("bunnycdn");
    if provider != "bunnycdn" && provider != "cloudflare" {
        return Err(err(StatusCode::BAD_REQUEST, "Provider deve ser 'bunnycdn' ou 'cloudflare'"));
    }
    ...
    if provider == "bunnycdn" {
        ...
        let resp = client
            ...
            .map_err(|e| {
                ...
                err(StatusCode::BAD_REQUEST, "Falha ao conectar à API do BunnyCDN. Verifique sua API key.")
            })?;

        if resp.status() == 401 || resp.status() == 403 {
            return Err(err(StatusCode::BAD_REQUEST, "API key do BunnyCDN inválida"));
        }
    }
...
pub async fn purge_cache(
    ...
    if let Some(ref urls) = body.urls {
        if urls.len() > 100 {
            return Err(err(StatusCode::BAD_REQUEST, "Máximo de 100 URLs por requisição de purge"));
        }
        for u in urls {
            if let Ok(parsed) = url::Url::parse(u) {
                let host = parsed.host_str().unwrap_or("");
                if !host.ends_with(&zone.domain) && host != zone.domain {
                    return Err(err(StatusCode::BAD_REQUEST, "URL de purge não corresponde ao domínio da zona"));
                }
            } else {
                return Err(err(StatusCode::BAD_REQUEST, "URL de purge inválida"));
            }
        }
    }
    ...
    if let Some(ref pzid) = zone.pull_zone_id {
        if !pzid.chars().all(|c| c.is_ascii_alphanumeric()) {
            return Err(err(StatusCode::BAD_REQUEST, "Formato de pull zone ID inválido"));
        }
    }
    ...
    match zone.provider.as_str() {
        "bunnycdn" => {
            let pull_zone_id = zone.pull_zone_id.as_deref()
                .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Pull zone ID não configurado"))?;
            ...
            let resp = client
                ...
                .map_err(|e| {
                    ...
                    err(StatusCode::BAD_GATEWAY, "Falha ao purgar o cache do BunnyCDN")
                })?;
            ...
            if !resp.status().is_success() {
                ...
                return Err(err(StatusCode::BAD_GATEWAY, &format!("BunnyCDN retornou HTTP {status}")));
            }
        }
        "cloudflare" => {
            let pull_zone_id = zone.pull_zone_id.as_deref()
                .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Zone ID do Cloudflare não configurado"))?;
            ...
            let resp = client
                ...
                .map_err(|e| {
                    ...
                    err(StatusCode::BAD_GATEWAY, "Falha ao purgar o cache do Cloudflare")
                })?;

            if !resp.status().is_success() {
                let status = resp.status().as_u16();
                return Err(err(StatusCode::BAD_GATEWAY, &format!("Cloudflare retornou HTTP {status}")));
            }
        }
        _ => return Err(err(StatusCode::BAD_REQUEST, "Provider de CDN desconhecido")),
    }
    ...
    Ok(Json(serde_json::json!({ "ok": true, "message": "Cache purgado com sucesso" })))
}
...
pub async fn zone_stats(
    ...
    match zone.provider.as_str() {
        "bunnycdn" => {
            ...
            let resp = client
                ...
                .map_err(|e| {
                    ...
                    err(StatusCode::BAD_GATEWAY, "Falha ao obter estatísticas do BunnyCDN")
                })?;
            ...
            let body: serde_json::Value = resp.json().await.map_err(|e| {
                ...
                err(StatusCode::BAD_GATEWAY, "Resposta inválida do BunnyCDN")
            })?;
        ...
        "cloudflare" => {
            ...
            let resp = client
                ...
                .map_err(|e| {
                    ...
                    err(StatusCode::BAD_GATEWAY, "Falha ao obter estatísticas do Cloudflare")
                })?;

            let body: serde_json::Value = resp.json().await.map_err(|e| {
                tracing::warn!("Cloudflare stats parse error: {e}");
                err(StatusCode::BAD_GATEWAY, "Resposta inválida do Cloudflare")
            })?;
        ...
        _ => Err(err(StatusCode::BAD_REQUEST, "Provider de CDN desconhecido")),
    }
}
        ...
        "bunnycdn" => {
            ...
            let resp = client
                ...
                .map_err(|e| {
                    ...
                    err(StatusCode::BAD_GATEWAY, "Falha na conexão com a API do CDN")
                })?;

            if resp.status().is_success() {
                Ok(Json(serde_json::json!({ "ok": true, "message": "API key do BunnyCDN é válida" })))
            } else {
                Ok(Json(serde_json::json!({ "ok": false, "message": format!("BunnyCDN retornou HTTP {}", resp.status()) })))
            }
        }
        "cloudflare" => {
            let headers = crate::helpers::cf_headers(&zone.api_key, None);
            ...
            Ok(Json(serde_json::json!({
                "ok": success,
                "message": if success { "Token da API do Cloudflare é válido" } else { "Token da API do Cloudflare é inválido" },
            })))
        }
        _ => Err(err(StatusCode::BAD_REQUEST, "Provider de CDN desconhecido")),
    }
    ...
    match zone.provider.as_str() {
        "bunnycdn" => {
            ...
            let resp = client
                ...
                .send()
                .await
                .map_err(|e| err(StatusCode::BAD_GATEWAY, &format!("Falha ao listar pull zones: {e}")))?;

            if !resp.status().is_success() {
                return Err(err(StatusCode::BAD_GATEWAY, "Falha ao listar pull zones do BunnyCDN"));
            }
        ...
        "cloudflare" => {
            ...
            let resp = client
                ...
                .send()
                .await
                .map_err(|e| err(StatusCode::BAD_GATEWAY, &format!("Falha ao listar zonas: {e}")))?;

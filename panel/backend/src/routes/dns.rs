...
async fn get_zone(state: &AppState, zone_id: Uuid, user_id: Uuid) -> Result<DnsZone, ApiError> {
    ...
    .ok_or_else(|| err(StatusCode::NOT_FOUND, "Zona DNS não encontrada"))
}
...
fn cf_client(token: &str, email: Option<&str>) -> Result<(reqwest::Client, reqwest::header::HeaderMap), ApiError> {
    ...
    if headers.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Credenciais do Cloudflare inválidas"));
    }
    headers.insert(
        "Content-Type",
...
fn pdns_client(api_key: &str) -> Result<(reqwest::Client, reqwest::header::HeaderMap), ApiError> {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_str(api_key)
            .map_err(|_| err(StatusCode::BAD_REQUEST, "Chave de API do PowerDNS inválida"))?,
    );
    headers.insert(
        "Content-Type",
        reqwest::header::HeaderValue::from_static("application/json"),
...
fn pdns_parse_record_id(id: &str) -> Result<(String, String, String), ApiError> {
    let bytes = hex::decode(id).map_err(|_| err(StatusCode::BAD_REQUEST, "ID de registro inválido"))?;
    let s = String::from_utf8(bytes).map_err(|_| err(StatusCode::BAD_REQUEST, "ID de registro inválido"))?;
    let parts: Vec<&str> = s.splitn(3, '\0').collect();
    if parts.len() != 3 {
        return Err(err(StatusCode::BAD_REQUEST, "ID de registro inválido"));
    }
...
pub async fn create_zone(
    ...
    if body.domain.trim().is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Domínio é obrigatório"));
    }
    ...
    match provider {
        "cloudflare" => {
            ...
            if cf_zone_id.is_empty() || cf_api_token.is_empty() {
                return Err(err(StatusCode::BAD_REQUEST, "Cloudflare Zone ID e API token são obrigatórios"));
            }
            ...
            .map_err(|e| {
                if e.to_string().contains("duplicate") {
                    err(StatusCode::CONFLICT, "Zona já existe")
                } else {
        ...
        "powerdns" => {
            ...
            if !status.is_success() {
                ...
                if status.as_u16() != 409 {
                    return Err(err(StatusCode::BAD_GATEWAY, &format!("Erro do PowerDNS: {body_text}")));
                }
            }
        ...
        _ => Err(err(StatusCode::BAD_REQUEST, "Provider não suportado. Use 'cloudflare' ou 'powerdns'.")),
    }
}
...
pub async fn list_records(
    ...
    match zone.provider.as_str() {
        "cloudflare" => {
            ...
            if !cf_resp.get("success").and_then(|v| v.as_bool()).unwrap_or(false) {
                return Err(err(StatusCode::BAD_GATEWAY, "Falha ao buscar registros DNS no Cloudflare"));
            }
        ...
        "powerdns" => {
            ...
            if !resp.status().is_success() {
                ...
                return Err(err(StatusCode::BAD_GATEWAY, &format!("Erro do PowerDNS: {body}")));
            }
        ...
        _ => Err(err(StatusCode::BAD_REQUEST, "Provider DNS desconhecido")),
    }
}
...
pub async fn create_record(
    ...
    if !allowed_types.contains(&body.rtype.as_str()) {
        return Err(err(StatusCode::BAD_REQUEST, &format!("Tipo de registro não suportado: {}", body.rtype)));
    }
    ...
    match zone.provider.as_str() {
        "cloudflare" => {
            ...
            if !cf_resp.get("success").and_then(|v| v.as_bool()).unwrap_or(false) {
                let errors = cf_resp.get("errors").cloned().unwrap_or_default();
                return Err(err(StatusCode::UNPROCESSABLE_ENTITY, &format!("Erro do Cloudflare: {errors}")));
            }
        ...
        "powerdns" => {
            ...
            if !resp.status().is_success() {
                let body_text = resp.text().await.unwrap_or_default();
                return Err(err(StatusCode::UNPROCESSABLE_ENTITY, &format!("Erro do PowerDNS: {body_text}")));
            }
...
pub async fn check_propagation(
    ...
    let name = body
        .get("name")
        .and_then(|v| v.as_str())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "name é obrigatório"))?;
    let rtype = body
        .get("type")
        .and_then(|v| v.as_str())
        .unwrap_or("A");
    ...
    {
        return Err(err(StatusCode::BAD_REQUEST, "Nome DNS inválido"));
    }
    let valid_types = ["A", "AAAA", "CNAME", "MX", "TXT", "NS", "SRV", "CAA"];
    if !valid_types.contains(&rtype) {
        return Err(err(StatusCode::BAD_REQUEST, "Tipo de registro inválido"));
    }
...
pub async fn dns_health_check(
    ...
    let domain = body
        .get("domain")
        .and_then(|v| v.as_str())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "domain é obrigatório"))?;
    ...
    {
        return Err(err(StatusCode::BAD_REQUEST, "Domínio inválido"));
    }
...
pub async fn dnssec_status(
    ...
    if zone.provider != "cloudflare" {
        return Ok(Json(serde_json::json!({
            "supported": false,
            "message": "Gerenciamento de DNSSEC disponível apenas para zonas Cloudflare",
        })));
    }
...
pub async fn dns_analytics(
    ...
    if zone.provider != "cloudflare" {
        return Ok(Json(serde_json::json!({
            "supported": false,
            "message": "Analytics disponível apenas para zonas Cloudflare",
        })));
    }
    ...
    if !status.is_success() {
        ...
        return Ok(Json(serde_json::json!({
            "supported": true,
            "available": false,
            "message": "Analytics de DNS não disponível (pode exigir um plano pago do Cloudflare)",
        })));
    }
...
pub async fn cf_zone_settings(
    ...
    if zone.provider != "cloudflare" {
        return Ok(Json(serde_json::json!({
            "supported": false,
            "message": "Gerenciamento de configurações disponível apenas para zonas Cloudflare",
        })));
    }
...
pub async fn cf_update_setting(
    ...
    if zone.provider != "cloudflare" {
        return Err(err(StatusCode::BAD_REQUEST, "Disponível apenas para zonas Cloudflare"));
    }

    let setting = body.get("setting").and_then(|v| v.as_str())
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Campo 'setting' ausente"))?;
    let value = body.get("value")
        .ok_or_else(|| err(StatusCode::BAD_REQUEST, "Campo 'value' ausente"))?;
    ...
    if !ALLOWED.contains(&setting) {
        return Err(err(StatusCode::BAD_REQUEST, &format!(
            "Setting '{setting}' não permitido. Permitidos: {}", ALLOWED.join(", ")
        )));
    }
    ...
    match setting {
        "security_level" => {
            ...
            if !["off", "essentially_off", "low", "medium", "high", "under_attack"].contains(&v) {
                return Err(err(StatusCode::BAD_REQUEST, "Valor de security_level inválido"));
            }
        }
        "development_mode" => {
            if !value.is_string() || !["on", "off"].contains(&value.as_str().unwrap_or("")) {
                return Err(err(StatusCode::BAD_REQUEST, "development_mode deve ser 'on' ou 'off'"));
            }
        }
        "ssl" => {
            let v = value.as_str().unwrap_or("");
            if !["off", "flexible", "full", "strict"].contains(&v) {
                return Err(err(StatusCode::BAD_REQUEST, "Valor de ssl inválido. Use: off, flexible, full, strict"));
            }
        }
        "always_use_https" => {
            if !value.is_string() || !["on", "off"].contains(&value.as_str().unwrap_or("")) {
                return Err(err(StatusCode::BAD_REQUEST, "always_use_https deve ser 'on' ou 'off'"));
            }
        }
        "min_tls_version" => {
            let v = value.as_str().unwrap_or("");
            if !["1.0", "1.1", "1.2", "1.3"].contains(&v) {
                return Err(err(StatusCode::BAD_REQUEST, "min_tls_version inválido. Use: 1.0, 1.1, 1.2, 1.3"));
            }
        }
        _ => {}
    }
...
pub async fn cf_purge_cache(
    ...
    if zone.provider != "cloudflare" {
        return Err(err(StatusCode::BAD_REQUEST, "Limpeza de cache disponível apenas para zonas Cloudflare"));
    }
    ...
    let purge_body = if let Some(files) = body.get("files").and_then(|f| f.as_array()) {
        if files.is_empty() || files.len() > 30 {
            return Err(err(StatusCode::BAD_REQUEST, "Forneça de 1 a 30 URLs para limpar"));
        }
...
pub async fn configure_tunnel(
    ...
    if token.is_empty() {
        return Err(err(StatusCode::BAD_REQUEST, "Token de tunnel ausente"));
    }

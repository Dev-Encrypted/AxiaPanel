...
pub async fn update_config(
    ...
    if let Some(ref endpoint) = body.telemetry_endpoint {
        if endpoint.is_empty() {
            ...
            return Err(err(
                ...
                "O endpoint de telemetry deve usar HTTPS",
            ));
        }
        if endpoint.len() > 512 {
            return Err(err(
                StatusCode::BAD_REQUEST,
                "URL do endpoint muito longa (máximo 512 caracteres)",
            ));
        }
    }
    ...
    if let Some(ref enabled) = body.telemetry_enabled {
        if enabled != "true" && enabled != "false" {
            return Err(err(
                StatusCode::BAD_REQUEST,
                "telemetry_enabled deve ser 'true' ou 'false'",
            ));
        }
    }
...
pub async fn send_now(
    ...
    if endpoint.is_empty() {
        return Err(err(
            StatusCode::BAD_REQUEST,
            "Nenhum endpoint de telemetry configurado",
        ));
    }
    ...
    Ok(Json(
        serde_json::json!({ "success": true, "message": "Enviando eventos de telemetry em segundo plano" }),
    ))
}
...
pub async fn check_updates(
    ...
    Ok(Json(
        serde_json::json!({ "success": true, "message": "Verificação de atualização iniciada" }),
    ))
}

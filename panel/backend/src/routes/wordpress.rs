...
pub async fn info(
    ...
    let resp: serde_json::Value = agent
        ...
        .await
        .map_err(|e| { tracing::warn!("WordPress info failed for {domain}: {e}"); err(StatusCode::BAD_GATEWAY, "Serviço WordPress indisponível") })?;
...
pub async fn plugins(
    ...
    let resp: serde_json::Value = agent
        ...
        .await
        .map_err(|e| { tracing::warn!("WordPress plugins failed for {domain}: {e}"); err(StatusCode::BAD_GATEWAY, "Serviço WordPress indisponível") })?;
...
pub async fn themes(
    ...
    let resp: serde_json::Value = agent
        ...
        .await
        .map_err(|e| { tracing::warn!("WordPress themes failed for {domain}: {e}"); err(StatusCode::BAD_GATEWAY, "Serviço WordPress indisponível") })?;
...
pub async fn update(
    ...
    if !["core", "plugins", "themes"].contains(&target.as_str()) {
        return Err(err(StatusCode::BAD_REQUEST, "Alvo inválido"));
    }
...
pub async fn plugin_action(
    ...
    if !ALLOWED_PLUGIN_ACTIONS.contains(&action.as_str()) {
        return Err(err(StatusCode::BAD_REQUEST, "Ação de plugin inválida"));
    }
...
pub async fn theme_action(
    ...
    if !ALLOWED_THEME_ACTIONS.contains(&action.as_str()) {
        return Err(err(StatusCode::BAD_REQUEST, "Ação de tema inválida"));
    }
...
pub async fn bulk_update(
    ...
    if !["plugins", "themes", "core", "all"].contains(&body.target.as_str()) {
        return Err(err(
            ...
            "O alvo deve ser plugins, themes, core ou all",
        ));
    }

    if body.site_ids.len() > 50 {
        return Err(err(StatusCode::BAD_REQUEST, "Máximo de 50 sites por atualização em massa"));
    }
    ...
    for site_id in &body.site_ids {
        ...
        let domain = match domain {
            Some((d,)) => d,
            None => {
                results.push(serde_json::json!({"site_id": site_id, "ok": false, "message": "Site não encontrado"}));
                continue;
            }
        };

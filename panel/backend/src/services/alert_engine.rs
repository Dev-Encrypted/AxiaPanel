    ...
    for server in &servers {
        ...
        if let Some(cpu) = server.cpu_usage {
            check_threshold(
                ...
                cpu_dur,
                cooldown,
                &format!("CPU em {:.0}% em {}", cpu, server.name),
                &format!(
                    "O uso de CPU está acima de {}% há {} minutos no servidor {}",
        ...
        if let (Some(used), Some(total)) = (server.mem_used_mb, server.ram_mb) {
            if total > 0 {
                ...
                check_threshold(
                    ...
                    mem_dur,
                    cooldown,
                    &format!("Memória em {:.0}% em {}", pct, server.name),
                    &format!(
                        "O uso de memória está acima de {}% há {} minutos no servidor {}",
        ...
        if let Some(disk) = server.disk_usage_pct {
            check_threshold(
                ...
                1, // fire immediately
                cooldown,
                &format!("Disco em {:.0}% em {}", disk, server.name),
                &format!(
                    "O uso de disco está acima de {}% no servidor {}",
                ...
                if rate_per_hour > 0.0 {
                    ...
                    if hours_to_full < 48.0 && hours_to_full > 0.0 {
                        ...
                        fire_alert_with_retry(
                            ...
                            "disk_forecast",
                            severity,
                            &format!("Disco ficará cheio em {:.0} horas em {}", hours_to_full, server.name),
                            &format!(
                                "Na taxa atual de crescimento de {:.1}%/hora, o disco ficará cheio em aproximadamente {:.0} horas. Uso atual: {:.1}%",
        ...
        if mem_trend.len() >= 30 {
            ...
            if increase > 10.0 && recent_avg > 60.0 {
                fire_alert_with_retry(
                    ...
                    "memory_leak",
                    "warning",
                    &format!("Possível vazamento de memória detectado em {}", server.name),
                    &format!(
                        "O uso de memória aumentou {:.1}% na última hora (de {:.1}% para {:.1}%). \
                         Este aumento sustentado sugere um vazamento de memória.",
...
async fn fire_ssl_alert(
    ...
    let title = if days_left <= 0 {
        format!("Certificado SSL EXPIRADO para {domain}")
    } else {
        format!("Certificado SSL expira em {days_left} dias para {domain}")
    };

    let message = if days_left <= 0 {
        format!(
            "O certificado SSL para {domain} expirou. Visitantes verão avisos de segurança. Renove imediatamente."
        )
    } else {
        format!(
            "O certificado SSL para {domain} expirará em {days_left} dias. Renove-o antes que expire."
        )
    };
        ...
        if status == "stopped" || status == "failed" {
            ...
            if state.as_ref().map(|s| s.0.as_str()) != Some("firing") {
                ...
                fire_alert_with_retry(
                    ...
                    "service_down",
                    "critical",
                    &format!("Serviço {} está {} em {}", name, status, server_name),
                    &format!(
                        "O serviço {} está {} no servidor {}. Isto pode causar downtime nos sites.",
            ...
            if state.as_ref().map(|s| s.0.as_str()) == Some("firing") {
                ...
                notifications::resolve_alert(
                    ...
                    None,
                    "service_down",
                    &format!("Serviço {} recuperado em {}", name, server_name),
                    &format!("O serviço {} está rodando novamente no servidor {}.", name, server_name),
                )
                .await;
            }
        }
        ...
        if state == "exited" || state == "dead" {
            ...
            if existing.as_ref().map(|s| s.0.as_str()) != Some("firing") {
                ...
                fire_alert_with_retry(
                    ...
                    "container_down",
                    "critical",
                    &format!("Container '{}' está {}", name, state),
                    &format!(
                        "O container Docker '{}' parou (estado: {}). Pode precisar ser reiniciado.",
                        name, state
                    ),
                )
                .await;
            ...
            if existing.as_ref().map(|s| s.0.as_str()) != Some("firing") {
                ...
                fire_alert_with_retry(
                    ...
                    "container_crashloop",
                    "critical",
                    &format!("Container '{}' em crash-loop", name),
                    &format!(
                        "O container Docker '{}' está em loop de reinício (estado: restarting), indicando crashes repetidos.",
                        name
                    ),
                )
                .await;
            ...
            if existing.as_ref().map(|s| s.0.as_str()) != Some("firing") {
                ...
                fire_alert_with_retry(
                    ...
                    "container_unhealthy",
                    "warning",
                    &format!("Container '{}' está com falha de saúde", name),
                    &format!("O health check do container Docker '{}' está falhando.", name),
                )
                .await;
            }
            ...
            for alert_type in &["container_down", "container_unhealthy", "container_crashloop"] {
                ...
                if was_firing.as_ref().map(|s| s.0.as_str()) == Some("firing") {
                    ...
                    notifications::resolve_alert(
                        ...
                        None,
                        alert_type,
                        &format!("Container '{}' recuperado", name),
                        &format!("O container Docker '{}' está rodando e saudável novamente.", name),
                    )
                    .await;
                }
            }
...
async fn check_escalations(pool: &PgPool) {
    ...
    for (alert_id, user_id, title, message) in &escalated {
        let esc_subject = format!("[ESCALADO] {}", title);
        ...
        if let Some(channels) = notifications::get_user_channels(pool, *user_id, None).await {
            let html = format!(
                "<div style=\"font-family:sans-serif;max-width:600px;margin:0 auto\">\
                 <h2 style=\"color:#ef4444\">[ESCALADO] {}</h2>\
                 <p>{}</p>\
                 <p style=\"color:#ef4444;font-weight:bold\">Este alerta não foi reconhecido. Investigue imediatamente.</p>\
                 <p style=\"color:#6b7280;font-size:14px\">Quando: {}</p>\
                 </div>",
                title,
                message,

...
fn derive_key_with_salt(secret: &str, salt: &[u8]) -> [u8; 32] {
    ...
    let mut key = [0u8; 32];
    hk.expand(b"axiapanel-encryption", &mut key)
...
fn derive_key(jwt_secret: &str) -> [u8; 32] {
    derive_key_with_salt(jwt_secret, b"axiapanel-secrets-v1:")
}
...
fn derive_credential_key(jwt_secret: &str) -> [u8; 32] {
    if let Ok(key) = std::env::var("SECRETS_ENCRYPTION_KEY") {
        if !key.is_empty() {
            return derive_key_with_salt(&key, b"axiapanel-credential-encryption-v1:");
        }
    }
    derive_key_with_salt(jwt_secret, b"axiapanel-credential-v1:")
}
...
pub fn decrypt(encrypted_b64: &str, jwt_secret: &str) -> Result<String, String> {
    ...
    let mut legacy_key = derive_key_legacy(jwt_secret, b"axiapanel-secrets-v1:");
...
pub fn decrypt_credential(encrypted_b64: &str, jwt_secret: &str) -> Result<String, String> {
    ...
    let mut legacy_key = derive_key_legacy(jwt_secret, b"axiapanel-credential-v1:");
    let legacy_cipher = Aes256Gcm::new_from_slice(&legacy_key)
        .map_err(|e| { legacy_key.zeroize(); format!("Cipher init failed: {e}") })?;
    legacy_key.zeroize();
    ...
    if let Ok(env_key) = std::env::var("SECRETS_ENCRYPTION_KEY") {
        if !env_key.is_empty() {
            let mut env_legacy_key = derive_key_legacy(&env_key, b"axiapanel-credential-encryption-v1:");

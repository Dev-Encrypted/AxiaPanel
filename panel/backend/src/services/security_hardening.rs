...
pub async fn alert_suspicious_ip(
    ...
    let subject = format!(
        "🚨 Alerta de Segurança: {} a partir de novo IP{}",
        event, proxy_tag
    );
    let message = format!(
        "Usuário: {email}\nIP: {ip}\nPaís: {}\nCidade: {}\nISP: {}{}\nQuando: {}",
    ...
    let html = format!(
        "<h2>Alerta de Segurança: {event}</h2>\
...
pub async fn alert_lockdown(pool: &PgPool, reason: &str, triggered_by: &str) {
    ...
    let message = format!(
        "O DockPanel entrou em modo de lockdown.\n\
         ...
         É necessária ação do admin para desbloquear."
    );
    let html = format!(
        "<h2 style='color:red'>🔒 Lockdown do Sistema</h2>\
         <p><strong>Acionado por:</strong> {triggered_by}</p>\
         <p><strong>Motivo:</strong> {reason}</p>\
         <p>Todos os terminais desabilitados. Registro bloqueado. Ação do admin necessária.</p>"
    );

...
pub async fn validate_url_not_internal(url: &str) -> Result<(), String> {
    let url = url.trim();
    if url.is_empty() {
        return Err("URL é obrigatória".to_string());
    }
    if !url.starts_with("https://") && !url.starts_with("http://") {
        return Err("URL deve usar http ou https".to_string());
    }
    ...
    if host.is_empty() {
        return Err("URL não possui hostname".to_string());
    }

    // Block obvious internal hostnames
    if host == "localhost" || host == "127.0.0.1" || host == "::1" || host == "0.0.0.0" {
        return Err("URL aponta para um endereço local".to_string());
    }
    ...
    match tokio::net::lookup_host(&lookup_host).await {
        Ok(addrs) => {
            for addr in addrs {
                ...
                if ip.is_loopback() || ip.is_unspecified() {
                    return Err("URL resolve para endereço loopback".to_string());
                }
                match ip {
                    std::net::IpAddr::V4(v4) => {
                        if v4.is_private() || v4.is_link_local() || v4.octets()[0] == 169 {
                            return Err(
                                "URL resolve para endereço privado/link-local".to_string(),
                            );
                        }
                    }
                    std::net::IpAddr::V6(v6) => {
                        if v6.is_loopback() {
                            return Err(
                                "URL resolve para endereço loopback".to_string(),
                            );
                        }
                    }
                }
        ...
        Err(_) => {
            return Err("Hostname da URL não pôde ser resolvido".to_string());
        }
    }

...
impl FromRequestParts<AppState> for AuthUser {
    ...
    async fn from_request_parts(
        ...
        let token = bearer_token.clone().or_else(|| {
            ...
            .ok_or_else(|| err(StatusCode::UNAUTHORIZED, "Autenticação obrigatória"))?;
        ...
        if bearer_token.is_none() {
            ...
            if method == "POST" || method == "PUT" || method == "DELETE" || method == "PATCH" {
                ...
                if !has_custom_header {
                    return Err(err(StatusCode::FORBIDDEN, "Cabeçalho CSRF ausente"));
                }
            }
        }
        ...
        if let Some(ref jti) = claims.jti {
            ...
            if blacklist.contains(jti) {
                return Err(err(StatusCode::UNAUTHORIZED, "Token foi revogado"));
            }
        }
        ...
        // Check global session revocation (revoke_all_sessions)
        {
            ...
            if let Some(ts) = *revoked_at {
                if (claims.iat as i64) < ts {
                    return Err(err(StatusCode::UNAUTHORIZED, "Sessão revogada. Faça login novamente."));
                }
            }
        }
...
impl FromRequestParts<AppState> for AdminUser {
    ...
    async fn from_request_parts(
        ...
        if claims.role != "admin" {
            return Err(err(StatusCode::FORBIDDEN, "Acesso de administrador obrigatório"));
        }
...
impl FromRequestParts<AppState> for ResellerUser {
    ...
    async fn from_request_parts(
        ...
        if claims.role != "admin" && claims.role != "reseller" {
            return Err(err(StatusCode::FORBIDDEN, "Acesso de administrador ou revendedor obrigatório"));
        }
        ...
        match server_id {
            Some(sid) => {
                ...
                let uid = user_id.ok_or_else(|| {
                    err(StatusCode::UNAUTHORIZED, "Autenticação obrigatória quando X-Server-Id é fornecido")
                })?;
                ...
                .await
                .map_err(|_| err(StatusCode::INTERNAL_SERVER_ERROR, "Falha na busca de servidor"))?;

                if owns.is_none() {
                    return Err(err(StatusCode::FORBIDDEN, "Servidor não encontrado ou acesso negado"));
                }
                ...
                let local_id = state
                    ...
                    .ok_or_else(|| {
                        err(
                            ...
                            "Servidor local ainda não registrado",
                        )
                    })?;

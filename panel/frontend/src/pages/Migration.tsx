...
export default function Migration() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : "Falha na análise");
    } finally {
      setAnalyzing(false);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-dark-50 font-mono">Assistente de Migração</h1>
        <p className="text-sm text-dark-300 mt-1">Importe sites, bancos de dados e e-mails do cPanel, Plesk ou HestiaCP</p>
      </div>
      ...
      <div className="flex items-center gap-2 text-xs font-mono">
        {[1, 2, 3, 4].map((s) => (
          <div key={s} className={`flex items-center gap-1 ${step >= s ? "text-rust-400" : "text-dark-400"}`}>
            ...
            <span className="hidden sm:inline">{["Origem", "Revisão", "Importar", "Concluído"][s - 1]}</span>
      ...
      {step === 1 && (
        <div className="bg-dark-800 border border-dark-600 rounded-lg p-6 space-y-5">
          <h2 className="text-lg font-bold text-dark-50 font-mono">Selecione a Origem do Backup</h2>

          <div className="flex gap-3">
            {[
              { id: "cpanel", label: "cPanel", desc: "Backup completo (.tar.gz)" },
              { id: "plesk", label: "Plesk", desc: "Backup de domínio" },
              { id: "hestiacp", label: "HestiaCP", desc: "Backup de usuário (.tar)" },
            ].map((s) => (
              <button
                key={s.id}
          ...
          <div>
            <label className="block text-sm text-dark-200 mb-1">Caminho do Arquivo de Backup</label>
            <input
              value={backupPath}
            ...
            />
            <p className="text-xs text-dark-400 mt-1">Envie o backup para seu servidor via SFTP primeiro e, em seguida, informe o caminho completo aqui.</p>
          </div>
          ...
          >
            {analyzing ? "Analisando..." : "Analisar Backup"}
          </button>
        </div>
      )}
      ...
      {step === 2 && inv && (
        <div className="space-y-4">
          <div className="bg-dark-800 border border-dark-600 rounded-lg p-5">
            <h2 className="text-lg font-bold text-dark-50 font-mono mb-1">Resultados da Análise</h2>
            <p className="text-sm text-dark-300">
              Encontrado(s) {inv.sites.length} site{inv.sites.length !== 1 ? "s" : ""}, {inv.databases.length} banco{inv.databases.length !== 1 ? "s" : ""} de dados, {inv.mail_accounts.length} conta{inv.mail_accounts.length !== 1 ? "s" : ""} de e-mail
            </p>
          </div>
                ...
                {inv.sites.map((s) => (
                  <label key={s.domain} className="flex items-center gap-3 p-2 rounded hover:bg-dark-700/30 cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <span className="text-xs text-dark-400 ml-2">{s.runtime} &middot; {fmtSize(s.size_bytes)} &middot; {s.file_count} arquivos</span>
                    </div>
                  </label>
                ))}
              </div>
          ...
          {inv.databases.length > 0 && (
            <div className="bg-dark-800 border border-dark-600 rounded-lg p-5">
              <h3 className="text-sm font-bold text-dark-50 font-mono uppercase tracking-wider mb-3">Bancos de Dados ({selectedDbs.size}/{inv.databases.length})</h3>
              <div className="space-y-2">
                {inv.databases.map((d) => (
          ...
          {inv.mail_accounts.length > 0 && (
            <div className="bg-dark-800 border border-dark-600 rounded-lg p-5">
              <h3 className="text-sm font-bold text-dark-50 font-mono uppercase tracking-wider mb-3">Contas de E-mail ({inv.mail_accounts.length})</h3>
              <p className="text-xs text-dark-400 mb-2">As contas de e-mail precisarão ser recriadas manualmente na seção E-mail.</p>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-rust-500 text-dark-950 rounded-lg text-sm font-bold hover:bg-rust-400 transition-colors disabled:opacity-50"
            >
              Importar {selectedSites.size + selectedDbs.size} ite{selectedSites.size + selectedDbs.size !== 1 ? "ns" : "m"}
            </button>
          </div>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="bg-dark-800 border border-dark-600 rounded-lg p-5 space-y-3">
          <h2 className="text-lg font-bold text-dark-50 font-mono">Importando...</h2>
          <div className="space-y-2">
            {progress.map((p) => (
            ...
            {importing && progress.length === 0 && (
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-dark-300">Iniciando importação...</span>
              </div>
            )}
          </div>
        </div>
      ...
      {step === 4 && (
        <div className="space-y-4">
          <div className="bg-dark-800 border border-dark-600 rounded-lg p-5">
            <h2 className="text-lg font-bold text-dark-50 font-mono mb-3">Migração Concluída</h2>
            <div className="space-y-2">
              {progress.map((p) => (
                <div key={p.step} className="flex items-center gap-3">
                  {p.status === "done" ? (

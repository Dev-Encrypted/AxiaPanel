...
export default function AuditLogContent() {
  ...
  const loadEntries = async (offset: number, append: boolean) => {
    try {
      ...
      setError(e instanceof Error ? e.message : "Falha ao carregar atividade");
    } finally {
      setLoading(false);
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-x-auto">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-dark-200 font-medium">Nenhuma atividade encontrada</p>
            <p className="text-dark-300 text-sm mt-1">Ações de admin aparecerão aqui</p>
          </div>
        ) : (
          <>
            {/* Desktop Table */}
            <table className="w-full hidden sm:table">
              <thead>
                <tr className="bg-dark-900 border-b border-dark-500">
                  ...
                  <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3">Usuário</th>
                  <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3 w-36">Ação</th>
                  <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3 hidden md:table-cell">Alvo</th>
                  <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3 hidden lg:table-cell">Detalhes</th>
                </tr>
              </thead>

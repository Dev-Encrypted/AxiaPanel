...
export default function Alerts() {
  ...
  const handleAcknowledge = async (id: string) => {
    try {
      ...
      setMessage({ text: "Alerta reconhecido", type: "success" });
      setTimeout(() => setMessage(null), 3000);
      fetchAlerts();
    } catch (e) {
      logger.error("Failed to acknowledge alert:", e);
      setMessage({ text: "Falha ao reconhecer alerta", type: "error" });
      setTimeout(() => setMessage(null), 3000);
    }
  };
  ...
  const handleResolve = async (id: string) => {
    try {
      ...
      setMessage({ text: "Alerta resolvido", type: "success" });
      setTimeout(() => setMessage(null), 3000);
      fetchAlerts();
    } catch (e) {
      logger.error("Failed to resolve alert:", e);
      setMessage({ text: "Falha ao resolver alerta", type: "error" });
      setTimeout(() => setMessage(null), 3000);
    }
  };
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-header-title">Alertas</h1>
          <p className="page-header-subtitle">Monitore e gerencie alertas do sistema</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        >
          ...
          <div className="text-sm text-dark-200">Ativos</div>
        </div>
        <div
          className={`p-4 rounded-lg border cursor-pointer transition-colors ${
        ...
        >
          ...
          <div className="text-sm text-dark-200">Reconhecidos</div>
        </div>
        <div
          className={`p-4 rounded-lg border cursor-pointer transition-colors ${
        ...
        >
          ...
          <div className="text-sm text-dark-200">Resolvidos (30d)</div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-dark-200 text-sm">
            {statusFilter === "firing"
              ? "Nenhum alerta ativo -- todos os sistemas operacionais"
                  ...
                  <div className="flex items-start gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-dark-300 font-mono">
                        ...
                        {alert.resolved_at && ` -- resolvido ${ago(alert.resolved_at)}`}
                        {alert.acknowledged_at && !alert.resolved_at && ` -- reconhecido ${ago(alert.acknowledged_at)}`}
                      </p>
                    </div>
                  </div>
                  {alert.status === "firing" && (

...
const snippets = [
  { label: "Reiniciar Nginx", cmd: "systemctl restart nginx" },
  { label: "Reiniciar PHP-FPM", cmd: "systemctl restart php8.3-fpm" },
  { label: "Uso de Disco", cmd: "df -h" },
  { label: "Uso de Memória", cmd: "free -h" },
  { label: "Top Processos", cmd: "top -bn1 | head -20" },
  { label: "Containers Docker", cmd: "docker ps" },
  { label: "Testar Nginx", cmd: "nginx -t" },
...
export default function Terminal() {
  ...
  // Load sites
  useEffect(() => {
    api.get<Site[]>("/sites").then(setSites).catch(() => setError("Falha ao carregar sites"));
  }, []);
  ...
  const resetIdleTimer = useCallback(() => {
    ...
    idleTimer.current = setTimeout(() => {
      if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
        ...
        wsRef.current.close();
        setStatus("Desconectado (timeout de inatividade — 30 min)");
        setConnected(false);
      }
    }, IDLE_TIMEOUT);
  }, []);
  ...
  const connect = useCallback(
    async (siteIdParam?: string) => {
      ...
      term.writeln("\x1b[34m● Conectando ao servidor...\x1b[0m");
      ...
      try {
        ...
        ws.onclose = () => {
          ...
          if (!intentionalClose.current && reconnectAttempts.current < MAX_RECONNECT_ATTEMPTS) {
            ...
            const msg = `Conexão perdida. Reconectando em ${delay / 1000}s... (tentativa ${reconnectAttempts.current}/${MAX_RECONNECT_ATTEMPTS})`;
            setError(msg);
            term.writeln(`\r\n\x1b[33m● ${msg}\x1b[0m`);
            ...
            setError("Conexão perdida. Clique em Reconectar para tentar novamente.");
            term.writeln("\r\n\x1b[31m● Conexão perdida após 3 tentativas. Clique em Reconectar para tentar novamente.\x1b[0m");
          } else {
            if (!statusRef.current) {
              term.writeln("\r\n\x1b[31m● Conexão fechada\x1b[0m");
            }
          }
        };
        ...
        term.writeln(
          `\r\n\x1b[31m● Erro: ${e instanceof Error ? e.message : "Falha na conexão"}\x1b[0m`
        );
      }
    },
  ...
  const headerLabel = selectedSite
    ? sites.find((s) => s.id === selectedSite)?.domain || "Terminal do Site"
      ...
      <div className="flex flex-col flex-1 border border-dark-500 min-h-0 overflow-hidden">
        ...
        <div className="px-3 sm:px-5 py-2 sm:py-3 border-b border-dark-500 bg-dark-800 shrink-0">
          ...
          <div className={`mt-2 ${showMoreTools ? "" : "hidden md:block"}`}>
                      ...
                      try {
                        ...
                        if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
                          ...
                          wsRef.current.send(
                            JSON.stringify({
                              type: "input",
                              data: `echo "Enviado: ${safeName}"\n`,
                            })
                          );
                        }
                        setStatus(`Enviado: ${file.name}`);
                        setTimeout(() => setStatus(""), 3000);
                      } catch {
                        setError(`Falha no envio: ${file.name}`);
                      }
                    };
            ...
            <button
              onClick={() => {
                if (xtermRef.current) {
                  ...
                  navigator.clipboard
                    ...
                    .then(() => {
                      setError("");
                      setStatus("Saída do terminal copiada para a área de transferência");
                      setTimeout(() => setStatus(""), 2000);
                    })
                    .catch(() => {
              ...
              title="Copiar toda a saída do terminal"
            >
              Copiar Saída
            </button>
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  navigator.clipboard
                    ...
                    .then(() => {
                      setError("");
                      setStatus("Link de compartilhamento copiado! Expira em 1 hora");
                      setTimeout(() => setStatus(""), 3000);
                    })
                    .catch(() => {
                      setError("Falha ao copiar o link de compartilhamento");
                    });
                } catch {
                  setError("Falha ao criar link de compartilhamento");
                }
              }}
              className="py-1.5 bg-dark-700 text-dark-200 rounded text-xs font-mono hover:bg-dark-600 active:bg-dark-500 touch-manipulation text-center"
              title="Compartilhar saída do terminal (link de 1h)"
            >
              Compartilhar
            </button>
            ...
            <button
              ...
              title={recording ? "Parar gravação e baixar arquivo .cast" : "Gravar sessão de terminal (formato asciinema)"}
            >
              {recording ? "Parar Grav" : "Gravar"}
            </button>
          ...
          <div className="flex flex-wrap gap-1.5 px-3 py-2 bg-dark-900 border-b border-dark-600 shrink-0">
            {snippets.map((s) => (
              <button
                ...
                title={`${s.cmd} (cola no terminal — pressione Enter para executar)`}
              >
                {s.label}
              </button>
            ))}
        ...
        {showSshInfo && (
          <div className="px-3 py-2 bg-dark-900 border-b border-dark-600 space-y-1.5 shrink-0">
            ...
            <div className="flex items-center gap-2 text-xs">
              <span className="text-dark-300 w-16">Porta:</span>
              <code className="text-dark-100 font-mono bg-dark-700 px-2 py-0.5 rounded">
                22
              </code>
            </div>
            <div className="flex items-center gap-2 text-xs">
              <span className="text-dark-300 w-16">Usuário:</span>
              <code className="text-dark-100 font-mono bg-dark-700 px-2 py-0.5 rounded">
                root
              </code>
            </div>
            <div className="flex items-center gap-2 text-xs">
              <span className="text-dark-300 w-16">Comando:</span>
              <code className="text-dark-100 font-mono bg-dark-700 px-2 py-0.5 rounded">
            ...
            {selectedSite && (
              <div className="flex items-center gap-2 text-xs">
                <span className="text-dark-300 w-16">Diretório:</span>
                <code className="text-dark-100 font-mono bg-dark-700 px-2 py-0.5 rounded">
                  /var/www/
        ...
        <div className="bg-dark-800 border-t border-dark-500 shrink-0 md:hidden px-1.5 py-1.5 space-y-1">
          ...
          <div className="grid grid-cols-7 gap-1">
            {[
              ...
              { label: "↓", key: "\x1b[B" },
              { label: "→", key: "\x1b[C" },
              { label: "Colar", key: "__paste__" },
            ].map((btn) => (
              <button key={btn.label} onClick={async () => {
                if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) return;
                if (btn.key === "__paste__") {
                  try { const text = await navigator.clipboard.readText(); wsRef.current.send(JSON.stringify({ type: "input", data: text })); } catch { setError("Acesso à área de transferência negado"); }

  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-dark-50">Notificações</h1>
          <p className="text-sm text-dark-400 mt-1">
            {unreadCount > 0
              ? `${unreadCount} ${unreadCount !== 1 ? "notificações não lidas" : "notificação não lida"}`
              : "Tudo em dia"}
          </p>
          <Link to="/settings" className="text-xs text-accent-400 hover:text-accent-300 mt-1 inline-block">
            Configurar canais de alerta &rarr;
          </Link>
        </div>
        {unreadCount > 0 && (
          <button
      ...
      {notifs.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-dark-400 text-sm">Nenhuma notificação ainda</p>
          <p className="text-dark-500 text-xs mt-1">
            Alertas e eventos do sistema aparecerão aqui
          </p>
        </div>
      ) : (

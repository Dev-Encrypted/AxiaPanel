...
export default function SecretsManager() {
  ...
  const createVault = async () => {
    try {
      ...
      setMessage({ text: "Vault criado", type: "success" });
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha", type: "error" });
    }
  };
  ...
  const updateVault = async () => {
    ...
    try {
      ...
      setMessage({ text: "Vault atualizado", type: "success" });
      await loadVaults();
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha", type: "error" });
    }
  ...
  const createSecret = async () => {
    ...
    try {
      ...
      setMessage({ text: "Segredo criado (criptografado)", type: "success" });
      loadSecrets();
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha", type: "error" });
    }
  ...
  const updateSecret = async () => {
    ...
    try {
      ...
      setMessage({ text: "Segredo atualizado", type: "success" });
      loadSecrets();
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha", type: "error" });
    }
  ...
  if (loading) return <div className="p-8 text-center text-dark-300 font-mono">Carregando segredos...</div>;

  return (
    <div className="p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-dark-600">
        <div>
          <h1 className="text-sm font-medium text-dark-300 uppercase font-mono tracking-widest">Gerenciador de Segredos</h1>
          <p className="text-sm text-dark-200 mt-1 font-mono">Segredos criptografados com AES-256-GCM e histórico de versões</p>
        </div>
        <button onClick={() => setShowVaultForm(!showVaultForm)}
          ...
          {showVaultForm ? "Cancelar" : "Novo Vault"}
        </button>
      </div>
      ...
      {showVaultForm && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-5 mb-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Nome do Vault</label>
          ...
          <div className="flex justify-end">
            <button onClick={createVault} className="px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium font-mono hover:bg-rust-600">Criar Vault</button>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-6">
        ...
        <div className="w-56 space-y-1 shrink-0">
              ...
              {editingVault === v.id ? (
                <div className="px-3 py-2 rounded-lg bg-dark-700 border border-accent-500/30 space-y-2">
                  ...
                  <div className="flex justify-end gap-1">
                    <button onClick={() => setEditingVault(null)} className="px-2 py-1 text-dark-200 text-xs font-mono">Cancelar</button>
                    <button onClick={updateVault} className="px-2 py-1 bg-accent-500 text-white rounded text-xs font-medium font-mono">Salvar</button>
                  </div>
                </div>
              ) : (
                <div
          ...
          {vaults.length === 0 && <p className="text-xs text-dark-300 font-mono px-3 py-2">Nenhum vault ainda</p>}
        </div>
        ...
        <div className="flex-1 space-y-4">
            ...
            <>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <button onClick={() => setShowSecretForm(!showSecretForm)}
                    ...
                    {showSecretForm ? "Cancelar" : "Adicionar Segredo"}
                  </button>
              ...
              {showSecretForm && (
                <div className="bg-dark-800 rounded-lg border border-dark-500 p-4 space-y-3">
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Chave</label>
                      <input type="text" value={secretForm.key} onChange={e => setSecretForm({ ...secretForm, key: e.target.value })}
                        placeholder="API_KEY"
                        className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Valor</label>
                      <input type="password" value={secretForm.value} onChange={e => setSecretForm({ ...secretForm, value: e.target.value })}
                        className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Tipo</label>
                  ...
                  <div className="flex items-center gap-4">
                    <label className="flex items-center gap-1.5 text-xs font-mono text-dark-100">
                      ...
                      Auto-injetar no .env no deploy
                    </label>
                    <button onClick={createSecret} className="ml-auto px-3 py-1.5 bg-rust-500 text-white rounded-lg text-xs font-medium font-mono hover:bg-rust-600">Salvar</button>
                  </div>
                </div>
              )}
              ...
              {editingSecret && (
                <div className="bg-dark-800 rounded-lg border border-accent-500/30 p-4 space-y-3">
                  <h3 className="text-xs font-medium text-accent-400 uppercase font-mono tracking-widest">Atualizar Segredo</h3>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Novo Valor (deixe vazio para manter o atual)</label>
                  ...
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setEditingSecret(null)} className="px-3 py-1.5 text-dark-200 text-xs font-mono">Cancelar</button>
                    <button onClick={updateSecret} className="px-3 py-1.5 bg-accent-500 text-white rounded-lg text-xs font-medium font-mono">Atualizar</button>
                  </div>
                </div>
              )}
              ...
              {viewingVersions && (
                <div className="bg-dark-800 rounded-lg border border-dark-500 p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Histórico de Versões</h3>
                    <button onClick={() => setViewingVersions(null)} className="text-dark-300 hover:text-dark-100 text-xs font-mono">Fechar</button>
                  </div>
                  {versions.length === 0 ? (
                    <p className="text-xs text-dark-300 font-mono">Sem histórico de versões</p>
                  ) : (
              ...
              {secrets.length === 0 ? (
                <div className="p-12 text-center">
                  <p className="text-dark-200 text-sm font-mono">Nenhum segredo neste vault</p>
                  <p className="text-dark-300 text-xs mt-1 font-mono">Adicione seu primeiro segredo acima</p>
                </div>
              ) : (
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-dark-900 border-b border-dark-500">
                        {["Chave", "Valor", "Tipo", "Injetar", "Ver", "Atualizado em", ""].map(h => (
                    ...
                    <tbody className="divide-y divide-dark-600">
                      {secrets.map(s => (
                        <tr key={s.id} className="hover:bg-dark-700/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-xs text-dark-300 font-mono">{s.auto_inject ? "Sim" : "-"}</td>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-1">
                              <button onClick={() => { setEditingSecret(s.id); setEditForm({ value: "", description: s.description || "" }); }}
                                className="px-2 py-1 bg-accent-500/10 text-accent-400 rounded text-xs font-mono hover:bg-accent-500/20">Editar</button>
                              <button onClick={() => viewVersionHistory(s.id)}
                                className="px-2 py-1 bg-dark-700 text-dark-200 rounded text-xs font-mono hover:bg-dark-600">Histórico</button>
                              <button onClick={() => deleteSecret(s.id)}
                                className="px-2 py-1 bg-danger-500/10 text-danger-400 rounded text-xs font-mono hover:bg-danger-500/20">Excl</button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          {!selectedVault && (
            <div className="p-12 text-center">
              <p className="text-dark-200 text-sm font-mono">Selecione ou crie um vault para gerenciar segredos</p>
            </div>
          )}
        </div>
      </div>

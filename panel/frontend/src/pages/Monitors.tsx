...
export default function Monitors() {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      setSuccess("Monitor criado");
      fetchMonitors();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao criar monitor");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleToggle = async (id: string, enabled: boolean) => {
    try {
      ...
      setSuccess(!enabled ? "Monitor habilitado" : "Monitor pausado");
      fetchMonitors();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao alternar");
    }
  };
  ...
  const handleForceCheck = async (id: string) => {
    try {
      ...
      setCheckNowMsg(res.message || "Verificação agendada");
  ...
  const getUrlLabel = () => {
    switch (formMonitorType) {
      ...
      case "heartbeat": return "Identificador";
      default: return "URL";
    }
  };
  ...
  const getUrlPlaceholder = () => {
    switch (formMonitorType) {
      ...
      case "ping": return "192.168.1.1 ou example.com";
      case "heartbeat": return "meu-cron-job";
  ...
  const getUrlHint = () => {
    switch (formMonitorType) {
      case "tcp": return "Hostname ou endereço IP";
      case "ping": return "Hostname ou IP para fazer ping via ICMP";
      case "heartbeat": return "Nome/identificador deste endpoint de heartbeat";
      default: return "A URL completa para monitorar, incluindo https://";
    }
  };
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-header-title">Monitoramento</h1>
          <p className="page-header-subtitle">
            {monitors.length > 0 ? (
              <>{upCount} no ar, {downCount} fora, {monitors.length - upCount - downCount} pendentes</>
            ) : (
              "Monitore seus sites e serviços"
            )}
          </p>
        </div>
      ...
      {success && (
        <div className="bg-rust-500/10 text-rust-400 text-sm px-4 py-3 rounded-lg border border-rust-500/20 mb-4">
          {success}
          <button onClick={() => setSuccess("")} className="ml-2 font-medium hover:underline">Dispensar</button>
        </div>
      )}
      {error && (
        <div className="bg-danger-500/10 text-danger-400 text-sm px-4 py-3 rounded-lg border border-danger-500/20 mb-4">
          {error}
          <button onClick={() => setError("")} className="ml-2 font-medium hover:underline">Dispensar</button>
        </div>
      )}
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-dark-800 rounded-lg border border-dark-500 p-5 mb-6">
          <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest mb-3">Novo Monitor</h3>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-xs font-medium text-dark-200 mb-1">Tipo de Monitor</label>
            ...
            <div>
              ...
              <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} required placeholder="Meu Site" className="w-full px-3 py-2 border border-dark-500 rounded-lg text-sm focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none" />
            </div>
            ...
            {formMonitorType === "tcp" && (
              <div>
                <label className="block text-xs font-medium text-dark-200 mb-1">Porta</label>
                ...
                <p className="text-xs text-dark-300 mt-1">Porta TCP a verificar (ex: 3306 para MySQL, 5432 para PostgreSQL)</p>
              </div>
            )}
            <div>
              <label className="block text-xs font-medium text-dark-200 mb-1">
                {formMonitorType === "heartbeat" ? "Intervalo Esperado" : "Intervalo de Verificação"}
              </label>
              ...
              <p className="text-xs text-dark-300 mt-1">
                {formMonitorType === "heartbeat"
                  ? "Frequência com que o serviço externo deve pingar este endpoint"
          ...
          {formMonitorType === "heartbeat" && (
            <div className="mb-4 p-3 bg-dark-700/50 rounded-lg border border-dark-600">
              <p className="text-xs text-dark-200">
                Após criar este monitor, você receberá uma URL de heartbeat. Configure seu cron ou serviço para fazer POST nessa URL a cada execução bem-sucedida.
                Se nenhum ping for recebido em 2x o intervalo esperado, um alerta será disparado.
              </p>
            </div>
          )}
          ...
          {formMonitorType === "http" && (
            <div className="mb-4 space-y-2">
              <div>
                <label className="block text-xs font-medium text-dark-200 mb-1">Verificar Palavra-chave (opcional)</label>
                <input type="text" value={formKeyword} onChange={(e) => setFormKeyword(e.target.value)} placeholder="Palavra-chave a verificar no corpo da resposta" className="w-full px-3 py-2 border border-dark-500 rounded-lg text-sm focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none" />
              </div>
              {formKeyword && (
                <label className="flex items-center gap-2 text-xs text-dark-200">
                  ...
                  A resposta DEVE conter esta palavra-chave (desmarque = NÃO deve conter)
                </label>
              )}
            </div>
          )}
          ...
          {formMonitorType === "http" && (
            <div className="mb-4">
              ...
              >
                {showHeaders ? "- Ocultar Cabeçalhos Personalizados" : "+ Cabeçalhos Personalizados (opcional)"}
              </button>
              {showHeaders && (
                <div className="mt-2 space-y-1">
                  ...
                  <p className="text-[10px] text-dark-300 mt-1">Envia cabeçalhos personalizados em cada verificação HTTP (ex: Authorization, X-API-Key)</p>
                </div>
              )}
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              {formSlackUrl && formSlackUrl === globalSlackUrl && <p className="text-[10px] text-dark-300 mt-0.5">Herdado das configurações globais</p>}
            </div>
            <div>
              ...
              {formDiscordUrl && formDiscordUrl === globalDiscordUrl && <p className="text-[10px] text-dark-300 mt-0.5">Herdado das configurações globais</p>}
            </div>
          </div>
          <div className="flex gap-3">
            <button type="submit" disabled={submitting} className="flex items-center gap-2 px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium hover:bg-rust-600 disabled:opacity-50">
              ...
              {submitting ? "Adicionando..." : "Criar Monitor"}
            </button>
      ...
      {!showForm && monitors.length === 0 ? (
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-12 text-center">
          ...
          <p className="text-dark-200 font-medium">Nenhum monitor ainda</p>
          <p className="text-dark-300 text-sm mt-2 max-w-md mx-auto">Monitore endpoints HTTP, portas TCP e alvos de ping com intervalos customizáveis, alertas via Slack, Discord e email, e uma página de status pública.</p>
        ...
        <div className="space-y-3">
              ...
              <div className="p-4 cursor-pointer" onClick={() => toggleExpand(m.id)}>
                <div className="flex items-start sm:items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                    ...
                    <button
                      ...
                      aria-label={m.enabled ? "Pausar monitor" : "Reativar monitor"}
                    ...
                    {deleteTarget === m.id ? (
                      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => handleDelete(m.id)} className="px-2 py-1 bg-danger-600 text-white rounded-md text-xs">Confirmar</button>
                        <button onClick={() => setDeleteTarget(null)} className="px-2 py-1 bg-dark-600 text-dark-200 rounded-md text-xs">Cancelar</button>
                      </div>
                    ) : (
                      <button
                ...
                {m.last_checked_at && (
                  <p className="text-xs text-dark-300 mt-1 ml-5.5">
                    Verificado em {formatDate(m.last_checked_at)} · a cada {m.check_interval}s
                  </p>
                )}
              </div>
              ...
              {expanded === m.id && (
                <div className="border-t border-dark-600 p-4">
                  ...
                  {m.monitor_type === "heartbeat" && (
                    <div className="mb-4 p-3 bg-dark-700/50 rounded-lg border border-dark-600">
                      <p className="text-xs font-medium text-dark-100 mb-1">URL do Heartbeat</p>
                      ...
                      <p className="text-[10px] text-dark-300 mt-1">
                        Faça POST nesta URL pelo seu cron/serviço. Se nenhum ping em {m.check_interval * 2}s, um alerta é disparado.
                      </p>
                    </div>
                  )}
                      ...
                      {uptimeData && (
                        <>
                          ...
                          <div className="text-center">
                            ...
                            <p className="text-[10px] text-dark-300">média</p>
                          </div>
                        </>
                      )}
                    </div>
                  ...
                  {chartData.length >= 2 && (
                    <div className="mb-4">
                      <h4 className="text-xs font-semibold text-dark-100 mb-1">Tempo de Resposta (24h)</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {/* Recent checks */}
                    <div>
                      <h4 className="text-xs font-semibold text-dark-100 mb-2">Verificações Recentes</h4>
                      {checks.length === 0 ? (
                        <p className="text-xs text-dark-300">Nenhuma verificação ainda</p>
                      ) : (
                    ...
                    {/* Incidents */}
                    <div>
                      <h4 className="text-xs font-semibold text-dark-100 mb-2">Incidentes</h4>
                      {incidents.length === 0 ? (
                        <p className="text-xs text-dark-300">Nenhum incidente registrado</p>
                      ) : (
                            ...
                            <div key={i.id} className="text-xs border border-dark-600 rounded-lg p-2">
                              <div className="flex items-center justify-between">
                                <span className={`font-medium ${i.resolved_at ? "text-rust-400" : "text-danger-400"}`}>
                                  {i.resolved_at ? "Resolvido" : "Em andamento"}
                                </span>
                              ...
                              {i.resolved_at && (
                                <p className="text-dark-300 mt-0.5">
                                  Duração: {Math.round((new Date(i.resolved_at).getTime() - new Date(i.started_at).getTime()) / 1000)}s
                                </p>
                              )}
                            </div>
                          ))}
          ...
          {monitors.length > displayCount && (
            ...
            >
              Mostrar mais ({monitors.length - displayCount} restantes)
            </button>
          )}
        </div>
      ) : null}

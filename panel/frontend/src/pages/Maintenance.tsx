...
export default function Maintenance() {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      setFormStartsAt("");
      setFormEndsAt("");
      setMessage({ text: "Janela de manutenção criada", type: "success" });
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setMessage({ text: "Janela de manutenção excluída", type: "success" });
      setTimeout(() => setMessage(null), 3000);
      fetchWindows();
    } catch (err) {
  ...
  const handleSilence = async () => {
    try {
      ...
      await api.post("/monitors/maintenance", {
        name: "Silenciamento de alertas (30 min)",
        starts_at: now.toISOString(),
        ends_at: end.toISOString(),
      });
      setSilenced(true);
      setMessage({ text: "Alertas silenciados por 30 minutos", type: "success" });
      setTimeout(() => setMessage(null), 5000);
      fetchWindows();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao silenciar");
    }
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-dark-200 font-mono">
          {windows.length > 0
            ? `${windows.length} janela${windows.length > 1 ? "s" : ""} de manutenção`
            : "Agende manutenção para suprimir alertas"}
        </p>
        <div className="flex gap-2">
          ...
          >
            {silenced ? "Silenciado (30m)" : "Silenciar Alertas"}
          </button>
          <button
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-dark-800 rounded-lg border border-dark-500 p-5 mb-6">
          <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest mb-3">Nova Janela de Manutenção</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div>
              <label className="block text-xs font-medium text-dark-200 mb-1">Nome</label>
              <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)} placeholder="Atualização do servidor" className="w-full px-3 py-2 border border-dark-500 rounded-lg text-sm focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-medium text-dark-200 mb-1">Início</label>
            ...
            <div>
              <label className="block text-xs font-medium text-dark-200 mb-1">Fim</label>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={submitting} className="flex items-center gap-2 px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium hover:bg-rust-600 disabled:opacity-50">
              ...
              {submitting ? "Criando..." : "Criar Janela"}
            </button>
      ...
      {windows.length === 0 && !showForm ? (
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-12 text-center">
          ...
          <p className="text-dark-200 text-sm">Nenhuma janela de manutenção agendada.</p>
          <p className="text-dark-300 text-xs mt-1">Os alertas são suprimidos durante janelas de manutenção.</p>
        </div>
      ) : (
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {!w.active && new Date(w.ends_at) < new Date() && (
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-dark-700 text-dark-300">Encerrada</span>
                    )}
                    {!w.active && new Date(w.starts_at) > new Date() && (
                      <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-accent-500/15 text-accent-400">Agendada</span>
                    )}
                  </div>

...
const LOG_TYPES = [
  { value: "nginx_access", label: "Nginx Acesso" },
  { value: "nginx_error", label: "Nginx Erro" },
  { value: "syslog", label: "Log do Sistema" },
  { value: "auth", label: "Log de Autenticação" },
...
const SITE_LOG_TYPES = [
  { value: "access", label: "Log de Acesso" },
  { value: "error", label: "Log de Erros" },
];
...
function SiteLogsContent() {
  ...
  useEffect(() => {
    api.get<Site[]>("/sites").then(setSites).catch(() => setError("Falha ao carregar sites. Tente novamente."));
  }, []);
  ...
  const fetchDockerLogs = async (container?: string) => {
    ...
    try {
      ...
      setLines([`Erro: ${e instanceof Error ? e.message : "Falha ao carregar logs Docker"}`]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchServiceLogs = async (service?: string) => {
    ...
    try {
      ...
      setLines([`Erro: ${e instanceof Error ? e.message : "Falha ao carregar logs do serviço"}`]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const executeTruncate = async () => {
    ...
    try {
      ...
      setError(`Falha ao limpar: ${e instanceof Error ? e.message : "Erro desconhecido"}`);
    } finally {
      setTruncating(null);
    }
  };
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      setLines([
        `Erro: ${e instanceof Error ? e.message : "Falha ao carregar logs"}`,
      ]);
    } finally {
      setLoading(false);
    }
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setLines([`Erro: ${e instanceof Error ? e.message : "Falha na busca"}`]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const connectStream = async (isReconnect = false) => {
    try {
      ...
      ws.onopen = () => {
        ...
        if (isReconnect) {
          setLines((prev) => [...prev, "--- Stream reconectado ---"]);
        }
      };
      ...
      ws.onclose = () => {
        ...
        if (!userStoppedRef.current) {
          setLines((prev) => [...prev, "--- Stream desconectado, reconectando em 3s... ---"]);
      ...
      setLines((prev) => [
        ...prev,
        `Erro de stream: ${e instanceof Error ? e.message : "Falha ao conectar"}`,
      ]);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 sm:px-6 py-4 border-b border-dark-500 bg-dark-800 shrink-0">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap justify-end ml-auto">
            ...
            {isFileLogType && mode === "tail" && (
              <>
                ...
                >
                  {streaming ? "Parar Stream" : "Stream Ao Vivo"}
                </button>
                {!streaming && (
                  <button
            ...
            <button
              ...
              title="Baixar logs como arquivo de texto"
            >
              Exportar
            </button>
            ...
            >
              {loading ? "Carregando..." : mode === "search" && isFileLogType ? "Buscar" : "Atualizar"}
            </button>
          </div>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            <option value="">Logs do sistema</option>
            {sites.map((s) => (
          ...
          {logType === "docker" && !selectedSite && (
            <select
              ...
              className="text-sm border border-dark-500 rounded-lg px-3 py-1.5 bg-dark-800"
              aria-label="Container Docker"
            >
              {dockerContainers.length === 0 ? (
                <option value="">Nenhum container encontrado</option>
              ) : (
          ...
          {isFileLogType && mode === "search" && (
            <>
              ...
              <select
                ...
                className="text-sm border border-dark-500 rounded-lg px-3 py-1.5 bg-dark-800"
                aria-label="Máximo de resultados"
              >
        ...
        {streaming && (
          <div className="mt-2 flex items-center gap-2">
            ...
            <span className="text-xs text-rust-400 font-medium">
              Stream ao vivo — {lines.length} linhas recebidas
            </span>
          </div>
        )}
      </div>
      ...
      {pendingTruncate && (
        <div className="mx-4 sm:mx-6 mt-4 border border-danger-500/30 bg-danger-500/5 rounded-lg px-4 py-3 flex items-center justify-between">
          <span className="text-xs text-danger-400 font-mono">Limpar arquivo de log? Isso não pode ser desfeito: {pendingTruncate}</span>
          <div className="flex items-center gap-2 shrink-0 ml-4">
            <button onClick={executeTruncate} className="px-3 py-1.5 bg-danger-500 text-white text-xs font-bold uppercase tracking-wider hover:bg-danger-400 transition-colors">Confirmar</button>
            <button onClick={() => setPendingTruncate(null)} className="px-3 py-1.5 bg-dark-600 text-dark-200 text-xs font-bold uppercase tracking-wider hover:bg-dark-500 transition-colors">Cancelar</button>
          </div>
        </div>
      )}
      ...
      {showStats && (
        <div className="px-4 sm:px-6 py-4 border-b border-dark-500 bg-dark-800/50">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-dark-100">Estatísticas de Log</h3>
            <div className="flex items-center gap-2">
              ...
              >
                {checkingErrors ? "Verificando..." : "Verificar Taxa de Erro"}
              </button>
              <button
                onClick={loadStats}
              ...
              >
                {statsLoading ? "Carregando..." : "Recarregar"}
              </button>
            </div>
          </div>
          {logStats ? (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-3">
              ...
              <div className="bg-dark-700 rounded-lg p-3">
                <div className="text-xs text-dark-300">URLs Mais Acessadas</div>
            ...
            <div className="text-dark-300 text-sm">Carregando estatísticas...</div>
          ) : (
            <div className="text-dark-300 text-sm">Sem estatísticas disponíveis</div>
          )}
          ...
          {errorCheck && (
            <div className={`mt-2 p-3 rounded-lg text-sm ${
              ...
              <span className={errorCheck.status === "warning" ? "text-danger-400" : "text-rust-400"}>
                Taxa de erro: {errorCheck.error_rate_percent}% ({errorCheck.error_5xx} / {errorCheck.total_requests} requisições)
        ...
        <div className="px-4 sm:px-6 py-4 border-b border-dark-500 bg-dark-800/50">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-dark-100">
              ...
              {logSizes && <span className="text-dark-300 ml-2 font-normal">({logSizes.total_mb} MB no total)</span>}
            </h3>
            <button
              ...
              className="px-3 py-1 bg-dark-700 text-dark-200 rounded text-xs hover:bg-dark-600 disabled:opacity-50"
            >
              {sizesLoading ? "Carregando..." : "Recarregar"}
            </button>
          </div>
          {logSizes?.files ? (
              ...
              {logSizes.files.map((f) => (
                <div key={f.path} className="flex items-center justify-between bg-dark-700 rounded px-3 py-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {truncating === f.path ? "..." : "Limpar"}
                    </button>
                  </div>
                </div>
              ))}
              {logSizes.logrotate && (
                <div className="text-xs text-rust-400 mt-2">Logrotate configurado para nginx</div>
              )}
              {!logSizes.logrotate && (
                <div className="text-xs text-warn-400 mt-2">Logrotate não detectado para nginx</div>
              )}
            </div>
          ) : sizesLoading ? (
            <div className="text-dark-300 text-sm">Carregando...</div>
          ) : (
            <div className="text-dark-300 text-sm">Sem dados disponíveis</div>
          )}
        </div>
      )}
        ...
        {lines.length === 0 ? (
          <div className="text-dark-200 text-center py-12">
            {logType === "docker" && dockerContainers.length === 0
              ? "Nenhum container Docker gerenciado encontrado"
              : mode === "search" && isFileLogType
                ? "Digite um padrão regex e clique em Buscar"
      ...
      <div className="px-6 pt-6 pb-0">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 pb-4 border-b border-dark-600">
          <div>
            ...
            <p className="text-sm text-dark-200 font-mono mt-1">Logs de sites, eventos do sistema e trilha de auditoria</p>
          </div>
        </div>
        {/* Tabs */}
        <div className="flex gap-6 mb-6 text-sm font-mono overflow-x-auto">
          <button onClick={() => setTab("site")} className={`whitespace-nowrap ${tab === "site" ? "border-b-2 border-rust-500 text-dark-50 pb-2" : "text-dark-300 hover:text-dark-100 pb-2"}`}>Logs de Site</button>
          <button onClick={() => setTab("system")} className={`whitespace-nowrap ${tab === "system" ? "border-b-2 border-rust-500 text-dark-50 pb-2" : "text-dark-300 hover:text-dark-100 pb-2"}`}>Logs do Sistema</button>
          <button onClick={() => setTab("audit")} className={`whitespace-nowrap ${tab === "audit" ? "border-b-2 border-rust-500 text-dark-50 pb-2" : "text-dark-300 hover:text-dark-100 pb-2"}`}>Auditoria</button>
        </div>
      </div>
      {/* Content */}

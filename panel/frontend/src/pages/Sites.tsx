    ...
    <div className="animate-fade-up">
      <div className="page-header">
        <div>
          ...
          <p className="page-header-subtitle">Gerencie seus sites e aplicações</p>
        </div>
      ...
      {error && (
        <div role="alert" className="bg-danger-500/10 text-danger-400 text-sm px-4 py-3 rounded-lg border border-danger-500/20 mb-4">
          {error}
          <button onClick={() => setError("")} className="float-right font-bold" aria-label="Fechar erro">&times;</button>
        </div>
      )}
      ...
      {showForm && (
        ...
        >
          {/* Quick CMS Install */}
          <div>
            <label className="block text-xs font-medium text-dark-200 mb-2">Instalação Rápida</label>
            <div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1">
              {[
                { id: "", label: "Site Personalizado", desc: "" },
                { id: "wordpress", label: "WordPress", desc: "Blog & CMS" },
                { id: "laravel", label: "Laravel", desc: "Framework PHP" },
                { id: "drupal", label: "Drupal", desc: "CMS Enterprise" },
                { id: "joomla", label: "Joomla", desc: "CMS" },
                { id: "symfony", label: "Symfony", desc: "Framework PHP" },
                { id: "codeigniter", label: "CodeIgniter", desc: "Framework PHP" },
              ].map((c) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="site-domain" className="block text-sm font-medium text-dark-100 mb-1">Domínio</label>
              <input
                id="site-domain"
                type="text"
                value={domain}
              ...
              />
              <p className="text-xs text-dark-400 mt-1.5">O domínio público do seu site (ex: example.com)</p>
            </div>
            {!cms ? (
              <div>
                ...
                >
                  <option value="static">Estático (HTML/CSS/JS)</option>
                ...
                <p className="text-xs text-dark-400 mt-1.5">
                  {runtime === "node" ? "App Node.js com processo gerenciado (systemd + nginx reverse proxy)" :
                   runtime === "python" ? "App Python com processo gerenciado (systemd + nginx reverse proxy)" :
                   runtime === "proxy" ? "Reverse proxy para uma porta (container Docker ou serviço externo)" :
                   runtime === "php" ? "PHP com PHP-FPM — WordPress, Laravel, Drupal, etc." :
                   "Arquivos estáticos HTML/CSS/JS servidos pelo nginx"}
                </p>
              </div>
            ) : (
              <div>
                <label className="block text-sm font-medium text-dark-100 mb-1">Título do Site</label>
                <input type="text" value={siteTitle} onChange={(e) => setSiteTitle(e.target.value)} placeholder={`Meu Site ${cms.charAt(0).toUpperCase() + cms.slice(1)}`} className="w-full px-3 py-2.5 border border-dark-500 rounded-lg focus:ring-2 focus:ring-accent-500 outline-none text-sm" />
                <p className="text-xs text-dark-400 mt-1.5">O título do seu site {cms.charAt(0).toUpperCase() + cms.slice(1)}</p>
              </div>
            )}
          </div>
          ...
          {cms && (
            ...
            <div className="col-span-2 border-t border-dark-700 pt-3 mt-1">
              <span className="text-xs font-medium text-dark-400 uppercase tracking-wider">Configuração do {cms.charAt(0).toUpperCase() + cms.slice(1)}</span>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-dark-100 mb-1">Email do Admin</label>
                ...
                <p className="text-xs text-dark-400 mt-1.5">Email do administrador do {cms.charAt(0).toUpperCase() + cms.slice(1)}</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-dark-100 mb-1">Usuário Admin</label>
              ...
              <div>
                <label className="block text-sm font-medium text-dark-100 mb-1">Senha do Admin</label>
                <input type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="Gerada automaticamente se vazia" className="w-full px-3 py-2.5 border border-dark-500 rounded-lg focus:ring-2 focus:ring-accent-500 outline-none text-sm" />
              </div>
            </div>
            </>
          )}
          ...
          {runtime === "proxy" && (
            <div>
              <label htmlFor="site-proxy-port" className="block text-sm font-medium text-dark-100 mb-1">Porta do Proxy</label>
              <input
                id="site-proxy-port"
                type="number"
                value={proxyPort}
              ...
              />
              <p className="text-xs text-dark-400 mt-1.5">A porta local em que sua aplicação escuta</p>
            </div>
          )}

          {(runtime === "node" || runtime === "python") && (
            <div>
              <label htmlFor="site-app-command" className="block text-sm font-medium text-dark-100 mb-1">Comando de Início</label>
              <input
                id="site-app-command"
                type="text"
                value={appCommand}
              ...
              <p className="text-xs text-dark-400 mt-1.5">
                {runtime === "node"
                  ? "ex: npm start, node server.js, npx next start"
                  : "ex: gunicorn app:app, uvicorn main:app, flask run"}
                {" "}— porta alocada automaticamente via env var $PORT
              </p>
            </div>
          )}
          ...
          {runtime === "php" && !cms && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="site-php-version" className="block text-sm font-medium text-dark-100 mb-1">Versão do PHP</label>
                <select
                  id="site-php-version"
                  value={phpVersion}
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 border border-dark-500 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none text-sm bg-dark-800"
                >
                  <option value="generic">PHP Genérico</option>
                ...
                <p className="text-xs text-dark-400 mt-1.5">Preset de configuração nginx para o seu framework PHP</p>
              </div>
            </div>
          )}
      ...
      {loading ? (
        ...
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-12 text-center">
          ...
          <p className="text-dark-200 font-medium text-lg">Nenhum site ainda</p>
          <p className="text-dark-300 text-sm mt-2 max-w-md mx-auto">Faça deploy de sites estáticos, PHP, Node.js ou Python com certificados SSL automáticos, configuração nginx e instalação CMS em um clique.</p>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-dark-500 bg-dark-900">
                <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase tracking-widest font-mono px-5 py-3">Domínio</th>
                ...
                <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase tracking-widest font-mono px-5 py-3 hidden lg:table-cell">Criado em</th>
              </tr>
            </thead>
                ...
                <tr key={site.id} className="hover:bg-dark-700/30 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      ...
                      {site.enabled === false ? "desabilitado" : site.status}
                    </span>
                  </td>
                  <td className="px-5 py-4 hidden md:table-cell">
                    {site.ssl_enabled ? (
                      ...
                      <span className="text-xs text-dark-300">Nenhum</span>
                    )}
                  </td>

...
export default function Users() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setMessage({
        text: e instanceof Error ? e.message : "Falha ao criar usuário",
        type: "error",
      });
    } finally {
      setCreating(false);
  ...
  const handleUpdateRole = async (id: string, newRole: string) => {
    try {
      ...
      setEditTarget(null);
      setMessage({ text: "Papel atualizado", type: "success" });
      loadUsers();
    } catch (e) {
      setMessage({
        text: e instanceof Error ? e.message : "Falha ao atualizar",
        type: "error",
      });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setMessage({
        text: e instanceof Error ? e.message : "Falha ao excluir",
        type: "error",
      });
    }
  };
  ...
  const handleToggleSuspend = async (user: User) => {
    ...
    try {
      ...
      setMessage({
        text: data.suspended ? `${user.email} foi suspenso` : `${user.email} foi reativado`,
        type: "success",
      });
      loadUsers();
    } catch (e) {
      setMessage({
        text: e instanceof Error ? e.message : "Falha ao atualizar usuário",
        type: "error",
      });
    } finally {
  ...
  const handleResetPassword = async (id: string) => {
    ...
    try {
      ...
      setMessage({
        text: e instanceof Error ? e.message : "Falha ao redefinir senha",
        type: "error",
      });
    } finally {
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <p className="page-header-subtitle">Gerencie contas de usuário e permissões</p>
        </div>
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-x-auto">
        {loading ? (
          ...
          <div className="p-12 text-center">
            <p className="text-dark-200 font-medium">Nenhum usuário</p>
          </div>
        ) : (
          <>
          {/* Desktop Table */}
          <table className="w-full hidden sm:table">
            <thead>
              <tr className="bg-dark-900 border-b border-dark-500">
                ...
                <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3 w-28">Papel</th>
                ...
                <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3 w-36">Criado em</th>
                <th scope="col" className="text-right text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3 w-44">Ações</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-dark-600">
              {users.filter((u) => u.email.toLowerCase().includes(search.toLowerCase())).map((user) => (
                <tr key={user.id} className={`table-row-hover ${user.role === "suspended" ? "opacity-60" : ""}`}>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {user.role === "suspended" && (
                          <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-danger-500/15 text-danger-400">Suspenso</span>
                        )}
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                    {user.role === "suspended" ? (
                      <span className="inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium bg-danger-500/15 text-danger-400">suspenso</span>
                    ) : editTarget === user.id ? (
                  ...
                  <td className="px-5 py-4 text-right">
                    {deleteTarget === user.id ? (
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => handleDelete(user.id)} className="px-2 py-1 bg-danger-500 text-white rounded text-xs">Confirmar</button>
                        <button onClick={() => setDeleteTarget(null)} className="px-2 py-1 bg-dark-600 text-dark-200 rounded text-xs">Cancelar</button>
                      </div>
                    ) : (
                      <div className="flex items-center justify-end gap-1">
                        {!isSelf(user.id) && (
                        ...
                        <button onClick={() => setDeleteTarget(user.id)} className="text-dark-300 hover:text-danger-500 transition-colors" title="Excluir usuário">
                  ...
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {user.role === "suspended" && (
                          <span className="inline-flex px-1.5 py-0.5 rounded-full text-[10px] font-medium bg-danger-500/15 text-danger-400 shrink-0">Suspenso</span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0 ml-2">
                    ...
                    {deleteTarget === user.id ? (
                      <div className="flex items-center gap-1">
                        <button onClick={() => handleDelete(user.id)} className="px-2 py-1 bg-danger-500 text-white rounded text-xs">Excl</button>
                        <button onClick={() => setDeleteTarget(null)} className="px-2 py-1 bg-dark-600 text-dark-200 rounded text-xs">Não</button>
                      </div>
                    ) : (
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  autoFocus
                />
                <p className="text-xs text-dark-400 mt-1">Email do usuário para login</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-xs text-dark-400 mt-1">Admin tem acesso total, Usuário tem acesso limitado</p>
              </div>
            </div>
        ...
        >
          <div className="bg-dark-800 rounded-lg shadow-xl p-6 w-[420px] dp-modal">
            ...
            <p className="text-sm text-dark-200 mb-4">
              Defina uma nova senha para <span className="font-mono text-dark-50">{users.find(u => u.id === resetPwTarget)?.email}</span>
            </p>
            <div>
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              >
                ...
                {resettingPw ? "Redefinindo..." : "Redefinir Senha"}
              </button>
            </div>
          </div>
        </div>

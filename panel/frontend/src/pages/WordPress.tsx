...
export default function WordPress() {
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      flash("WordPress instalado com sucesso!", "success");
      setShowInstall(false);
      loadInfo();
    } catch (e) {
      flash(e instanceof Error ? e.message : "Falha na instalação", "error");
    } finally {
      setBusy("");
    }
  };
  ...
  const handleUpdate = async (target: string) => {
    ...
    try {
      ...
      flash(`${target.charAt(0).toUpperCase() + target.slice(1)} atualizado`, "success");
      loadInfo();
    } catch (e) {
      flash(e instanceof Error ? e.message : "Falha na atualização", "error");
    } finally {
      setBusy("");
    }
  };
  ...
  const handlePluginAction = async (name: string, action: string) => {
    ...
    try {
      ...
      flash(`Plugin ${action === "activate" ? "ativado" : action === "deactivate" ? "desativado" : action === "update" ? "atualizado" : action === "delete" ? "excluído" : action}: ${name}`, "success");
      loadPlugins();
    } catch (e) {
      flash(e instanceof Error ? e.message : `Falha em ${action}`, "error");
    } finally {
      setBusy("");
    }
  };
  ...
  const handleThemeAction = async (name: string, action: string) => {
    ...
    try {
      ...
      flash(`Tema ${action === "activate" ? "ativado" : action === "deactivate" ? "desativado" : action === "update" ? "atualizado" : action === "delete" ? "excluído" : action}: ${name}`, "success");
      loadThemes();
    } catch (e) {
      flash(e instanceof Error ? e.message : `Falha em ${action}`, "error");
    } finally {
  ...
  const handleAutoUpdate = async (enabled: boolean) => {
    ...
    try {
      ...
      flash(`Atualizações automáticas ${enabled ? "habilitadas" : "desabilitadas"}`, "success");
    } catch (e) {
      flash(e instanceof Error ? e.message : "Falha", "error");
    } finally {
      setBusy("");
    }
  };
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex items-center gap-3 mb-6 pb-4 border-b border-dark-600">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-dark-200 font-mono mt-0.5">Gerenciar instalação do WordPress</p>
        </div>
      </div>
        ...
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-8">
          <div className="text-center max-w-md mx-auto">
            ...
            <p className="text-sm text-dark-200 mb-6">
              Nenhum wp-config.php encontrado na raiz deste site. Instale o WordPress com um clique.
            </p>
            <button
          ...
          {showInstall && (
            <div className="mt-8 pt-6 border-t border-dark-500 space-y-4">
              <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Detalhes da Instalação</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">URL do Site</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">Título do Site</label>
                  <input
                    type="text"
              ...
              <h4 className="text-xs font-semibold text-dark-200 uppercase tracking-wider pt-2">Conta de Admin</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">Usuário</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">Senha</label>
                  <input
                    type="password"
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">E-mail</label>
                  <input
                    type="email"
              ...
              <h4 className="text-xs font-semibold text-dark-200 uppercase tracking-wider pt-2">Banco de Dados MySQL</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">Nome do Banco</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">Host do Banco</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">Usuário do Banco</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">Senha do Banco</label>
                  <input
                    type="password"
              ...
              <p className="text-xs text-dark-300">
                Você precisa de um banco de dados MySQL/MariaDB. Crie um na página Apps (faça deploy do MariaDB) ou use um banco externo.
              </p>
              <div className="flex items-center gap-3 pt-2">
                ...
                >
                  {busy === "install" ? "Instalando..." : "Instalar WordPress"}
                </button>
                <button
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {info.update_available && (
                      <span className="px-2 py-0.5 bg-warn-500/15 text-warn-400 rounded text-xs font-medium">
                        {info.update_available} disponível
                      </span>
                    )}
                  </div>
                  <p className="text-sm text-dark-200">
                    ...
                    {updatesAvailable > 0 && (
                      <span className="text-warn-500 ml-1">
                        ({updatesAvailable} atualizaç{updatesAvailable !== 1 ? "ões" : "ão"} disponí{updatesAvailable !== 1 ? "veis" : "vel"})
                      </span>
                    )}
                  </p>
                </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {info.update_available && (
                  ...
                  >
                    {busy === "update-core" ? "Atualizando..." : "Atualizar Core"}
                  </button>
                )}
                {plugins.some((p) => p.update === "available") && (
                  ...
                  >
                    {busy === "update-plugins" ? "Atualizando..." : "Atualizar Todos os Plugins"}
                  </button>
                )}
                {themes.some((t) => t.update === "available") && (
                  <button
                    ...
                    className="px-3 py-1.5 bg-rust-500 text-white rounded-lg text-xs font-medium hover:bg-rust-600 disabled:opacity-50"
                  >
                    {busy === "update-themes" ? "Atualizando..." : "Atualizar Todos os Temas"}
                  </button>
                )}
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (result.rolled_back) {
                        setMessage({ text: "Atualização falhou no health check — revertida para o snapshot", type: "warning" });
                      } else {
                        setMessage({ text: `Atualização segura concluída: ${result.core_before} → ${result.core_after}`, type: "success" });
                      }
                      loadInfo();
                    } catch (e) {
                      setMessage({ text: e instanceof Error ? e.message : "Falha na atualização segura", type: "error" });
                    } finally {
                      setBusy("");
                    }
                  }}
                  ...
                  title="Cria um snapshot antes de atualizar e reverte automaticamente se o health check falhar"
                >
                  {busy === "safe-update" ? "Atualizando..." : "Atualização Segura (Rollback)"}
                </button>
                <label className="flex items-center gap-2 ml-2 cursor-pointer">
                  <span className="text-xs text-dark-200">Auto-atualização</span>
                  <button
                    ...
                    <tr key={p.name} className="hover:bg-dark-700/30 transition-colors">
                      ...
                      <td className="px-4 py-2.5">
                        {p.update === "available" ? (
                          ...
                          <span className="text-xs text-dark-300">atualizado</span>
                        )}
                      </td>
                      <td className="px-4 py-2.5">

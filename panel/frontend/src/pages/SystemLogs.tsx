...
export default function SystemLogsContent() {
  ...
  const loadEntries = async (offset: number, append: boolean) => {
    try {
      ...
      setError(e instanceof Error ? e.message : "Falha ao carregar logs do sistema");
    } finally {
      setLoading(false);
    ...
    <div className="px-6 pb-6 animate-fade-up">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-dark-800 rounded-lg border border-dark-500 px-4 py-3">
          <div className="text-xs text-dark-300 uppercase font-mono tracking-wider">Erros (24h)</div>
          <div className="text-2xl font-bold font-mono text-danger-400 mt-1">{counts.error}</div>
        </div>
        <div className="bg-dark-800 rounded-lg border border-dark-500 px-4 py-3">
          <div className="text-xs text-dark-300 uppercase font-mono tracking-wider">Avisos (24h)</div>
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-x-auto">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-dark-200 font-medium">Nenhum evento do sistema registrado</p>
            <p className="text-dark-300 text-sm mt-1">Erros e avisos do backend aparecerão aqui</p>
          </div>
        ) : (
          <>
            {/* Desktop Table */}
            <table className="w-full hidden sm:table">
              <thead>
                <tr className="bg-dark-900 border-b border-dark-500">
                  <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3 w-28">Quando</th>
                  <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3 w-24">Nível</th>
                  <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3 w-40">Origem</th>
                  <th scope="col" className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-3">Mensagem</th>
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-5 py-3 align-top">
                        ...
                        {isExpanded && !entry.details && (
                          <div className="mt-2 text-xs text-dark-300 font-mono">Sem detalhes adicionais</div>
                        )}
                      </td>
                    </tr>
                  );

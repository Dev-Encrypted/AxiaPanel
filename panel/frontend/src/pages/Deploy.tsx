...
export default function Deploy() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setConfig(cfg);
      setMessage({ text: "Configuração de deploy salva.", type: "success" });
    } catch (err) {
  ...
  const handleDeploy = async () => {
    ...
    try {
      ...
      if (result.deploy_id) {
        ...
        setMessage({ text: "Deploy concluído.", type: "success" });
        setDeploying(false);
        await load();
      }
    } catch (err) {
  ...
  const handleRollback = (releaseId: string) => {
    setPendingConfirm({ type: "rollback", id: releaseId, label: `Reverter para a release ${releaseId}? Esta release será ativada imediatamente.` });
  };
  ...
  const executeRollback = async (releaseId: string) => {
    ...
    try {
      ...
      setMessage({ text: `Revertido para a release ${releaseId}.`, type: "success" });
      await load();
    } catch (err) {
  ...
  const handleRemove = () => {
    setPendingConfirm({ type: "remove", id: "", label: "Remover configuração de deploy? Isto não excluirá os arquivos do seu site." });
  };
  ...
  const executeRemove = async () => {
    try {
      ...
      setKeepReleases(5);
      setReleases([]);
      setMessage({ text: "Configuração de deploy removida.", type: "success" });
    } catch (err) {
      setMessage({ text: err instanceof Error ? err.message : "Falha ao remover", type: "error" });
    }
  };
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div>
        ...
        <span className="text-sm text-dark-300 mx-2">/</span>
        <span className="text-sm text-dark-50 font-medium">Deploy Git</span>
      </div>

      <div className="flex items-center justify-between">
        <h1 className="text-sm font-medium text-dark-300 uppercase font-mono tracking-widest">Deploy Git</h1>
        {config && (
          <div className="flex items-center gap-3">
            ...
            >
              {deploying ? "Deployando..." : "Deploy Agora"}
            </button>
            <button
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
        <div className="px-5 py-4 border-b border-dark-600">
          <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Repositório</h2>
        </div>
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-end">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-sm text-dark-100">Auto-deploy ao receber webhook de push</span>
              </label>
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-dark-100 mb-1">Script de Deploy (opcional)</label>
            <textarea
              value={deployScript}
              onChange={(e) => setDeployScript(e.target.value)}
              placeholder={"# Executado após git pull. Exemplo:\nnpm install\nnpm run build"}
              rows={4}
          ...
          <div className="border border-dark-500 rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-medium text-dark-50">Deploy Sem Downtime</h3>
                <p className="text-xs text-dark-300 mt-0.5">
                  Deploys atômicos por symlink no estilo Capistrano. Cada deploy cria uma release imutável. O rollback é instantâneo.
                </p>
              </div>
            ...
            {atomicDeploy && (
              <div className="flex items-center gap-4">
                <div>
                  <label className="block text-xs font-medium text-dark-200 mb-1">Manter Releases</label>
                  <input
                    type="number"
                    value={keepReleases}
                ...
                <p className="text-xs text-dark-300 mt-4">
                  Releases antigas são removidas automaticamente. Diretórios compartilhados (uploads, .env) persistem entre releases.
                </p>
              </div>
            )}
          </div>
      ...
      {config?.atomic_deploy && releases.length > 0 && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
          <div className="px-5 py-4 border-b border-dark-600">
            ...
            <p className="text-xs text-dark-200 mt-1">{releases.length} release{releases.length !== 1 ? "s" : ""} em disco. A release ativa atende ao tráfego.</p>
          </div>
              ...
              <div key={rel.id} className="px-5 py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {!rel.active && (
                    ...
                    >
                      {rollingBack === rel.id ? "Revertendo..." : "Reverter"}
                    </button>
                  )}
                </div>
              </div>
      ...
      {config && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
          <div className="px-5 py-4 border-b border-dark-600">
            ...
            <p className="text-xs text-dark-200 mt-1">Para repositórios privados via SSH. Adicione esta chave como uma deploy key no seu provedor Git.</p>
          </div>
          <div className="p-5">
      ...
      {config && config.last_deploy && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
          <div className="px-5 py-4 flex items-center justify-between">
            <div>
              <span className="text-sm font-medium text-dark-100">Último deploy: </span>

...
const PRESETS = [
  ...
  { label: "Todo domingo à meia-noite", value: "0 0 * * 0" },
  { label: "Toda segunda às 6h", value: "0 6 * * 1" },
  { label: "Dia 1 de cada mês", value: "0 0 1 * *" },
];
...
export default function Crons() {
  ...
  const loadCrons = async () => {
    ...
    try {
      ...
      setMessage({ text: e instanceof Error ? e.message : "Falha ao carregar crons", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      setCommand("");
      setSchedule("0 * * * *");
      setMessage({ text: "Cron job criado", type: "success" });
      loadCrons();
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha ao criar cron", type: "error" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleToggle = async (cron: Cron) => {
    try {
      ...
      setMessage({ text: e instanceof Error ? e.message : "Falha ao atualizar cron", type: "error" });
    }
  };
  ...
  const handleDelete = async (cronId: string) => {
    try {
      ...
      setMessage({ text: "Cron job excluído", type: "success" });
      loadCrons();
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha ao excluir", type: "error" });
    }
  };
  ...
  const handleRun = async (cron: Cron) => {
    ...
    try {
      ...
      setMessage({ text: e instanceof Error ? e.message : "Falha na execução", type: "error" });
    } finally {
      setRunning(null);
    }
  };
  ...
  const handleEdit = async (cron: Cron) => {
    if (editId === cron.id) {
      ...
      try {
        ...
        setEditId(null);
        setMessage({ text: "Cron job atualizado", type: "success" });
        loadCrons();
      } catch (e) {
        setMessage({ text: e instanceof Error ? e.message : "Falha ao atualizar", type: "error" });
      }
    } else {
      // Enter edit mode
      setEditId(cron.id);
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-dark-600">
        ...
        >
          {showForm ? "Cancelar" : "Adicionar Cron Job"}
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-dark-800 rounded-lg border border-dark-500 p-5 mb-6 space-y-4">
          {/* Preset Templates */}
          <div>
            <label className="block text-sm font-medium text-dark-100 mb-1">Modelo</label>
            <select
              value=""
              onChange={(e) => {
                ...
                const templates: Record<string, { label: string; command: string; schedule: string }> = {
                  ...
                  "log-cleanup": { label: "Limpeza de Logs (30 dias)", command: `find /var/log/nginx -name '*.log' -mtime +30 -delete`, schedule: "0 3 * * 0" },
                  "disk-check": { label: "Alerta de Uso de Disco", command: `df -h / | awk 'NR==2{if($5+0>90)print "DISK WARNING: "$5}'`, schedule: "0 */6 * * *" },
                };
            ...
            >
              ...
              <option value="log-cleanup">Limpeza de Logs (30 dias)</option>
              <option value="disk-check">Alerta de Uso de Disco</option>
            </select>
          </div>
          <div>
            <label htmlFor="cron-label" className="block text-sm font-medium text-dark-100 mb-1">Rótulo (opcional)</label>
            <input
              id="cron-label"
              type="text"
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              placeholder="Limpar arquivos temporários"
          ...
          <div>
            <label htmlFor="cron-command" className="block text-sm font-medium text-dark-100 mb-1">Comando</label>
            <input
              id="cron-command"
              type="text"
              value={command}
            ...
            />
            <p className="text-xs text-dark-300 mt-1">Comando shell para executar</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="cron-preset" className="block text-sm font-medium text-dark-100 mb-1">Preset de Schedule</label>
              <select
                id="cron-preset"
            ...
            <div>
              <label htmlFor="cron-schedule" className="block text-sm font-medium text-dark-100 mb-1">Expressão Cron</label>
              <input
                id="cron-schedule"
                type="text"
                value={schedule}
                ...
                className="w-full px-3 py-2.5 border border-dark-500 rounded-lg focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none text-sm font-mono"
              />
              <p className="text-xs text-dark-300 mt-1">Expressão cron, ex.: */5 * * * * a cada 5 minutos</p>
            </div>
          </div>
          <div className="flex gap-3">
            ...
            >
              ...
              {submitting ? "Adicionando..." : "Criar Cron Job"}
            </button>
            <button
              type="button"
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-dark-200 font-medium">Nenhum cron job ainda</p>
            <p className="text-dark-300 text-sm mt-2 max-w-md mx-auto">Agende tarefas recorrentes com expressões cron. Execute scripts, backups ou comandos de manutenção em qualquer intervalo.</p>
          </div>
        ) : (
              ...
              <div key={cron.id} className="px-5 py-4 hover:bg-dark-700/30 transition-colors">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        aria-label={cron.enabled ? "Desabilitar cron" : "Habilitar cron"}
                      >
                    ...
                    <div className="flex items-center gap-4 mt-2 ml-12">
                      ...
                      {cron.last_run && (
                        <span className="text-xs text-dark-300">
                          Última execução: {formatDate(cron.last_run)}
                          {cron.last_status && (
                  ...
                  <div className="flex items-center gap-2 shrink-0 flex-wrap">
                    ...
                    >
                      {running === cron.id ? "Executando..." : "Executar Agora"}
                    </button>
                    <button
                      ...
                      aria-label="Editar cron"
                    >
                      Editar
                    </button>
                    {deleteTarget === cron.id ? (
                      <div className="flex items-center gap-1">
                        <button onClick={() => handleDelete(cron.id)} className="px-2 py-1 bg-danger-600 text-white rounded-md text-xs">Confirmar</button>
                ...
                {editId === cron.id && (
                  <div className="mt-3 ml-12 p-4 bg-dark-900 rounded-lg border border-dark-500 space-y-3">
                    <div>
                      <label className="block text-xs font-medium text-dark-200 mb-1">Rótulo</label>
                      <input
                        type="text"
                        value={label}
                        onChange={(e) => setLabel(e.target.value)}
                    ...
                    <div>
                      <label className="block text-xs font-medium text-dark-200 mb-1">Comando</label>
                      <input
                        type="text"
                        value={command}
                        onChange={(e) => setCommand(e.target.value)}

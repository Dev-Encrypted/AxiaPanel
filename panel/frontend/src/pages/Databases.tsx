  ...
  return (
    <div className="animate-fade-up">
      <div className="flex items-center gap-3 mb-5 pb-4 border-b border-dark-600">
        ...
        <h1 className="text-sm font-medium text-dark-300 uppercase font-mono tracking-widest">Navegador de Schema</h1>
      ...
      {loading && <p className="text-dark-400 text-sm">Carregando schema...</p>}
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-1.5 truncate">
                      {hasFk && <span className="w-1.5 h-1.5 rounded-full bg-rust-400 shrink-0" title="Possui relacionamentos" />}
                      {name}
                    </span>
          ...
          <div className="flex-1 space-y-4">
            {selectedTable && tableSchema ? (
              <>
                <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
                  <div className="px-4 py-3 border-b border-dark-600 flex items-center justify-between">
                    ...
                    <span className="text-xs text-dark-400">{tableSchema.rows?.length || 0} colunas</span>
                  </div>
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-dark-600 bg-dark-700/50">
                        <th className="text-left px-4 py-2 text-xs text-dark-300 uppercase">Coluna</th>
                        <th className="text-left px-4 py-2 text-xs text-dark-300 uppercase">Tipo</th>
                        <th className="text-left px-4 py-2 text-xs text-dark-300 uppercase">Nullable</th>
                        <th className="text-left px-4 py-2 text-xs text-dark-300 uppercase">Padrão</th>
                        {database.engine !== "postgres" && <th className="text-left px-4 py-2 text-xs text-dark-300 uppercase">Chave</th>}
                      </tr>
                    </thead>
                    <tbody>
                      {(tableSchema.rows || []).map((col: string[], i: number) => (
                        <tr key={i} className="border-b border-dark-700 hover:bg-dark-700/30">
                          ...
                          <td className="px-4 py-2 text-dark-300">{col[database.engine === "postgres" ? 3 : 2] === "YES" ? "Sim" : "Não"}</td>
                ...
                {tableIndexes && tableIndexes.rows?.length > 0 && (
                  <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
                    <div className="px-4 py-3 border-b border-dark-600">
                      <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Índices</h3>
                    </div>
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-dark-600 bg-dark-700/50">
                ...
                {(() => {
                  ...
                  return (
                    <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
                      <div className="px-4 py-3 border-b border-dark-600">
                        <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Relacionamentos</h3>
                      </div>
              ...
              <div className="bg-dark-800 rounded-lg border border-dark-500 p-8 text-center">
                <p className="text-dark-400 text-sm">Selecione uma tabela para ver seu schema, índices e relacionamentos.</p>
                {fks.length > 0 && (
                  <div className="mt-6 text-left">
                    <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest mb-3">Todos os Relacionamentos ({fks.length})</h3>
...
function SqlBrowser({
  ...
  const loadTables = useCallback(async () => {
    ...
    try {
      ...
      setQueryError(e instanceof Error ? e.message : "Falha ao carregar tabelas");
    } finally {
  ...
  const executeQuery = useCallback(
    async (queryStr?: string) => {
      ...
      try {
        ...
        setQueryError(e instanceof Error ? e.message : "Falha na consulta");
      } finally {
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              {result && (
                <span className="text-xs text-dark-300">
                  {result.execution_time_ms}ms &middot; {result.row_count} {result.row_count !== 1 ? "linhas" : "linha"}
          ...
          {result &&
            ...
            !queryError && (
              <div className="bg-dark-800 rounded-lg border border-dark-500 p-8 text-center text-dark-300 text-sm">
                ...
                Consulta executada com sucesso ({result.execution_time_ms}ms)
              </div>
            )}
          ...
          {!result && !queryError && !queryLoading && (
            <div className="bg-dark-800 rounded-lg border border-dark-500 p-12 text-center flex-1 flex flex-col items-center justify-center">
              ...
              <p className="text-dark-300 text-sm">
                Selecione uma tabela ou escreva uma consulta
              </p>
              <p className="text-dark-500 text-xs mt-1">
                Ctrl+Up/Down para navegar no histórico de consultas
              </p>
            </div>
          )}
        </div>
...
export default function Databases() {
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      setSuccessMsg(`Banco "${dbName}" criado`);
      setDbName("");
      fetchData();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Falha ao criar banco");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const getSiteDomain = (siteId: string) =>
    sites.find((s) => s.id === siteId)?.domain || "Desconhecido";
  ...
  return (
    <div className="animate-fade-up">
      <div className="page-header">
        <div>
          <h1 className="page-header-title">Bancos de Dados</h1>
          <p className="page-header-subtitle">Crie e gerencie seus bancos de dados</p>
        </div>
      ...
      {error && (
        <div role="alert" className="bg-danger-500/10 text-danger-400 text-sm px-4 py-3 rounded-lg border border-danger-500/20 mb-4">
          {error}
          <button onClick={() => setError("")} className="float-right font-bold" aria-label="Fechar erro">
            &times;
          </button>
        </div>
      )}
      ...
      {successMsg && (
        <div className="bg-rust-500/10 text-rust-400 text-sm px-4 py-3 rounded-lg border border-rust-500/20 mb-4">
          {successMsg}
          <button onClick={() => setSuccessMsg("")} className="float-right font-bold" aria-label="Dispensar">
            &times;
          </button>
        </div>
      )}
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-dark-300 mt-1">Nome para a instância do seu banco</p>
            </div>
            <div>
              ...
              <p className="text-xs text-dark-300 mt-1">MySQL, MariaDB ou PostgreSQL</p>
            </div>
          </div>
      ...
      {loading ? (
        ...
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-12 text-center">
          ...
          <p className="text-dark-200 font-medium">Nenhum banco de dados ainda</p>
          <p className="text-dark-300 text-sm mt-2 max-w-md mx-auto">Crie bancos MySQL ou PostgreSQL com containers Docker automáticos, gestão de usuários e navegador SQL integrado.</p>
        ...
        <div className="space-y-0">
                  ...
                  <tr key={db.id} className="hover:bg-dark-700/30 transition-colors">
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={async () => {
                            try {
                              ...
                              setError("");
                              setSuccessMsg(`Point-in-time recovery ${newEnabled ? "habilitado" : "desabilitado"} para ${db.name}`);
                            } catch (e) { setError(e instanceof Error ? e.message : "Falha ao alternar PITR"); }
                          }}
                          className="px-2 py-1 rounded text-xs font-medium bg-dark-700 text-dark-300 hover:bg-dark-600 transition-colors"
                          title="Alternar recuperação point-in-time (WAL/binlog)"
                        >
                          PITR
                        </button>
                        <button
                          ...
                          title="Gerar nova senha do banco"
                        >
                          {resettingPw === db.id ? "..." : "Redefinir Senha"}
                        </button>
                        <button
                        ...
                        >
                          {credentialsId === db.id ? "Ocultar" : "Conectar"}
                        </button>
                        {confirmDeleteId === db.id ? (
                          <>
                            <span className="text-xs text-danger-400">Tem certeza?</span>
                            <button
          ...
          {pwResetSuccess && (
            <div className="mt-4 bg-dark-800 rounded-lg border border-accent-500/30 p-4">
              ...
              <p className="text-xs text-dark-300 mb-2">Salve esta senha agora — ela não será mostrada novamente depois que você sair desta página.</p>
              <div className="flex items-center gap-2">
                ...
                >
                  {copied === "new_password" ? "Copiado!" : "Copiar"}
                </button>
              </div>
            </div>
          )}
          ...
          {credentialsId && (
            <div className="mt-4 bg-dark-800 rounded-lg border border-dark-500 p-5">
              {credentialsLoading ? (
                <div className="text-center text-dark-300 py-4">Carregando credenciais...</div>
              ) : credentials ? (
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {[
                      ...
                      { label: "Porta", value: String(credentials.port), key: "port" },
                      { label: "Banco", value: credentials.database, key: "database" },
                      { label: "Usuário", value: credentials.username, key: "username" },
                      { label: "Senha", value: credentials.password, key: "password" },
                      { label: "Host Interno", value: credentials.internal_host, key: "internal_host" },
                    ].map((field) => (
                      <div key={field.key}>
                        <label className="block text-xs font-medium text-dark-200 mb-1">
                          {field.label}

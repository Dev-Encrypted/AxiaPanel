...
export default function Dashboard() {
  ...
  const fetchSlowData = useCallback(() => {
    api
      ...
      .catch(() => setError("Falha ao carregar sites. Tente novamente."));
    api
      .get<{ id: string }[]>("/databases")
      .then((list) => setDbCount(list.length))
      .catch(() => setError("Falha ao carregar bancos de dados. Tente novamente."));
    api
      .get<Intelligence>("/dashboard/intelligence")
      .then(setIntel)
      .catch(() => setError("Falha ao carregar dados do dashboard"));
    api
    ...
    api
      ...
      .catch(() => setError("Falha ao carregar status de atualizações"));
    api
  ...
  const fetchRealtimeData = useCallback(() => {
    api
      ...
      .then(setSystem)
      .catch((e) => setError(e instanceof Error ? e.message : "Falha ao carregar informações do sistema"));
    api
  ...
  const overallStatus = useMemo(() => {
    ...
    if (alertCount > 0 || healthScore < 50)
      return { label: "Problemas Detectados", color: "bg-danger-500/10 border-danger-500/20 text-danger-400", dot: "bg-danger-400" };
    if (healthScore < 80)
      return { label: "Performance Degradada", color: "bg-warn-500/10 border-warn-500/20 text-warn-400", dot: "bg-warn-500" };
    return { label: "Todos os Sistemas Operacionais", color: "bg-rust-500/10 border-rust-500/20 text-rust-400", dot: "bg-rust-500" };
  }, [intel]);
    ...
    <div className="p-4 sm:p-6 lg:p-8 animate-fade-up">
      <div className="page-header">
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className="text-xs text-dark-400 mt-0.5">{system?.hostname || "Carregando..."}</p>
          </div>
          <span className="flex items-center gap-1.5" title={wsConnected ? "Recebendo métricas ao vivo via WebSocket" : "Consultando métricas via HTTP"}>
            <span className={`w-1.5 h-1.5 rounded-full ${wsConnected ? "bg-rust-500 animate-pulse" : "bg-dark-400"}`} />
            <span className="text-[10px] text-dark-400 font-mono">{wsConnected ? "Ao vivo" : "Polling"}</span>
          </span>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={() => setShowWidgetConfig(!showWidgetConfig)}
            ...
            {showWidgetConfig ? "Concluído" : "Personalizar"}
          </button>
          <div className="h-4 w-px bg-dark-600 hidden sm:block" />
      ...
      {showWidgetConfig && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-4 mb-4">
          <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest mb-3">Widgets do Dashboard</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {[
              { id: "metrics", label: "CPU / Memória / Disco" },
              { id: "disk_io", label: "I/O de Disco" },
              { id: "charts", label: "Gráficos Históricos" },
              { id: "status_bar", label: "Barra de Status" },
              { id: "health_banner", label: "Indicador de Saúde" },
              { id: "sites_grid", label: "Grade de Sites" },
              { id: "activity", label: "Atividade Recente" },
              { id: "issues", label: "Problemas Ativos" },
              { id: "ssl_countdown", label: "Contagem SSL" },
              { id: "network", label: "I/O de Rede" },
              { id: "processes", label: "Processos Principais" },
              { id: "system_info", label: "Informações do Sistema" },
              { id: "onboarding", label: "Primeiros Passos" },
              { id: "bookmarks", label: "Links Rápidos" },
            ].map(w => (
          ...
          <button onClick={() => { setWidgetConfig({}); localStorage.removeItem('dp-dashboard-widgets'); }}
            className="mt-3 text-[10px] text-dark-400 hover:text-dark-200">Restaurar padrões</button>
        </div>
      )}
      ...
      {confirmAction && (
        <div className={`rounded-lg border px-4 py-3 mb-4 flex items-center justify-between ${
          ...
          <span className={`text-xs font-mono ${confirmAction === "reboot" ? "text-danger-400" : "text-warn-400"}`}>
            {confirmAction === "nginx" ? "Reiniciar Nginx? Isso interromperá brevemente o tráfego web." :
             confirmAction === "php" ? "Reiniciar PHP-FPM? Requisições ativas podem ser interrompidas." :
             "Reiniciar servidor? Todos os serviços ficarão indisponíveis temporariamente."}
          </span>
            ...
            <button onClick={async () => {
              ...
              try {
                if (action === "reboot") {
                  ...
                  setActionMessage({ text: "Servidor reiniciando...", type: "success" });
                  setTimeout(() => setActionMessage(null), 5000);
                } else {
                  await api.post("/agent/diagnostics/fix", { fix: action === "nginx" ? "restart_nginx" : "restart_php" });
                  setActionMessage({ text: action === "nginx" ? "Nginx reiniciado" : "PHP-FPM reiniciado", type: "success" });
                  setTimeout(() => setActionMessage(null), 3000);
                }
              } catch {
                setActionMessage({ text: `Falha ao ${action === "reboot" ? "reiniciar servidor" : `reiniciar ${action === "nginx" ? "Nginx" : "PHP-FPM"}`}`, type: "error" });
                setTimeout(() => setActionMessage(null), 3000);
              }
      ...
      {isVisible("onboarding") && !dismissed && system && (() => {
        const steps: OnboardingStep[] = [
          { id: "site", label: "Crie seu primeiro site", description: "Configure um site com Nginx, PHP ou reverse proxy", link: "/sites", check: () => sites.total > 0 },
          { id: "app", label: "Faça deploy de um app Docker", description: "Deploy em um clique a partir de 151 templates", link: "/apps", check: () => appCount > 0 },
          { id: "2fa", label: "Habilitar 2FA", description: "Proteja seu painel com autenticação de dois fatores", link: "/settings", check: () => twoFaEnabled },
          { id: "backup", label: "Configurar backups", description: "Configure backups para qualquer site", link: sitesList.length > 0 ? `/sites/${sitesList[0].id}` : "/sites", check: () => sitesList.some(s => !!s.backup_schedule) },
          { id: "diagnostics", label: "Executar diagnóstico", description: "Verifique a saúde do servidor e corrija problemas", link: "/diagnostics", check: () => true },
        ];
        ...
        if (isCollapsed) {
          return (
            <div className="mb-6 bg-dark-800 border border-dark-500 rounded-lg px-5 py-3 animate-fade-up flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-xs font-medium text-dark-200">Configuração: {completed}/{steps.length} concluído</span>
              ...
              <div className="flex items-center gap-3">
                <button onClick={toggleCollapse} className="text-rust-400 hover:text-rust-300 text-xs font-medium">Expandir</button>
                <button onClick={dismissOnboarding} className="text-dark-400 hover:text-dark-300 text-xs">Dispensar</button>
              </div>
            </div>
          );
        }
        ...
        return (
          <div className="mb-6 bg-dark-800 border border-dark-500 rounded-lg p-5 animate-fade-up">
            ...
            <p className="text-xs text-dark-300 mb-4">Complete estes passos para configurar seu servidor. <span className="text-dark-200">{completed}/{steps.length} concluídos</span></p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 stagger-children">
              {steps.map(step => (
                ...
                >
                  ...
                  {!step.check() && (
                    <span className="text-[10px] text-rust-500 font-medium">Começar &rarr;</span>
                  )}
                </Link>
              ))}
            </div>
        ...
        <>
          ...
          {isVisible("system_info") && <div className="hidden sm:grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-px bg-dark-600 border border-dark-500 rounded-lg overflow-hidden mb-6">
            {[
              ...
              ["Processador", system.cpu_model],
              ["Temperatura", system.cpu_temp != null ? `${system.cpu_temp.toFixed(0)}°C` : "N/D"],
              ["Processos", system.process_count.toLocaleString()],
            ].map(([label, value]) => (
              <div key={label} className="bg-dark-800 px-4 py-3 flex flex-col card-interactive">
                ...
                <span title={String(value)} className={`text-sm text-dark-50 font-medium truncate ${label === "Temperatura" && system.cpu_temp != null ? tempColor(system.cpu_temp) : ""}`}>{value}</span>
              </div>
            ))}
          </div>}
          ...
          {isVisible("metrics") && <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 stagger-children">
            {[
              { label: "CPU", pct: system.cpu_usage, type: "cpu" as const, detail: `${system.cpu_count} núcleos${system.load_avg_1 !== undefined ? ` · Carga ${system.load_avg_1?.toFixed(2)}` : ""}`,
              ...
              { label: "Memória", pct: system.mem_usage_pct, type: "mem" as const, detail: `${(system.mem_used_mb / 1024).toFixed(1)} / ${(system.mem_total_mb / 1024).toFixed(1)} GB${system.swap_total_mb > 0 ? ` · Swap ${(system.swap_used_mb / 1024).toFixed(1)}G` : ""}`,
              ...
              { label: "Disco", pct: system.disk_usage_pct, type: "disk" as const, detail: `${system.disk_used_gb.toFixed(0)} / ${system.disk_total_gb.toFixed(0)} GB · ${(system.disk_total_gb - system.disk_used_gb).toFixed(0)} GB livre${dockerDiskUsage ? ` · Imagens: ${dockerDiskUsage}` : ""}`,
              ...
              <div key={label} className="border border-dark-600 bg-dark-800 rounded-lg p-5 relative overflow-hidden shadow-lg shadow-black/10 elevation-1">
                ...
                <div className="relative text-center">
                  ...
                  {label === "Disco" && diskForecast !== null && diskForecast < 30 && (
                    <p className={`text-[10px] mt-1 font-medium ${diskForecast < 7 ? "text-danger-400" : "text-warn-400"}`}>
                      Disco cheio em ~{diskForecast}d na taxa atual
                    </p>
                  )}
                </div>
              </div>
          ...
          {isVisible("charts") && metricsHistory.length >= 2 && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 animate-fade-up">
              {[
                ...
                { label: "Memória", data: metricsHistory.map(p => p.mem), color: "var(--color-accent-500)" },
                { label: "Disco", data: metricsHistory.map(p => p.disk), color: "var(--color-warn-500)" },
          ...
          {isVisible("status_bar") && <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-px bg-dark-600 border border-dark-600 rounded-lg overflow-hidden mb-6 shadow-sm shadow-black/5 stagger-children">
            ...
            <div className="bg-dark-800 px-4 py-3 flex flex-col card-interactive">
              ...
              <span className="text-sm text-dark-50 font-medium">{sites.total}{sites.active > 0 && <span className="text-rust-400 ml-1 text-xs">({sites.active} ativos)</span>}</span>
            </div>
            <div className="bg-dark-800 px-4 py-3 flex flex-col card-interactive">
              <span className="text-[10px] text-dark-300 uppercase tracking-widest mb-1">Bancos</span>
            ...
            <div className="bg-dark-800 px-4 py-3 flex flex-col card-interactive">
              ...
              <span className="text-sm text-dark-50 font-medium">
                ...
                <span className="text-[10px] text-dark-400 ml-1">em execução</span>
              </span>
            </div>
            {intel && <>
              <div className={`px-4 py-3 flex flex-col card-interactive ${
                ...
                <span className="text-[10px] text-dark-300 uppercase tracking-widest mb-1">Saúde</span>
              ...
              <div className={`px-4 py-3 flex flex-col card-interactive ${
                ...
                <span className="text-[10px] text-dark-300 uppercase tracking-widest mb-1">Alertas</span>
                {intel.firing_alerts > 0
                  ? <span className="text-sm text-danger-400 font-bold">{intel.firing_alerts} ativos</span>
              ...
              <div className={`px-4 py-3 flex flex-col card-interactive ${
                ...
                <span className="text-[10px] text-dark-300 uppercase tracking-widest mb-1">Incidentes</span>
                {intel.open_incidents > 0
                  ? <span className="text-sm text-danger-400 font-bold">{intel.open_incidents} abertos</span>
                  : <span className="text-sm text-rust-400 font-medium">0</span>
                }
              </div>
              <div className={`px-4 py-3 flex flex-col card-interactive ${
                ...
                {intel.stale_backups > 0
                  ? <span className="text-sm text-warn-400 font-bold">{intel.stale_backups} desatualizados</span>
                  : <span className="text-sm text-rust-400 font-medium">atualizado</span>
                }
              </div>
            </>}
            <div className="bg-dark-800 px-4 py-3 flex flex-col card-interactive">
              <span className="text-[10px] text-dark-300 uppercase tracking-widest mb-1">Atualizações</span>
              {updateCount > 0
                ? <span className="text-sm text-warn-400 font-bold">{updateCount} disponíveis</span>
                : <span className="text-sm text-rust-400 font-medium">atualizado</span>
              }
            </div>
            {/* Disk I/O */}
            {diskIo && <>
              <div className="bg-dark-800 px-4 py-3 flex flex-col card-interactive">
                <span className="text-[10px] text-dark-300 uppercase tracking-widest mb-1">Leitura Disco</span>
                <span className="text-sm text-dark-50 font-medium font-mono">{formatRate(diskIo.read_bytes_sec)}</span>
              </div>
              <div className="bg-dark-800 px-4 py-3 flex flex-col card-interactive">
                <span className="text-[10px] text-dark-300 uppercase tracking-widest mb-1">Escrita Disco</span>
                <span className="text-sm text-dark-50 font-medium font-mono">{formatRate(diskIo.write_bytes_sec)}</span>
              </div>
            </>}
            {/* Feature #6: Bandwidth usage summary */}
            <div className="bg-dark-800 px-4 py-3 flex flex-col card-interactive">
              <span className="text-[10px] text-dark-300 uppercase tracking-widest mb-1">Banda</span>
              <span className="text-sm text-dark-50 font-medium">
            ...
            {/* Feature #9: Mail queue widget */}
            <div className="bg-dark-800 px-4 py-3 flex flex-col card-interactive">
              <span className="text-[10px] text-dark-300 uppercase tracking-widest mb-1">Fila de E-mail</span>
              <span className={`text-sm font-medium ${(mailQueue ?? 0) > 0 ? "text-warn-400" : "text-dark-50"}`}>
                {mailQueue ?? 0} <span className="text-[10px] text-dark-400">mensagens</span>
              </span>
            </div>
          </div>}
          ...
          {rebootRequired && (
            <div className="border border-warn-500/50 bg-warn-500/5 p-4 mb-6 flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm text-warn-400 font-bold">Reinicialização Necessária</p>
                <p className="text-xs text-dark-300 mt-1">Atualizações recentes (como uma nova versão do kernel) precisam de uma reinicialização para serem aplicadas.</p>
              </div>
          ...
          {isVisible("sites_grid") && sitesList.length > 0 && (
            <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mb-6">
              <div className="px-4 py-2.5 border-b border-dark-600 flex justify-between items-center">
                ...
                <Link to="/sites" className="text-[10px] text-rust-400 hover:text-rust-300">Gerenciar</Link>
              </div>
          ...
          {isVisible("activity") && recentActivity.length > 0 && (
            <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mb-6">
              <div className="px-4 py-2.5 border-b border-dark-600 flex justify-between items-center">
                ...
                <Link to="/logs" className="text-[10px] text-rust-400 hover:text-rust-300">Ver tudo</Link>
              </div>
          ...
          {isVisible("issues") && intel && (intel.top_issues.length > 0 || intel.ssl_countdowns.length > 0) && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
              {intel.top_issues.length > 0 && (
                <div className="border border-dark-500 bg-dark-800 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-xs text-dark-300 uppercase tracking-widest">Problemas Ativos</h3>
                    <Link to="/monitors" className="text-xs text-rust-400 hover:text-rust-300">Ver tudo</Link>
                  </div>
              ...
              {intel.ssl_countdowns.length > 0 && (
                <div className="border border-dark-500 bg-dark-800 rounded-lg p-4">
                  <h3 className="text-xs text-dark-300 uppercase tracking-widest mb-3">Certificados SSL</h3>
                  <div className="space-y-2">
                    {intel.ssl_countdowns.map((ssl, i) => (
                      <div key={i} className="flex items-center justify-between">
                        ...
                        }`}>{ssl.days_left}d restantes</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {intel && intel.recommendations && intel.recommendations.length > 0 && (
            <div className="border border-dark-500 bg-dark-800 rounded-lg overflow-hidden mb-6">
              <div className="px-4 py-2.5 border-b border-dark-600 flex justify-between items-center">
                <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Recomendações</h3>
                <span className="text-[10px] text-dark-400">{intel.recommendations.length} {intel.recommendations.length === 1 ? "item" : "itens"}</span>
              </div>
              <div className="divide-y divide-dark-600">
          ...
          {(bookmarks.length > 0 || showAddBookmark) && isVisible("bookmarks") && (
            <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mb-6">
              <div className="px-4 py-2.5 border-b border-dark-600 flex justify-between items-center">
                <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Links Rápidos</h3>
                <button onClick={() => setShowAddBookmark(!showAddBookmark)} className="text-[10px] text-rust-400 hover:text-rust-300">
                  {showAddBookmark ? "Cancelar" : "+ Adicionar"}
                </button>
              </div>
              {showAddBookmark && (
                <div className="px-4 py-3 border-b border-dark-600 flex flex-col sm:flex-row gap-2">
                  <input value={bmLabel} onChange={e => setBmLabel(e.target.value)} placeholder="Rótulo"
                  ...
                  <input value={bmUrl} onChange={e => setBmUrl(e.target.value)} placeholder="/sites ou https://..."
                    className="flex-1 px-2 py-1.5 bg-dark-900 border border-dark-500 rounded text-xs text-dark-100 placeholder-dark-400 min-h-[44px] sm:min-h-0" />
                  <button onClick={() => {
                    if (bmLabel && bmUrl) {
                  ...
                  }} className="px-3 py-1.5 bg-rust-500 text-white rounded text-xs min-h-[44px] sm:min-h-0">Adicionar</button>
                </div>
              )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {isVisible("network") && network.length > 0 && (
              <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-x-auto">
                <div className="px-5 py-3 border-b border-dark-600">
                  <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">I/O de Rede</h3>
                </div>
                    ...
                    {network.filter(n => n.rx_bytes > 0 || n.tx_bytes > 0).map((iface) => (
                      <tr key={iface.name} className="hover:bg-dark-700/30 transition-colors">
                        ...
                        <td className="px-5 py-2.5 text-right font-mono">
                          ...
                          {iface.rx_rate != null && (
                            <div className="text-[10px] text-dark-400 mt-0.5">{formatSize(iface.rx_bytes)} no total</div>
                          )}
                        </td>
                        <td className="px-5 py-2.5 text-right font-mono">
                          ...
                          {iface.tx_rate != null && (
                            <div className="text-[10px] text-dark-400 mt-0.5">{formatSize(iface.tx_bytes)} no total</div>
                          )}
                        </td>
                      </tr>
                    ))}
            ...
            {isVisible("processes") && processes.length > 0 && (
              <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-x-auto">
                <div className="px-5 py-3 border-b border-dark-600">
                  <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Processos Principais</h3>
                </div>
                <table className="w-full">
                  <thead>
                    <tr className="bg-dark-900">
                      <th className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-5 py-2">Processo</th>

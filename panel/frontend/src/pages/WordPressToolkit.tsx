...
export default function WordPressToolkit() {
  ...
  const fetchSites = useCallback(async () => {
    try {
      ...
      setError(e instanceof ApiError ? e.message : "Falha ao carregar sites WordPress");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleBulkUpdate = async (target: string) => {
    ...
    try {
      ...
      await fetchSites();
      setError("");
      setSuccess(`${selected.size} site(s) atualizado(s) com sucesso`);
      setSelected(new Set());
    } catch (e) {
      setError(e instanceof ApiError ? e.message : "Falha na atualização em massa");
    } finally {
      setUpdating(false);
    }
  };
  ...
  const handleScan = async (siteId: string) => {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : "Falha na verificação de vulnerabilidades");
    } finally {
      setScanning(null);
    }
  };
  ...
  const handleSecurityCheck = async (siteId: string) => {
    try {
      ...
      setError(e instanceof ApiError ? e.message : "Falha na verificação de segurança");
    }
  };
  ...
  const handleHarden = async (siteId: string, fixes: string[]) => {
    ...
    try {
      ...
      setSuccess("Hardening aplicado com sucesso");
      // Re-check after hardening
      await handleSecurityCheck(siteId);
    } catch (e) {
      setError(e instanceof ApiError ? e.message : "Falha no hardening");
    } finally {
      setHardening(null);
    }
  };
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex items-center justify-between gap-3 mb-6 pb-4 border-b border-dark-600">
        <div>
          ...
          <p className="text-sm text-dark-200 font-mono mt-0.5">
            Gerencie todos os sites WordPress em um só lugar
          </p>
        </div>
        <div className="flex items-center gap-3 text-xs font-mono">
          ...
          {updatesAvailable > 0 && (
            <span className="px-2 py-0.5 bg-warn-500/15 text-warn-400 rounded">
              {updatesAvailable} atualizaç{updatesAvailable !== 1 ? "ões" : "ão"}
            </span>
          )}
          {criticalVulns > 0 && (
            <span className="px-2 py-0.5 bg-danger-500/15 text-danger-400 rounded">
              {criticalVulns} crítica{criticalVulns !== 1 ? "s" : ""}
            </span>
          )}
        </div>
      </div>
      ...
      {sites.length === 0 && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-12 text-center">
          ...
          <h2 className="text-lg font-semibold text-dark-50 mb-2">Nenhum Site WordPress</h2>
          <p className="text-sm text-dark-200 mb-4">
            Nenhuma instalação WordPress detectada nos seus sites. Instale o WordPress em um site para vê-lo aqui.
          </p>
          <Link
            to="/sites"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-dark-800 rounded-lg border border-dark-500 p-4">
              ...
              <div className={`text-2xl font-bold font-mono ${criticalVulns > 0 ? "text-danger-400" : totalVulns > 0 ? "text-warn-400" : "text-dark-50"}`}>
                {totalVulns}
                {criticalVulns > 0 && (
                  <span className="text-sm text-danger-400 ml-2">({criticalVulns} crítica{criticalVulns !== 1 ? "s" : ""})</span>
                )}
              </div>
            </div>
          </div>
          ...
          {selected.size > 0 && (
            <div className="bg-dark-800 rounded-lg border border-rust-500/30 p-3 mb-4 flex items-center gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  {updating ? "Atualizando..." : "Atualizar Plugins"}
                </button>
                <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            <div className="col-span-full flex items-center gap-2 mb-1">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-xs text-dark-300 font-mono">Selecionar todos</span>
              </label>
            </div>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-dark-200 font-mono mb-3">
                      ...
                      {site.vulns === 0 && (
                        <span className="text-dark-400">Sem vulns</span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <Link
                ...
                {scanResults[site.site_id] && (
                  <div className="mt-3 pt-3 border-t border-dark-600">
                    <div className="text-xs font-mono text-dark-300 mb-2">Resultados da Última Verificação</div>
                    <div className="grid grid-cols-3 gap-2 text-xs font-mono">
                      ...
                      <div>
                        <span className="text-dark-400">Críticas: </span>
                      ...
                      <div>
                        <span className="text-dark-400">Altas: </span>
                        ...
                        <div className="mt-2 space-y-1 max-h-32 overflow-y-auto">
                          {scanResults[site.site_id].vulnerabilities.map((v, i) => (
                            ...
                            >
                              ...
                              />
                              <span className="text-dark-100 truncate">{v.title || v.name || "Desconhecido"}</span>
        ...
        <>
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm text-dark-200">
              Execute verificações de segurança nos seus sites WordPress e aplique correções de hardening.
            </p>
            <button
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {checks && (
                        ...
                        >
                          {failedChecks.length === 0
                            ? "Tudo OK"
                            : `${failedChecks.length} Problema${failedChecks.length !== 1 ? "s" : ""}`}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      {fixableIds.length > 0 && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-accent-600 text-white rounded-lg text-xs font-medium hover:bg-accent-700 disabled:opacity-50"
                        >
                          {hardening === site.site_id ? "Corrigindo..." : `Corrigir ${fixableIds.length} Problema${fixableIds.length !== 1 ? "s" : ""}`}
                        </button>
                      )}
                      <button
                  ...
                  {checks && checks.length === 0 && (
                    <div className="px-4 py-6 text-center text-sm text-dark-300">
                      Nenhuma verificação de segurança retornada para este site.
                    </div>
                  )}

                  {!checks && (
                    <div className="px-4 py-6 text-center text-sm text-dark-400">
                      Clique em "Verificar" para executar uma auditoria de segurança neste site.
                    </div>
                  )}
                </div>
              );

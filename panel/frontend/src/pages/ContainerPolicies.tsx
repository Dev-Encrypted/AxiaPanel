...
export default function ContainerPolicies() {
  ...
  const loadPolicies = async () => {
    try {
      ...
      setMessage({ text: e instanceof Error ? e.message : "Falha ao carregar policies", type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editUserId) {
        ...
        setMessage({ text: "Policy atualizada", type: "success" });
      } else {
        ...
        setMessage({ text: "Policy criada", type: "success" });
      }
      resetForm();
      loadPolicies();
    } catch (e) {
  ...
  const handleDelete = async (userId: string) => {
    try {
      ...
      setMessage({ text: "Policy removida", type: "success" });
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falhou", type: "error" });
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-dark-50">Policies de Isolamento de Container</h1>
          <p className="text-sm text-dark-300 mt-1">Gerencie limites por usuário de container, quotas e isolamento de network</p>
        </div>
        <button
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={resetForm}>
          <div className="bg-dark-800 rounded-lg border border-dark-500 p-6 w-full max-w-lg" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-semibold text-dark-50 mb-4">
              {editUserId ? "Editar Policy" : "Criar Policy"}
            </h2>

            <div className="space-y-4">
              {!editUserId && (
                <div>
                  <label className="block text-sm text-dark-200 mb-1">Usuário</label>
                  <select
                    value={formUserId}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-dark-200 mb-1">Máx. de Containers</label>
                  <input
                ...
                <div>
                  <label className="block text-sm text-dark-200 mb-1">Memória Máx. (MB)</label>
                  <input
              ...
              <div>
                <label className="block text-sm text-dark-200 mb-1">CPU Máx. (%)</label>
                <input
                  ...
                  className="w-full px-3 py-2 border border-dark-500 rounded-lg text-sm"
                />
                <p className="text-xs text-dark-400 mt-1">100% = 1 núcleo de CPU. 400% = 4 núcleos.</p>
              </div>

              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-dark-100">Isolamento de Network</p>
                  <p className="text-xs text-dark-400">Isolar containers do usuário em uma network Docker separada</p>
                </div>
                <button
                  type="button"
              ...
              <div>
                <label className="block text-sm text-dark-200 mb-1">Images Permitidas (separadas por vírgula, opcional)</label>
                <input
                  type="text"
                  value={allowedImages}
                  onChange={e => setAllowedImages(e.target.value)}
                  className="w-full px-3 py-2 border border-dark-500 rounded-lg text-sm"
                  placeholder="postgres, redis, nginx (ou * para todas)"
                />
              </div>
            </div>
            ...
            <div className="flex justify-end gap-2 mt-6">
              ...
              >
                {saving ? "Salvando..." : editUserId ? "Atualizar" : "Criar"}
              </button>
            </div>
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-dark-600 bg-dark-700/50">
              <th className="text-left px-4 py-3 text-xs font-medium text-dark-300 uppercase">Usuário</th>
              <th className="text-center px-4 py-3 text-xs font-medium text-dark-300 uppercase">Containers</th>
              <th className="text-center px-4 py-3 text-xs font-medium text-dark-300 uppercase">Memória</th>
              ...
              <th className="text-right px-4 py-3 text-xs font-medium text-dark-300 uppercase">Ações</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={6} className="text-center py-8 text-dark-400">Carregando...</td></tr>
            ) : policies.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-8 text-dark-400">Nenhuma policy configurada. Usuários têm acesso ilimitado a containers por padrão.</td></tr>
            ) : (
              policies.map(p => (
                <tr key={p.id} className="border-b border-dark-700 hover:bg-dark-700/30">
                  <td className="px-4 py-3">
                    <p className="text-dark-100">{p.user_email || "Desconhecido"}</p>
                  ...
                  <td className="text-center px-4 py-3">
                    {p.network_isolation ? (
                      <span className="inline-flex px-2 py-0.5 rounded text-xs bg-rust-500/20 text-rust-400">Isolada</span>
                    ) : (
                      <span className="text-dark-400 text-xs">Compartilhada</span>
                    )}
                  </td>
                  <td className="text-right px-4 py-3">
                    <button onClick={() => openEdit(p)} className="text-xs text-accent-400 hover:text-accent-300 mr-3">Editar</button>
                    <button onClick={() => handleDelete(p.user_id)} className="text-xs text-danger-400 hover:text-danger-300">Remover</button>
                  </td>
                </tr>
              ))
            )}
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 p-5">
        <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest mb-3">Como Funciona</h3>
        <div className="space-y-2 text-sm text-dark-200">
          <p><strong className="text-dark-100">Limites de container</strong> — Número máximo de containers Docker que um usuário pode fazer deploy.</p>
          <p><strong className="text-dark-100">Memória e CPU</strong> — Máximo de recursos por deployment de container.</p>
          <p><strong className="text-dark-100">Isolamento de network</strong> — Containers de cada usuário rodam em uma network Docker separada, prevenindo comunicação entre containers de usuários diferentes.</p>
          <p><strong className="text-dark-100">Images permitidas</strong> — Restringe quais images Docker um usuário pode fazer deploy. Deixe vazio para nenhuma restrição.</p>
          <p className="text-xs text-dark-400 mt-2">Usuários sem policy não têm limites de container aplicados.</p>
        </div>
      </div>
    </div>
  );

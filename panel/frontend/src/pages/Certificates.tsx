...
const STATUS_STYLES: Record<string, { bg: string; text: string; label: string }> = {
  expired: { bg: "bg-danger-500/15", text: "text-danger-400", label: "Expirado" },
  critical: { bg: "bg-danger-500/15", text: "text-danger-400", label: "Crítico" },
  warning: { bg: "bg-warn-500/15", text: "text-warn-400", label: "Aviso" },
...
export default function Certificates() {
  ...
  const handleRenew = async (cert: Certificate) => {
    ...
    try {
      ...
      setMessage({ text: `Certificado renovado para ${cert.domain}`, type: "success" });
      loadCerts();
    } catch (e) {
      setMessage({
        text: e instanceof Error ? e.message : "Falha ao renovar certificado",
        type: "error",
      });
    } finally {
      setRenewingId(null);
  ...
  const handleDelete = async (cert: Certificate) => {
    ...
    try {
      ...
      setMessage({ text: `Certificado excluído para ${cert.domain}`, type: "success" });
      loadCerts();
    } catch (e) {
      setMessage({
        text: e instanceof Error ? e.message : "Falha ao excluir certificado",
        type: "error",
      });
      setDeleteTarget(null);
    } finally {
  ...
  return (
    <div>
      <p className="text-sm text-dark-200 font-mono mb-4">
        {certs.length > 0
          ? `${certs.length} certificado${certs.length > 1 ? "s" : ""} SSL monitorado${certs.length > 1 ? "s" : ""}`
      ...
      {certs.length === 0 ? (
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-12 text-center">
          ...
          <p className="text-dark-200 text-sm">Nenhum certificado SSL. Habilite SSL nos seus sites para vê-los aqui.</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-dark-600">
                <th className="text-left px-4 py-3 text-xs font-medium text-dark-300 uppercase font-mono">Domínio</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-dark-300 uppercase font-mono">Expira em</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-dark-300 uppercase font-mono">Dias Restantes</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-dark-300 uppercase font-mono">Status</th>
                <th className="text-right px-4 py-3 text-xs font-medium text-dark-300 uppercase font-mono">Ações</th>
              </tr>
            </thead>
            <tbody>
              {certs.map((cert) => {
                ...
                return (
                  <tr key={cert.site_id} className="border-b border-dark-600/50 last:border-b-0">
                    ...
                    <td className="px-4 py-3 text-dark-200 font-mono">
                      {cert.expiry ? new Date(cert.expiry).toLocaleDateString() : "Desconhecido"}
                    </td>
                    <td className="px-4 py-3 font-mono">
                      <span className={cert.days_left <= 7 ? "text-danger-400 font-bold" : cert.days_left <= 30 ? "text-warn-400" : "text-dark-100"}>
                        {cert.days_left < 0 ? `${Math.abs(cert.days_left)}d em atraso` : `${cert.days_left}d`}
                      </span>
                    </td>
                      ...
                      {deleteTarget === cert.site_id ? (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          ...
                          >
                            ...
                            {renewingId === cert.site_id ? "Renovando..." : "Renovar"}
                          </button>
                          <button

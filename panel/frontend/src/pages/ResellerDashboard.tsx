    ...
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-dark-50 font-mono">
          {data.panel_name || "Painel de Revenda"}
        </h1>
        <p className="text-sm text-dark-300 mt-1">Gerencie seus usuários, sites e cotas de recursos</p>
      </div>

      {/* Quota Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Usuários", used: data.used_users, max: data.max_users, link: "/reseller/users" },
          { label: "Sites", used: data.used_sites, max: data.max_sites },
          { label: "Bancos de Dados", used: data.used_databases, max: data.max_databases },
        ].map((q) => (
          <div key={q.label} className="bg-dark-800 border border-dark-600 rounded-lg p-5">
            ...
            {q.link && (
              <Link to={q.link} className="text-xs text-rust-400 hover:text-rust-300 mt-2 inline-block">Gerenciar &rarr;</Link>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="bg-dark-800 border border-dark-600 rounded-lg p-5">
        <h2 className="text-lg font-bold text-dark-50 font-mono mb-3">Servidores Alocados ({data.server_count})</h2>
        {servers.length === 0 ? (
          <p className="text-sm text-dark-400">Nenhum servidor alocado ainda. Contate o administrador.</p>
        ) : (

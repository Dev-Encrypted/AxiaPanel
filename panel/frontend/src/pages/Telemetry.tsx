...
export default function Telemetry() {
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      flash(e instanceof Error ? e.message : "Falha ao salvar", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  const sendNow = async () => {
    ...
    try {
      ...
      flash("Eventos de telemetria sendo enviados", "success");
      setTimeout(loadEvents, 3000);
    } catch (e: unknown) {
      flash(e instanceof Error ? e.message : "Falha ao enviar", "error");
    } finally {
      setSending(false);
    }
  };
  ...
  const previewReport = async () => {
    try {
      ...
      flash(e instanceof Error ? e.message : "Falha ao gerar prévia", "error");
    }
  };
  ...
  const exportReport = async () => {
    try {
      ...
      flash(e instanceof Error ? e.message : "Falha ao exportar", "error");
    }
  };
  ...
  const executeClear = async () => {
    ...
    try {
      ...
      flash(`${data.deleted} eventos limpos`, "success");
      loadEvents();
      loadStats();
    } catch (e: unknown) {
      flash(e instanceof Error ? e.message : "Falha ao limpar", "error");
    } finally {
      setClearing(false);
    }
  };
  ...
  const checkUpdates = async () => {
    ...
    try {
      ...
      flash("Verificação de atualização iniciada", "success");
      setTimeout(loadConfig, 5000);
    } catch (e: unknown) {
      flash(e instanceof Error ? e.message : "Falha ao verificar", "error");
    } finally {
      setChecking(false);
    }
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 animate-fade-up">
      <div className="page-header">
        <div>
          <h1 className="page-header-title">Telemetria e Atualizações</h1>
          <p className="text-xs text-dark-400 mt-0.5">
            Relatórios de diagnóstico e gerenciamento de versão{config.current_version ? ` — v${config.current_version}` : ""}
          </p>
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <div className="bg-dark-800 border border-dark-600 rounded-lg p-3">
            <div className="text-xs text-dark-400 mb-1">Total de Eventos</div>
            <div className="text-xl font-mono font-bold text-dark-100">{stats.total}</div>
          </div>
          <div className="bg-dark-800 border border-dark-600 rounded-lg p-3">
            <div className="text-xs text-dark-400 mb-1">Não Enviados</div>
            <div className="text-xl font-mono font-bold text-amber-400">{stats.unsent}</div>
          </div>
          <div className="bg-dark-800 border border-dark-600 rounded-lg p-3">
            <div className="text-xs text-dark-400 mb-1">Últimas 24h</div>
          ...
          <div className="bg-dark-800 border border-dark-600 rounded-lg p-3">
            ...
            <div className={`text-sm font-medium ${enabled ? "text-emerald-400" : "text-dark-400"}`}>
              {enabled ? "Envio habilitado" : "Apenas local"}
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex gap-1 mb-4 border-b border-dark-600 pb-px">
        {(["events", "updates", "config"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "events" ? "Eventos" : t === "updates" ? "Atualizações" : "Configuração"}
          </button>
        ))}
      </div>
        ...
        <div>
          ...
          {pendingClear !== null && (
            <div className="border border-danger-500/30 bg-danger-500/5 rounded-lg px-4 py-3 mb-4 flex items-center justify-between">
              <span className="text-xs text-danger-400 font-mono">
                {pendingClear === "all" ? "Limpar TODOS os eventos de telemetria?" : `Limpar eventos com mais de ${pendingClear} dias?`}
              </span>
              <div className="flex items-center gap-2 shrink-0 ml-4">
                <button onClick={executeClear} className="px-3 py-1.5 bg-danger-500 text-white text-xs font-bold uppercase tracking-wider hover:bg-danger-400 transition-colors">Confirmar</button>
                <button onClick={() => setPendingClear(null)} className="px-3 py-1.5 bg-dark-600 text-dark-200 text-xs font-bold uppercase tracking-wider hover:bg-dark-500 transition-colors">Cancelar</button>
              </div>
            </div>
          )}
          ...
          {events.length === 0 ? (
            <div className="bg-dark-800 border border-dark-600 rounded-lg p-8 text-center text-dark-400 text-sm">
              Nenhum evento de telemetria registrado ainda. Eventos são capturados automaticamente quando ocorrem erros ou problemas.
            </div>
          ) : (
            <div className="bg-dark-800 border border-dark-600 rounded-lg overflow-hidden">
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-3 py-2 border-t border-dark-600">
                  <span className="text-xs text-dark-400">{total} eventos no total</span>
        ...
        <div className="space-y-4">
          <div className="bg-dark-800 border border-dark-600 rounded-lg p-4">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-medium text-dark-100">Status da Versão</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <div className="text-xs text-dark-400 mb-1">Versão Atual</div>
                <div className="text-sm font-mono text-dark-100">v{config.current_version}</div>
              </div>
              <div>
                <div className="text-xs text-dark-400 mb-1">Última Disponível</div>
                <div className={`text-sm font-mono ${config.update_available_version ? "text-rust-400" : "text-emerald-400"}`}>
                  {config.update_available_version ? `v${config.update_available_version}` : "Atualizado"}
                </div>
              </div>
              <div>
                <div className="text-xs text-dark-400 mb-1">Última Verificação</div>
                <div className="text-sm text-dark-300">
                  {config.update_checked_at ? formatDate(config.update_checked_at) : "Nunca"}
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-dark-800 border border-dark-600 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-medium text-dark-100">
                  Notas de Release — v{config.update_available_version}
                </h3>
                {safeHttpUrl(config.update_release_url) && (
                  <a href={safeHttpUrl(config.update_release_url)!} target="_blank" rel="noopener noreferrer"
...
systemctl is-active dockpanel-agent dockpanel-api`}</pre>
      ...
      {tab === "config" && (
        <div className="space-y-4">
          <div className="bg-dark-800 border border-dark-600 rounded-lg p-4">
            <h3 className="text-sm font-medium text-dark-100 mb-4">Configuração de Telemetria</h3>

            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm text-dark-200">Habilitar Telemetria Remota</div>
                  <div className="text-xs text-dark-400 mt-0.5">
                    Envia eventos de diagnóstico para um endpoint remoto para análise
                  </div>
                </div>
              ...
              <div>
                <label className="text-xs text-dark-400 mb-1 block">URL do Endpoint (HTTPS obrigatório)</label>
          ...
          <div className="bg-dark-800 border border-dark-600 rounded-lg p-4">
            <h3 className="text-sm font-medium text-dark-100 mb-3">Privacidade</h3>
            <div className="space-y-2 text-xs text-dark-300">
              <p>A telemetria é <strong className="text-dark-100">totalmente opcional</strong>. Quando desabilitada, todos os eventos são armazenados apenas localmente.</p>
              <p>Quando habilitada, o seguinte é coletado:</p>
            ...
            {config.telemetry_installation_id && (
              <div className="mt-3 pt-3 border-t border-dark-700">
                <span className="text-xs text-dark-400">ID da Instalação: </span>
          ...
          {stats && stats.by_category.length > 0 && (
            <div className="bg-dark-800 border border-dark-600 rounded-lg p-4">
              <h3 className="text-sm font-medium text-dark-100 mb-3">Eventos por Categoria</h3>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setShowPreview(false)}>
          <div className="bg-dark-800 border border-dark-600 rounded-xl max-w-3xl w-full max-h-[80vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-dark-600">
              <h3 className="text-sm font-medium text-dark-100">Prévia do Relatório de Telemetria</h3>
            ...
            <div className="px-4 py-2 border-t border-dark-600 text-xs text-dark-400">
              Isto é exatamente o que seria enviado. Todos os dados pessoais foram removidos.
            </div>
          </div>
        </div>
      )}

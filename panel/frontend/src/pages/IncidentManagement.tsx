...
export default function IncidentManagement() {
  ...
  const createIncident = async () => {
    try {
      ...
      setMessage({ text: "Incidente criado", type: "success" });
      setShowCreate(false);
  ...
  const postUpdate = async () => {
    ...
    try {
      ...
      setMessage({ text: "Atualização publicada", type: "success" });
  ...
  const deleteIncident = async (id: string) => {
    try {
      ...
      setMessage({ text: "Incidente excluído", type: "success" });
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha", type: "error" });
    }
  };
  ...
  const createComponent = async () => {
    try {
      ...
      setMessage({ text: "Componente criado", type: "success" });
  ...
  const deleteComponent = async (id: string) => {
    try {
      ...
      setMessage({ text: "Componente excluído", type: "success" });
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha", type: "error" });
    }
  };
  ...
  const tabs: { key: Tab; label: string }[] = [
    ...
    { key: "settings", label: "Configurações da Status Page" },
  ];
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-dark-600">
        <div>
          <h1 className="text-sm font-medium text-dark-300 uppercase font-mono tracking-widest">Gerenciamento de Incidentes</h1>
          <p className="text-sm text-dark-200 mt-1 font-mono">Gerencie incidentes, componentes e status page pública</p>
        </div>
        ...
        <div className="mb-4 px-4 py-3 rounded-lg border border-danger-500/30 bg-danger-500/5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 shrink-0 ml-4">
            <button onClick={() => { const { type, id } = pendingDelete; setPendingDelete(null); type === "incident" ? deleteIncident(id) : deleteComponent(id); }}
              className="px-3 py-1.5 bg-danger-500 text-white text-xs font-bold uppercase tracking-wider hover:bg-danger-400 transition-colors">Confirmar</button>
            <button onClick={() => setPendingDelete(null)}
              className="px-3 py-1.5 bg-dark-600 text-dark-200 text-xs font-bold uppercase tracking-wider hover:bg-dark-500 transition-colors">Cancelar</button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={() => setShowCreate(!showCreate)}
              ...
              {showCreate ? "Cancelar" : "Criar Incidente"}
            </button>
          </div>
          ...
          {showCreate && (
            <div className="bg-dark-800 rounded-lg border border-dark-500 p-5 space-y-3">
              <div>
                <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Título</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Severidade</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Status Inicial</label>
              ...
              <div>
                <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Descrição</label>
          ...
          {updateIncident && (
            <div className="bg-dark-800 rounded-lg border border-accent-500/30 p-5 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-medium text-accent-400 uppercase font-mono tracking-widest">Publicar Atualização</h3>
                <button onClick={() => setUpdateIncident(null)} className="text-dark-300 hover:text-dark-100 text-sm font-mono">Fechar</button>
              </div>
              {/* Timeline */}
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <div className="col-span-3">
                  <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Mensagem</label>
                  <input type="text" value={updateForm.message} onChange={e => setUpdateForm({ ...updateForm, message: e.target.value })}
                    placeholder="Qual é a última?"
                    className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
                </div>
              </div>
              <div className="flex justify-end">
                <button onClick={postUpdate}
                  className="px-4 py-2 bg-accent-500 text-white rounded-lg text-sm font-medium font-mono hover:bg-accent-600">Publicar Atualização</button>
              </div>
            </div>
          )}
          ...
          {incidents.length === 0 ? (
            <div className="p-12 text-center">
              <p className="text-dark-200 text-sm font-mono">Nenhum incidente</p>
              <p className="text-dark-300 text-xs mt-1 font-mono">Todos os sistemas operacionais</p>
            </div>
          ) : (
                ...
                <div key={inc.id} className="bg-dark-800 rounded-lg border border-dark-500 px-5 py-4 flex items-center justify-between hover:bg-dark-700/30 transition-colors">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-dark-300 font-mono mt-1">
                      Iniciado {timeAgo(inc.started_at)}
                      {inc.resolved_at && <> — resolvido {timeAgo(inc.resolved_at)}</>}
                    </p>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setPendingDelete({ type: "incident", id: inc.id, label: `Excluir incidente "${inc.title}"?` })}
        ...
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={() => setShowCompForm(!showCompForm)}
              className="px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium font-mono hover:bg-rust-600">
              {showCompForm ? "Cancelar" : "Adicionar Componente"}
            </button>
          </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={compForm.name} onChange={e => setCompForm({ ...compForm, name: e.target.value })}
                    placeholder="ex.: API, Website, Banco de Dados"
                    className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Grupo</label>
                  <input type="text" value={compForm.group_name} onChange={e => setCompForm({ ...compForm, group_name: e.target.value })}
                    placeholder="ex.: Serviços Principais"
                    className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Monitores Vinculados</label>
          ...
          {components.length === 0 ? (
            <div className="p-12 text-center">
              <p className="text-dark-200 text-sm font-mono">Nenhum componente configurado</p>
              <p className="text-dark-300 text-xs mt-1 font-mono">Adicione componentes para organizar sua status page</p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-dark-600">
                  {components.map(c => (
                    <tr key={c.id} className="hover:bg-dark-700/30 transition-colors">
                      ...
                      <td className="px-5 py-4 text-sm text-dark-200 font-mono">{c.monitor_ids?.length || 0} vinculados</td>
                      <td className="px-5 py-4">
                        <button onClick={() => setPendingDelete({ type: "component", id: c.id, label: `Excluir componente "${c.name}"?` })}
      ...
      {tab === "settings" && !config && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-8 text-center">
          <p className="text-dark-300 text-sm">Não foi possível carregar as configurações da status page. Tente atualizar.</p>
        </div>
      )}
      {tab === "settings" && config && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Título da Página</label>
              <input type="text" value={config.title} onChange={e => setConfig({ ...config, title: e.target.value })}
                className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Cor de Destaque</label>
          ...
          <div>
            <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">URL do Logo</label>
          ...
          <div className="flex gap-6 text-sm font-mono">
            <label className="flex items-center gap-2 text-dark-100">
              <input type="checkbox" checked={config.enabled} onChange={e => setConfig({ ...config, enabled: e.target.checked })} /> Habilitado
            </label>
            <label className="flex items-center gap-2 text-dark-100">
              <input type="checkbox" checked={config.show_subscribe} onChange={e => setConfig({ ...config, show_subscribe: e.target.checked })} /> Exibir Inscrição
            </label>
            <label className="flex items-center gap-2 text-dark-100">
              <input type="checkbox" checked={config.show_incident_history} onChange={e => setConfig({ ...config, show_incident_history: e.target.checked })} /> Exibir Histórico
            </label>
          </div>
          <div className="flex justify-end">
            <button onClick={saveConfig}
              className="px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium font-mono hover:bg-rust-600">Salvar Configurações</button>
          </div>
        </div>
      )}
    </div>

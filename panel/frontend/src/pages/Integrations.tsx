...
function WhmcsContent() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMsg({ text: "WHMCS configurado", type: "success" });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 p-5">
        <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest mb-4">Configuração do WHMCS</h3>
        <div className="space-y-3">
          <div>
            <label className="block text-sm text-dark-200 mb-1">URL da API</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm text-dark-200 mb-1">Identificador da API</label>
            ...
            <div>
              <label className="block text-sm text-dark-200 mb-1">Segredo da API</label>
              <input type="password" value={apiSecret} onChange={e => setApiSecret(e.target.value)} placeholder="Digite para atualizar" className="w-full px-3 py-2 text-sm border border-dark-500 rounded-lg" />
            </div>
          </div>
          <div className="flex items-center justify-between pt-2">
            <div>
              {config?.configured && config?.webhook_secret && (
                <p className="text-xs text-dark-400">URL do Webhook: <code className="text-dark-200">/api/whmcs/webhook?secret={config.webhook_secret}</code></p>
              )}
            </div>
      ...
      {services.length > 0 && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
          <div className="px-4 py-3 border-b border-dark-600">
            <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Mapeamentos de Serviço ({services.length})</h3>
          </div>
          <table className="w-full text-sm">
            <thead><tr className="border-b border-dark-600 bg-dark-700/50">
              ...
              <th className="text-left px-4 py-2 text-xs text-dark-300 uppercase">Plano</th>
...
function MigrationsContent() {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-dark-800 rounded-lg border border-dark-500 p-5">
        <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest mb-3">Migrações de Apps</h3>
        <p className="text-sm text-dark-200 mb-4">Migre containers Docker entre servidores. Inicie migrações a partir da página de Apps em um servidor específico.</p>
        {loading ? (
          ...
          <p className="text-dark-400 text-sm">Nenhuma migração ainda.</p>
        ) : (
          <table className="w-full text-sm">
            <thead><tr className="border-b border-dark-600 bg-dark-700/50">
              <th className="text-left px-4 py-2 text-xs text-dark-300 uppercase">Container</th>
              <th className="text-left px-4 py-2 text-xs text-dark-300 uppercase">Status</th>
              <th className="text-left px-4 py-2 text-xs text-dark-300 uppercase">Progresso</th>
            </tr></thead>
            <tbody>
              {migrations.map((m) => (
  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-dark-600">
        <div>
          <h1 className="text-sm font-medium text-dark-300 uppercase font-mono tracking-widest">Integrações</h1>
          <p className="text-sm text-dark-200 mt-1">Webhooks, extensões, faturamento e migrações</p>
        </div>
      </div>
      <div className="flex gap-6 mb-6 text-sm font-mono overflow-x-auto">
        ...
        <button onClick={() => setTab("extensions")} className={`whitespace-nowrap ${tab === "extensions" ? "border-b-2 border-rust-500 text-dark-50 pb-2" : "text-dark-300 hover:text-dark-100 pb-2"}`}>Extensões</button>
        ...
        <button onClick={() => setTab("migrations")} className={`whitespace-nowrap ${tab === "migrations" ? "border-b-2 border-rust-500 text-dark-50 pb-2" : "text-dark-300 hover:text-dark-100 pb-2"}`}>Migrações</button>
      </div>

...
export default function SiteDetail() {
  ...
  const executeConfirm = async () => {
    ...
    try {
      if (type === "staging_push") {
        ...
        setStagingMessage("Staging publicado em produção");
      } else if (type === "staging_delete") {
        await api.delete(`/sites/${id}/staging`);
        setStaging({ exists: false });
        setStagingMessage("Staging excluído");
      }
    } catch (e) {
      setStagingMessage(e instanceof Error ? e.message : "Falha na ação");
    } finally {
  ...
  const handleProvisionSSL = async () => {
    ...
    try {
      ...
      setSite(updated);
      setSslMessage("Certificado SSL provisionado com sucesso!");
    } catch (err) {
      setSslMessage(
        err instanceof Error ? err.message : "Falha no provisionamento SSL"
      );
    } finally {
  ...
  const handleProvisionDns01 = async (wildcard: boolean) => {
    ...
    try {
      ...
      setSslMessage(wildcard
        ? "Certificado SSL wildcard provisionado via DNS-01!"
        : "Certificado SSL provisionado via DNS-01!"
      );
    } catch (err) {
      setSslMessage(err instanceof Error ? err.message : "Falha no provisionamento DNS-01");
    } finally {
    ...
    <div className="p-6 lg:p-8">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <button disabled={checkingHealth} onClick={async () => {
              ...
              {checkingHealth ? "Verificando..." : "Verificar Saúde"}
            </button>
            {health && (
              <span className={`text-xs font-mono ${health.healthy ? "text-rust-400" : "text-danger-400"}`}>
                {health.status > 0 ? `${health.status}` : "Fora do ar"} · {health.response_time_ms}ms
              </span>
            )}
          </div>
        </div>
        <div>
          {confirmDelete ? (
            <div className="flex items-center gap-2">
              <span className="text-sm text-danger-400">Tem certeza?</span>
              <button
            ...
            <div className="flex items-center gap-2">
              ...
              <button disabled={toggling} onClick={async () => {
                ...
                } catch (e) { setError(e instanceof Error ? e.message : "Falha ao alternar"); }
                ...
                {toggling ? "..." : site?.enabled !== false ? "Desabilitar" : "Habilitar"}
              </button>
              {/* Rename */}
              ...
              {showCloneInput ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && cloneDomainValue) {
                        ...
                        api.post(`/sites/${id}/clone`, { domain: cloneDomainValue })
                          .then(() => setCloneMsg(`Site clonado em ${cloneDomainValue}`))
                          .catch((err) => setCloneMsg(err instanceof Error ? err.message : "Falha ao clonar"))
                    ...
                    placeholder="Clonar para domínio"
                  />
                  <button
                  ...
                  <button onClick={() => setShowCloneInput(false)} className="px-3 py-2 bg-dark-600 text-dark-200 rounded-lg text-sm font-medium">Cancelar</button>
                </div>
              ) : (
      ...
      {showRename && (
        <div className="mb-4 p-4 rounded-lg bg-dark-800 border border-dark-600">
          <h3 className="text-sm font-medium text-dark-100 mb-3">Renomear Domínio</h3>
          <div className="flex items-center gap-3">
            ...
            <button disabled={renaming || !newDomain || newDomain === site.domain} onClick={async () => {
              ...
              try {
                ...
                setRenameMsg(`Domínio renomeado para ${newDomain}`);
                setSite(s => s ? { ...s, domain: newDomain } : s);
                setShowRename(false);
              } catch (e) { setRenameMsg(e instanceof Error ? e.message : "Falha ao renomear"); }
          ...
          {renameMsg && (
            <p className={`mt-2 text-xs ${renameMsg.includes("renomeado") ? "text-rust-400" : "text-danger-400"}`}>{renameMsg}</p>
          )}
        </div>
      )}
      ...
      {cloneMsg && (
        <div className={`mb-4 px-4 py-3 rounded-lg text-sm ${
          cloneMsg.includes("clonado") ? "bg-rust-500/10 text-rust-400 border border-rust-500/20" : "bg-danger-500/10 text-danger-400 border border-danger-500/20"
        }`}>{cloneMsg}</div>
      )}
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
        <dl className="divide-y divide-dark-600">
          <div className="px-5 py-4 grid grid-cols-3">
            <dt className="text-sm font-medium text-dark-200">Domínio</dt>
          ...
          {site.proxy_port && (
            <div className="px-5 py-4 grid grid-cols-3">
              <dt className="text-sm font-medium text-dark-200">Porta do Proxy</dt>
              <dd className="text-sm text-dark-50 col-span-2 font-mono">{site.proxy_port}</dd>
            </div>
          )}
          {site.runtime === "php" && (
            <div className="px-5 py-4 grid grid-cols-3">
              <dt className="text-sm font-medium text-dark-200">Versão do PHP</dt>
              <dd className="text-sm col-span-2">
                <div className="flex items-center gap-3">
                  <select
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setSite(updated);
                        setPhpMessage(`Alternado para PHP ${newVersion}`);
                      } catch (err) {
                        setPhpMessage(err instanceof Error ? err.message : "Falha ao alternar");
                      } finally {
                  ...
                  {switchingPhp && (
                    <span className="text-xs text-dark-200">Alternando...</span>
                  )}
                  {phpMessage && (
                    <span className={`text-xs ${phpMessage.includes("Alternado") ? "text-rust-400" : "text-danger-400"}`}>
                      {phpMessage}
                    </span>
                  )}
                </div>
              ...
              {site.ssl_enabled ? (
                <div className="space-y-1">
                  <span className="inline-flex items-center gap-1 text-rust-400">
                    ...
                    Habilitado (Let's Encrypt)
                  </span>
                  {site.ssl_expiry && (
                    <p className="text-xs text-dark-200">
                      Expira em: {new Date(site.ssl_expiry).toLocaleDateString()}
                    </p>
                  )}
                </div>
              ) : (
                <div>
                  <div className="flex items-center gap-3">
                    <span className="text-dark-300">Não configurado</span>
                    {site.status === "active" && (
                      <>
                        <button
                        ...
                        >
                          {provisioning ? "Provisionando..." : "Let's Encrypt"}
                        </button>
                        <button
                          ...
                          title="Usa o challenge DNS-01 do Cloudflare — funciona atrás de CDN ou quando a porta 80 está bloqueada"
                        >
                          {dns01Loading ? "Provisionando..." : "DNS-01 (CF)"}
                        </button>
                        <button
                          ...
                          title="SSL wildcard (*.dominio) via Cloudflare DNS-01 — cobre todos os subdomínios"
                        >
                          {dns01Loading ? "..." : "SSL Wildcard"}
                        </button>
                        <button
                    ...
                    <div className="mt-3 space-y-3">
                      ...
                      <button disabled={uploadingSsl || !sslCert || !sslKey} onClick={async () => {
                        ...
                        try {
                          ...
                          setSslMessage("Certificado SSL personalizado instalado com sucesso!");
                          setSslCert(""); setSslKey(""); setShowSslUpload(false);
                          const updated = await api.get<Site>(`/sites/${id}`);
                          setSite(updated);
                        } catch (e) { setSslMessage(e instanceof Error ? e.message : "Falha no envio"); }
                        finally { setUploadingSsl(false); }
                      }} className="px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium hover:bg-rust-600 disabled:opacity-50">
                        {uploadingSsl ? "Instalando..." : "Instalar Certificado"}
                      </button>
                    </div>
                  )}
                </div>
          ...
          <div className="px-5 py-4 grid grid-cols-3">
            <dt className="text-sm font-medium text-dark-200">Criado em</dt>
            <dd className="text-sm text-dark-50 col-span-2">
              {formatDate(site.created_at)}
            </dd>
          </div>
          <div className="px-5 py-4 grid grid-cols-3">
            <dt className="text-sm font-medium text-dark-200">Última Atualização</dt>
            <dd className="text-sm text-dark-50 col-span-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mt-6">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-dark-50">Gerenciador de Arquivos</p>
              <p className="text-xs text-dark-200">Navegar e editar</p>
            </div>
          </div>
        </Link>
        <Link
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-dark-200">Acesso tipo SSH</p>
            </div>
          </div>
        </Link>
        <Link
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-dark-200">Criar e restaurar</p>
            </div>
          </div>
        </Link>
        <Link
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-dark-50">Crons</p>
              <p className="text-xs text-dark-200">Tarefas agendadas</p>
            </div>
          </div>
        </Link>
        <Link
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-dark-200">Push para deploy</p>
            </div>
          </div>
        </Link>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-dark-200">Gerenciar WP</p>
              </div>
            </div>
          </Link>
        )}
      ...
      {site.status === "active" && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
          <div className="px-5 py-4 border-b border-dark-600">
            <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Limites de Recursos</h2>
          </div>
          <div className="p-5 space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div>
                <label className="block text-xs font-medium text-dark-200 mb-1">Rate Limit (req/s por IP)</label>
                <input
                  type="number"
                  value={rateLimit}
              ...
              <div>
                <label className="block text-xs font-medium text-dark-200 mb-1">Upload Máximo (MB)</label>
                <input
                  type="number"
                  value={maxUpload}
              ...
              {site.runtime === "php" && (
                <>
                  <div>
                    <label className="block text-xs font-medium text-dark-200 mb-1">Memória PHP (MB)</label>
                    <input
                      type="number"
                      value={phpMemory}
                  ...
                  <div>
                    <label className="block text-xs font-medium text-dark-200 mb-1">Workers PHP</label>
                    <input
                      type="number"
                      value={phpWorkers}
            ...
            <div>
              <label className="block text-xs font-medium text-dark-200 mb-1">
                Diretivas Nginx Personalizadas <span className="text-dark-300 font-normal">(injetadas no bloco server)</span>
              </label>
              <textarea
                value={customNginx}
              ...
              <p className="text-[10px] text-dark-300 mt-1">
                A configuração é validada antes de aplicar. Diretivas inválidas serão rejeitadas.
              </p>
            </div>

            <div className="flex items-center justify-between">
              <p className="text-xs text-dark-300">
                {rateLimit ? `${rateLimit} req/s por IP` : "Sem rate limit"} · {maxUpload} MB de upload
                {site.runtime === "php" ? ` · ${phpMemory} MB de memória · ${phpWorkers} workers` : ""}
              </p>
              <div className="flex items-center gap-3">
                {limitsMessage && (
                  <span className={`text-xs ${limitsMessage.includes("salvos") ? "text-rust-400" : "text-danger-400"}`}>
                    {limitsMessage}
                  </span>
                )}
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setSite(updated);
                      setLimitsMessage("Limites salvos");
                    } catch (err) {
                      setLimitsMessage(err instanceof Error ? err.message : "Falha ao salvar");
                    } finally {
                ...
                >
                  {savingLimits ? "Salvando..." : "Aplicar Limites"}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
          <div className="px-5 py-4 border-b border-dark-600 flex items-center justify-between">
            ...
            <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
              ...
              {site.fastcgi_cache ? "Habilitado" : "Desabilitado"}
            </span>
          </div>
          <div className="p-5 space-y-4">
            <p className="text-sm text-dark-200">
              Faz cache de respostas PHP no nível do Nginx. Melhora drasticamente o tempo de carregamento para WordPress, Laravel e outras aplicações PHP. Usuários logados e requisições POST contornam o cache automaticamente.
            </p>
            <div className="flex items-center gap-3">
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    setCacheMsg(site.fastcgi_cache ? "Cache desabilitado" : "Cache habilitado");
                  } catch (e) {
                    setCacheMsg(e instanceof Error ? e.message : "Falha ao alternar");
                  } finally {
              ...
              >
                {cacheToggling ? "..." : site.fastcgi_cache ? "Desabilitar Cache" : "Habilitar Cache"}
              </button>
              {site.fastcgi_cache && (
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setCacheMsg("Cache limpo");
                    } catch (e) {
                      setCacheMsg(e instanceof Error ? e.message : "Falha ao limpar");
                    } finally {
                  ...
                  className="px-4 py-2 bg-dark-700 text-dark-100 rounded-lg text-sm font-medium hover:bg-dark-600 disabled:opacity-50 transition-colors"
                >
                  {cachePurging ? "Limpando..." : "Limpar Cache"}
                </button>
              )}
              {cacheMsg && (
                <span className={`text-xs ${cacheMsg.includes("Falha") || cacheMsg.includes("falha") ? "text-danger-400" : "text-rust-400"}`}>
                  {cacheMsg}
                </span>
              )}
            </div>
          ...
          <div className="px-5 py-4 border-b border-dark-600 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
                ...
                {site.redis_cache ? "Habilitado" : "Desabilitado"}
              </span>
            </div>
          </div>
          <div className="p-5 space-y-4">
            <p className="text-sm text-dark-200">
              Faz cache de queries de banco e objetos PHP no Redis para carregamentos drasticamente mais rápidos. Essencial para WordPress + WooCommerce. Cada site usa um database Redis isolado.
            </p>
            <div className="flex items-center gap-3">
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    setRedisMsg(site.redis_cache ? "Cache Redis desabilitado" : "Cache Redis habilitado");
                  } catch (e) {
                    setRedisMsg(e instanceof Error ? e.message : "Falha ao alternar");
                  } finally {
              ...
              >
                {redisToggling ? "..." : site.redis_cache ? "Desabilitar Redis" : "Habilitar Redis"}
              </button>
              {site.redis_cache && (
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setRedisMsg("Cache Redis limpo");
                    } catch (e) {
                      setRedisMsg(e instanceof Error ? e.message : "Falha ao limpar");
                    } finally {
                  ...
                  className="px-4 py-2 bg-dark-700 text-dark-100 rounded-lg text-sm font-medium hover:bg-dark-600 disabled:opacity-50 transition-colors"
                >
                  {redisPurging ? "Limpando..." : "Limpar Cache"}
                </button>
              )}
              {redisMsg && (
                <span className={`text-xs ${redisMsg.includes("Falha") || redisMsg.includes("falha") ? "text-danger-400" : "text-rust-400"}`}>
                  {redisMsg}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
              {site.waf_enabled && (
                <span className={`text-[10px] font-mono ${site.waf_mode === "prevention" ? "text-danger-400" : "text-warn-400"}`}>
                  {site.waf_mode === "prevention" ? "Bloqueando" : "Detectando"}
                </span>
              )}
              <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
                site.waf_enabled ? "bg-rust-500/10 text-rust-400" : "bg-dark-700 text-dark-300"
              }`}>
                {site.waf_enabled ? "Ativo" : "Desligado"}
              </span>
            </div>
          </div>
          <div className="p-5 space-y-4">
            <p className="text-sm text-dark-200">
              ModSecurity WAF com OWASP Core Rule Set. Protege contra SQL injection, XSS, RCE e outros ataques do OWASP Top 10. Comece em modo Detecção para monitorar sem bloquear.
            </p>
            <div className="flex items-center gap-3 flex-wrap">
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    setWafMsg(site.waf_enabled ? "WAF desabilitado" : "WAF habilitado (modo detecção)");
                  } catch (e) {
                    setWafMsg(e instanceof Error ? e.message : "Falha ao alternar");
                  } finally {
              ...
              >
                {wafToggling ? "..." : site.waf_enabled ? "Desabilitar WAF" : "Habilitar WAF"}
              </button>
              {site.waf_enabled && (
                <>
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setWafMsg(`Alternado para modo ${newMode}`);
                      } catch (e) {
                        setWafMsg(e instanceof Error ? e.message : "Falha ao alternar modo");
                      } finally {
                        setWafToggling(false);
                      }
                    }}
                  ...
                  >
                    {site.waf_mode === "prevention" ? "Alternar para Detecção" : "Alternar para Prevenção"}
                  </button>
                  <button
                    ...
                    className="px-4 py-2 bg-dark-700 text-dark-100 rounded-lg text-sm font-medium hover:bg-dark-600 disabled:opacity-50 transition-colors"
                  >
                    {wafLogsLoading ? "Carregando..." : "Ver Eventos"}
                  </button>
                </>
              )}
              {wafMsg && (
                <span className={`text-xs ${wafMsg.includes("Falha") || wafMsg.includes("falha") ? "text-danger-400" : "text-rust-400"}`}>
                  {wafMsg}
                </span>
              )}
            </div>
            {!wafLogsLoading && wafLogs !== null && wafLogs.length === 0 && (
              <p className="text-xs text-dark-300 mt-2">Nenhum evento WAF registrado. Eventos aparecem quando o firewall detecta requisições suspeitas.</p>
            )}
                ...
                <table className="w-full text-xs">
                  <thead className="sticky top-0 bg-dark-900">
                    <tr className="border-b border-dark-600">
                      <th className="text-left px-3 py-2 text-dark-300 font-mono">Hora</th>
                      ...
                      <th className="text-left px-3 py-2 text-dark-300 font-mono">Regra</th>
                      <th className="text-left px-3 py-2 text-dark-300 font-mono">Ação</th>
                    </tr>
                  </thead>
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
        <div className="px-5 py-4 border-b border-dark-600">
          <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Cabeçalhos de Segurança</h2>
          <p className="text-xs text-dark-200 mt-1">Cabeçalhos Content-Security-Policy e Permissions-Policy por site</p>
        </div>
        <div className="p-5 space-y-4">
          <div>
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              ...
              {cspPolicy && <button onClick={() => setCspPolicy("")} className="px-2 py-1 bg-dark-700 text-dark-200 rounded text-xs hover:bg-dark-600">Limpar</button>}
            </div>
          </div>
          <div>
          ...
          {headersMsg && <p className={`text-xs ${headersMsg.includes("Salvos") ? "text-rust-400" : "text-danger-400"}`}>{headersMsg}</p>}
          <button
            onClick={async () => {
              ...
              try {
                ...
                setHeadersMsg("Salvos — nginx recarregado");
              } catch (e) { setHeadersMsg(e instanceof Error ? e.message : "Falha"); }
              finally { setHeadersSaving(false); }
            }}
            disabled={headersSaving}
            className="px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium hover:bg-rust-600 disabled:opacity-50 transition-colors"
          >{headersSaving ? "Salvando..." : "Salvar Cabeçalhos"}</button>
        </div>
      </div>
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
        <div className="px-5 py-4 border-b border-dark-600 flex items-center justify-between">
          <div>
            <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Proteção contra Bots</h2>
            <p className="text-xs text-dark-200 mt-1">Bloqueia scrapers e bots maliciosos. Buscadores legítimos (Google, Bing) sempre são permitidos.</p>
          </div>
          <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium ${
            botMode !== "off" ? "bg-rust-500/10 text-rust-400" : "bg-dark-700 text-dark-300"
          }`}>
            {botMode === "off" ? "Desligado" : botMode === "rate-limit" ? "Rate Limit" : botMode === "challenge" ? "JS Challenge" : "Bloquear"}
          </span>
        </div>
        <div className="p-5 space-y-3">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[
              { mode: "off", label: "Desligado", desc: "Sem proteção contra bots" },
              { mode: "rate-limit", label: "Rate Limit", desc: "5 req/s por IP para todo o tráfego" },
              { mode: "challenge", label: "JS Challenge", desc: "Verificação do navegador via cookie" },
              { mode: "block", label: "Bloquear", desc: "Bloqueia user-agents de bots conhecidos" },
            ].map(opt => (
              <button
                key={opt.mode}
                onClick={async () => {
                  ...
                  try {
                    ...
                    setBotMsg(`Proteção contra bots: ${opt.label}`);
                  } catch (e) { setBotMsg(e instanceof Error ? e.message : "Falha"); }
      ...
      {site.status === "active" && (site.runtime === "php" || site.runtime === "static") && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
          <div className="px-5 py-4 border-b border-dark-600">
            <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Otimização de Imagens</h2>
          </div>
          <div className="p-5 space-y-4">
            <p className="text-sm text-dark-200">
              Converte imagens para WebP ou AVIF para tamanhos menores e carregamentos mais rápidos. Os arquivos originais são preservados — versões otimizadas são servidas automaticamente pelo nginx.
            </p>
            <div className="flex items-center gap-3 flex-wrap">
              <button
                ...
                className="px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium hover:bg-rust-600 disabled:opacity-50 transition-colors"
              >
                {imgOptRunning ? "Convertendo..." : "Converter para WebP"}
              </button>
              <button
                disabled={imgOptRunning}
                onClick={async () => {
                ...
                className="px-4 py-2 bg-dark-700 text-dark-100 rounded-lg text-sm font-medium hover:bg-dark-600 disabled:opacity-50 transition-colors"
              >
                {imgOptRunning ? "..." : "Converter para AVIF"}
              </button>
              {imgOptResult && imgOptResult.format !== "error" && (
                <span className="text-xs text-rust-400">
                  {imgOptResult.converted}/{imgOptResult.total_images} convertidas, {imgOptResult.saved_mb}MB economizados ({imgOptResult.format?.toUpperCase()})
                </span>
              )}
            </div>
          </div>
      ...
      {site.status === "active" && !site.parent_site_id && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
          <div className="px-5 py-4 border-b border-dark-600 flex items-center justify-between">
            <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Ambiente de Staging</h2>
          ...
          <div className="p-5">
            {staging?.exists && staging.site ? (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div>
                    <p className="text-xs font-medium text-dark-200 mb-1">Domínio</p>
                    <a
                  ...
                  <div>
                    <p className="text-xs font-medium text-dark-200 mb-1">Última Sincronização</p>
                    <p className="text-sm text-dark-50">
                      {staging.site.synced_at ? formatDate(staging.site.synced_at) : "Nunca"}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs font-medium text-dark-200 mb-1">Uso de Disco</p>
                    <p className="text-sm text-dark-50">
                  ...
                  <div>
                    <p className="text-xs font-medium text-dark-200 mb-1">Criado em</p>
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-dark-600">
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setStagingMessage("Arquivos sincronizados da produção");
                        fetchStaging();
                      } catch (e) {
                        setStagingMessage(e instanceof Error ? e.message : "Falha na sincronização");
                      } finally {
                        setStagingLoading(false);
                      }
                    }}
                  ...
                  >
                    {stagingLoading ? "Trabalhando..." : "Sincronizar da Produção"}
                  </button>
                  <button
                    disabled={stagingLoading}
                    onClick={() => setPendingConfirm({ type: "staging_push", label: "Isso vai sobrescrever os arquivos de produção com os do staging. Continuar?" })}
                  ...
                  <button
                    disabled={stagingLoading}
                    onClick={() => setPendingConfirm({ type: "staging_delete", label: "Excluir este ambiente de staging? Isso não pode ser desfeito." })}
                ...
                {stagingMessage && (
                  <p className={`text-xs ${stagingMessage.includes("Falha") || stagingMessage.includes("falha") ? "text-danger-400" : "text-rust-400"}`}>
                    {stagingMessage}
                  </p>
                )}
              </div>
              ...
              <div className="space-y-3">
                <p className="text-sm text-dark-200">
                  Crie uma cópia de staging deste site para testar mudanças antes de publicar.
                </p>
                {showStagingForm ? (
      ...
      {site.status === "active" && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
          ...
          >
            <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Regras de Redirect</h2>
            <div className="flex items-center gap-2">
              {redirects.length > 0 && (
                <span className="text-xs text-dark-200">{redirects.length} {redirects.length !== 1 ? "regras" : "regra"}</span>
              )}
          ...
          {showRedirects && (
            <div className="p-5 space-y-4">
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            loadRedirects();
                            setRedirectMsg("Redirect removido");
                          } catch (e) {
                            setRedirectMsg(e instanceof Error ? e.message : "Falha");
                          }
                        }}
              ...
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <label className="block text-xs font-medium text-dark-200 mb-1">Caminho de Origem</label>
                  <input
                    type="text"
                ...
                <div className="flex-1">
                  <label className="block text-xs font-medium text-dark-200 mb-1">URL de Destino</label>
                  <input
                    type="text"
                ...
                <div className="w-24">
                  <label className="block text-xs font-medium text-dark-200 mb-1">Tipo</label>
                  <select
                    value={redirectType}
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setRedirectTarget("");
                      loadRedirects();
                      setRedirectMsg("Redirect adicionado");
                    } catch (e) {
                      setRedirectMsg(e instanceof Error ? e.message : "Falha");
                    }
                  }}
              ...
              {redirectMsg && (
                <p className={`text-xs ${redirectMsg.includes("Falha") || redirectMsg.includes("falha") ? "text-danger-400" : "text-rust-400"}`}>
                  {redirectMsg}
                </p>
              )}
            </div>
      ...
      {site.status === "active" && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
          <button
            ...
            className="w-full px-5 py-4 border-b border-dark-600 flex items-center justify-between text-left"
          >
            <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Proteção por Senha</h2>
            <div className="flex items-center gap-2">
              {protectedPaths.length > 0 && (
                <span className="text-xs text-dark-200">{protectedPaths.length} {protectedPaths.length !== 1 ? "caminhos" : "caminho"}</span>
              )}
          ...
          {showProtect && (
            <div className="p-5 space-y-4">
              ...
              {protectedPaths.length > 0 && (
                <div className="space-y-2">
                  <div className="flex items-center gap-4 text-xs text-dark-200 mb-2">
                    <span>Caminhos protegidos: {protectedPaths.join(", ")}</span>
                    <span>Usuários: {protectedUsers.join(", ")}</span>
                  </div>
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            loadProtected();
                            setProtectMsg("Proteção removida");
                          } catch (e) {
                            setProtectMsg(e instanceof Error ? e.message : "Falha");
                          }
                        }}
                        className="p-1 text-dark-300 hover:text-danger-500 transition-colors"
                        title="Remover"
              ...
              <div className="flex items-end gap-3">
                <div>
                  <label className="block text-xs font-medium text-dark-200 mb-1">Caminho</label>
                  <input
                    type="text"
                    value={protectPath}
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-200 mb-1">Usuário</label>
                  <input
                    type="text"
                    value={protectUser}
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-200 mb-1">Senha</label>
                  <input
                    type="password"
                    value={protectPass}
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setProtectPass("");
                      loadProtected();
                      setProtectMsg("Proteção habilitada");
                    } catch (e) {
                      setProtectMsg(e instanceof Error ? e.message : "Falha");
                    }
                  }}
              ...
              {protectMsg && (
                <p className={`text-xs ${protectMsg.includes("Falha") || protectMsg.includes("falha") ? "text-danger-400" : "text-rust-400"}`}>
                  {protectMsg}
                </p>
              )}
            </div>
      ...
      {site.status === "active" && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
          <button
            ...
            className="w-full px-5 py-4 border-b border-dark-600 flex items-center justify-between text-left"
          >
            <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Aliases de Domínio</h2>
            <div className="flex items-center gap-2">
              {aliases.length > 0 && (
                <span className="text-xs text-dark-200">{aliases.length} {aliases.length !== 1 ? "aliases" : "alias"}</span>
              )}
          ...
          {showAliases && (
            <div className="p-5 space-y-4">
              <p className="text-xs text-dark-300">
                Domínios adicionais que servem o mesmo conteúdo. O DNS precisa apontar para este servidor.
              </p>
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            loadAliases();
                            setAliasMsg("Alias removido");
                          } catch (e) {
                            setAliasMsg(e instanceof Error ? e.message : "Falha");
                          }
                        }}
                        className="p-1 text-dark-300 hover:text-danger-500 transition-colors"
                        title="Remover"
              ...
              <div className="flex items-end gap-3">
                <div className="flex-1">
                  <label className="block text-xs font-medium text-dark-200 mb-1">Alias de Domínio</label>
                  <input
                    type="text"
                    value={newAlias}
              ...
              {aliasMsg && (
                <p className={`text-xs ${aliasMsg.includes("Falha") || aliasMsg.includes("falha") ? "text-danger-400" : "text-rust-400"}`}>
                  {aliasMsg}
                </p>
              )}
            </div>
      ...
      {site.status === "active" && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
          <button onClick={() => { setShowAccessLogs(!showAccessLogs); if (!showAccessLogs) loadLogs(logType === "php" ? "access" : logType as "access" | "error"); }}
            ...
            <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Logs de Acesso</h2>
          ...
          {showAccessLogs && (
            <div className="border-t border-dark-600">
              <div className="flex items-center justify-between px-5 py-2 bg-dark-900">
                <div className="flex gap-2">
                  <button onClick={() => { setLogType("access"); loadLogs("access"); }}
                    className={`px-2 py-1 rounded text-xs font-medium ${logType === "access" ? "bg-rust-500/15 text-rust-400" : "text-dark-300 hover:text-dark-100"}`}>Acesso</button>
                  <button onClick={() => { setLogType("error"); loadLogs("error"); }}
                    className={`px-2 py-1 rounded text-xs font-medium ${logType === "error" ? "bg-danger-400/15 text-danger-400" : "text-dark-300 hover:text-dark-100"}`}>Erro</button>
                  {site.runtime === "php" && (
                    <button onClick={() => { setLogType("php"); loadPhpErrors(); }}
                      className={`px-2 py-1 rounded text-xs font-medium ${logType === "php" ? "bg-warn-500/15 text-warn-400" : "text-dark-300 hover:text-dark-100"}`}>Erros PHP</button>
                  )}
                </div>
              ...
              <pre className="p-4 text-[11px] font-mono text-dark-200 bg-dark-950 max-h-80 overflow-y-auto overflow-x-auto whitespace-pre-wrap">
                {logsLoading ? "Carregando logs..." : (logType === "access" ? accessLogs : logType === "error" ? errorLogs : phpErrors) || "Nenhum log disponível"}
              </pre>
            </div>
          )}
        </div>
      ...
      {site.status === "active" && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
          <button onClick={() => { setShowStats(!showStats); if (!showStats) loadStats(); }}
            className="w-full px-5 py-4 flex items-center justify-between hover:bg-dark-700/30 transition-colors">
            <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Estatísticas de Tráfego</h2>
            ...
            <div className="border-t border-dark-600 p-5">
              <div className="grid grid-cols-3 gap-4 mb-4">
                <div className="text-center">
                  ...
                  <p className="text-xs text-dark-300">Requisições</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-dark-50">{stats.unique_ips.toLocaleString()}</p>
                  <p className="text-xs text-dark-300">IPs Únicos</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-dark-50">{stats.bandwidth_mb} MB</p>
                  <p className="text-xs text-dark-300">Banda</p>
                </div>
              </div>
              {stats.top_pages && stats.top_pages.length > 0 && (
                <div>
                  <h4 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest mb-2">Páginas Mais Acessadas</h4>
                  <div className="space-y-1">
              ...
              {stats.status_codes && Object.keys(stats.status_codes).length > 0 && (
                <div className="mt-4">
                  <h4 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest mb-2">Códigos de Status</h4>
      ...
      {site?.runtime === "php" && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
          <button onClick={() => { setShowPhpExts(!showPhpExts); if (!showPhpExts) loadPhpExtensions(); }}
            className="w-full px-5 py-4 flex items-center justify-between hover:bg-dark-700/30 transition-colors">
            <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Extensões PHP</h2>
          ...
          {showPhpExts && !phpExtsLoading && phpExts && (
            <div className="border-t border-dark-600 p-5">
              <div className="mb-4">
                <h4 className="text-xs text-dark-300 uppercase font-mono mb-2">Instaladas ({phpExts.installed.length})</h4>
                <div className="flex flex-wrap gap-1.5">
              ...
              <div>
                <h4 className="text-xs text-dark-300 uppercase font-mono mb-2">Disponíveis para instalar</h4>
                <div className="flex flex-wrap gap-1.5">
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mt-6">
        <button onClick={() => { setShowEnvVars(!showEnvVars); if (!showEnvVars) loadEnvVars(); }}
          className="w-full px-5 py-4 flex items-center justify-between hover:bg-dark-700/30 transition-colors">
          <h2 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Variáveis de Ambiente</h2>
        ...
        {showEnvVars && (
          <div className="border-t border-dark-600 p-5 space-y-2">
            {envVars.map((v, i) => (
              <div key={i} className="flex gap-2">
                ...
                <input value={v.value} onChange={e => { const n = [...envVars]; n[i] = { ...n[i], value: e.target.value }; setEnvVars(n); }}
                  placeholder="valor" className="flex-1 px-2 py-1.5 bg-dark-900 border border-dark-500 rounded text-xs font-mono text-dark-100 focus:ring-2 focus:ring-accent-500 outline-none" />
            ...
            <div className="flex gap-2 items-center">
              <button onClick={() => setEnvVars([...envVars, { key: "", value: "" }])} className="text-xs text-rust-400 hover:text-rust-300">+ Adicionar variável</button>
              <button disabled={savingEnv} onClick={async () => {
                setSavingEnv(true);
                setEnvMsg("");
                try {
                  await api.put(`/sites/${id}/env`, { vars: envVars.filter(v => v.key) });
                  setEnvMsg("Variáveis de ambiente salvas");
                } catch (e) { setEnvMsg(e instanceof Error ? e.message : "Falha ao salvar"); }
                finally { setSavingEnv(false); }
              }} className="px-3 py-1 bg-rust-500 text-white rounded text-xs font-medium hover:bg-rust-600 disabled:opacity-50">
                {savingEnv ? "Salvando..." : "Salvar"}
              </button>
              {envMsg && (
                <span className={`text-xs ${envMsg.includes("salvas") ? "text-rust-400" : "text-danger-400"}`}>{envMsg}</span>
              )}
            </div>
            <p className="text-xs text-dark-300">Salva no arquivo .env na raiz do site. Apps Node.js/Python são reiniciados automaticamente.</p>
          </div>
        )}
      </div>

...
export default function Backups() {
  ...
  const loadBackups = async () => {
    ...
    try {
      ...
      setMessage({
        text: e instanceof Error ? e.message : "Falha ao carregar backups",
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setMessage({
        text: e instanceof Error ? e.message : "Falha no backup",
        type: "error",
      });
      setCreating(false);
    }
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden mb-6">
        <div className="px-5 py-3 border-b border-dark-600 flex items-center justify-between">
          <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Backup Agendado</h3>
          {!showScheduleForm && (
            <button
          ...
          {showScheduleForm ? (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">Agendamento</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">Destino</label>
                  ...
                  {destinations.length === 0 && (
                    <p className="text-xs text-warn-500 mt-1">Adicione um destino em Configurações primeiro</p>
                  )}
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-dark-100 mb-1">Manter últimos N backups</label>
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => setShowScheduleForm(false)} className="px-3 py-1.5 text-sm text-dark-100 bg-dark-700 rounded-lg hover:bg-dark-600">Cancelar</button>
                {schedule && (
                  <button
                    onClick={async () => {
                      try {
                        ...
                        setMessage({ text: "Agendamento removido", type: "success" });
                      } catch (e) {
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      loadSchedule();
                      setShowScheduleForm(false);
                      setMessage({ text: "Agendamento salvo", type: "success" });
                    } catch (e) {
                      setMessage({ text: e instanceof Error ? e.message : "Falha", type: "error" });
                    } finally {
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-dark-50">
                  ...
                  Manter {schedule.retention_count} backups
                ...
                {schedule.last_run && (
                  <p className="text-xs text-dark-200 mt-1 font-mono">
                    Última execução: {new Date(schedule.last_run).toLocaleString()}
            ...
            <p className="text-sm text-dark-300 text-center">Nenhum backup agendado configurado</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-x-auto">
        {loading ? (
          <div className="p-8 text-center text-dark-300">Carregando...</div>
        ) : backups.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-dark-200 text-sm">Nenhum backup ainda</p>
            <p className="text-dark-300 text-xs mt-1">
              Crie seu primeiro backup para proteger os arquivos do site
            </p>
          </div>
        ) : (

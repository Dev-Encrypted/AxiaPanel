...
export default function WebhookGateway() {
  ...
  const createEndpoint = async () => {
    try {
      ...
      setMessage({ text: "Endpoint criado", type: "success" });
      loadEndpoints();
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha", type: "error" });
    }
  ...
  const createRoute = async () => {
    ...
    try {
      ...
      setMessage({ text: "Rota criada", type: "success" });
      loadRoutes(selectedEndpoint);
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha", type: "error" });
    }
  ...
  const replayDelivery = async (id: string) => {
    try {
      ...
      setMessage({ text: `Reenviado para ${r.replayed_to} rota(s)`, type: "success" });
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha", type: "error" });
    }
  };
  ...
  const tabs: { key: Tab; label: string }[] = [
    ...
    { key: "inspector", label: "Inspetor de Requisições" },
  ...
  if (loading) return <div className="p-8 text-center text-dark-300 font-mono">Carregando...</div>;
    ...
    <div className="p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-dark-600">
        <div>
          ...
          <p className="text-sm text-dark-200 mt-1 font-mono">Receba, inspecione, roteie e reenvie webhooks</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-4">
        <select value={selectedEndpoint || ""} onChange={e => setSelectedEndpoint(e.target.value || null)}
          ...
          {endpoints.map(ep => <option key={ep.id} value={ep.id}>{ep.name} ({ep.total_received} recebidos)</option>)}
          {endpoints.length === 0 && <option value="">Nenhum endpoint</option>}
        </select>
        {webhookUrl && (
        ...
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={() => setShowEpForm(!showEpForm)}
              ...
              {showEpForm ? "Cancelar" : "Novo Endpoint"}
            </button>
          </div>
          ...
          {showEpForm && (
            <div className="bg-dark-800 rounded-lg border border-dark-500 p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={epForm.name} onChange={e => setEpForm({ ...epForm, name: e.target.value })}
                    placeholder="ex.: GitHub Webhooks"
                    className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Verificação de Assinatura</label>
                  <select value={epForm.verify_mode} onChange={e => setEpForm({ ...epForm, verify_mode: e.target.value })}
                    ...
                    <option value="hmac_sha1">HMAC-SHA1 (GitHub Legado)</option>
                  </select>
                </div>
              </div>
              {epForm.verify_mode !== "none" && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Chave Secreta</label>
                    <input type="password" value={epForm.verify_secret} onChange={e => setEpForm({ ...epForm, verify_secret: e.target.value })}
                      className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Cabeçalho da Assinatura</label>
                    <input type="text" value={epForm.verify_header} onChange={e => setEpForm({ ...epForm, verify_header: e.target.value })}
                      placeholder="ex.: X-Hub-Signature-256"
                      className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
                  </div>
                </div>
              )}
              <div className="flex justify-end">
                <button onClick={createEndpoint} className="px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium font-mono hover:bg-rust-600">Criar</button>
              </div>
            </div>
          )}
          ...
          {endpoints.length === 0 ? (
            <div className="p-12 text-center">
              <p className="text-dark-200 text-sm font-mono">Nenhum endpoint de webhook</p>
              <p className="text-dark-300 text-xs mt-1 font-mono">Crie um endpoint para começar a receber webhooks</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-dark-900 border-b border-dark-500">
                    {["Nome", "Verificação", "Recebidos", "Último", ""].map(h => (
                ...
                <tbody className="divide-y divide-dark-600">
                  {endpoints.map(ep => (
                    <tr key={ep.id} className={`hover:bg-dark-700/30 transition-colors cursor-pointer ${selectedEndpoint === ep.id ? "bg-dark-700/20" : ""}`}
                      ...
                      <td className="px-5 py-4 text-xs text-dark-300 font-mono">{ep.last_received_at ? formatDate(ep.last_received_at) : "Nunca"}</td>
                      <td className="px-5 py-4">
                        <button onClick={e => { e.stopPropagation(); deleteEndpoint(ep.id); }}
                          className="px-3 py-1 bg-danger-500/10 text-danger-400 rounded-md text-xs font-medium font-mono hover:bg-danger-500/20">Excluir</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {tab === "inspector" && (
        <div className="space-y-4">
          {inspecting && (
            <div className="bg-dark-800 rounded-lg border border-accent-500/30 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-medium text-accent-400 uppercase font-mono tracking-widest">Detalhes da Requisição</h3>
                <button onClick={() => setInspecting(null)} className="text-dark-300 hover:text-dark-100 text-xs font-mono">Fechar</button>
              </div>
              <div className="grid grid-cols-3 gap-3 text-xs font-mono">
                <div><span className="text-dark-300">Método:</span> <span className="text-dark-50">{inspecting.method}</span></div>
                <div><span className="text-dark-300">IP:</span> <span className="text-dark-50">{inspecting.source_ip || "desconhecido"}</span></div>
                <div><span className="text-dark-300">Assinatura Válida:</span> <span className={inspecting.signature_valid === true ? "text-rust-400" : inspecting.signature_valid === false ? "text-danger-400" : "text-dark-300"}>{inspecting.signature_valid === null ? "N/A" : inspecting.signature_valid ? "Sim" : "Não"}</span></div>
              </div>
              <div>
              ...
              <div>
                ...
                <pre className="bg-dark-900 p-2 rounded text-xs font-mono text-dark-100 overflow-auto max-h-48">{inspecting.body ? JSON.stringify(JSON.parse(inspecting.body), null, 2) : "(vazio)"}</pre>
              </div>
              {inspecting.forwarded && (
                <div>
                  <p className="text-xs text-dark-300 font-mono mb-1">Resposta do Forward ({inspecting.forward_status}, {inspecting.forward_duration_ms}ms):</p>
                  <pre className="bg-dark-900 p-2 rounded text-xs font-mono text-dark-100 overflow-auto max-h-24">{inspecting.forward_response || "(vazio)"}</pre>
                </div>
              )}
            </div>
          )}
          ...
          {deliveries.length === 0 ? (
            <div className="p-12 text-center">
              <p className="text-dark-200 text-sm font-mono">Nenhuma entrega ainda</p>
              <p className="text-dark-300 text-xs mt-1 font-mono">Envie um webhook para a URL do seu endpoint para vê-lo aqui</p>
            </div>
          ) : (
            <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="bg-dark-900 border-b border-dark-500">
                    {["Quando", "IP", "Assinatura", "Encaminhado", "Status", "Duração", ""].map(h => (
                    ...
                    <tr key={d.id} className="hover:bg-dark-700/30 transition-colors cursor-pointer" onClick={() => setInspecting(d)}>
                      ...
                      <td className="px-4 py-3">
                        {d.signature_valid === null ? <span className="text-xs text-dark-400 font-mono">-</span> :
                          d.signature_valid ? <span className="text-xs text-rust-400 font-mono">Válida</span> :
                            <span className="text-xs text-danger-400 font-mono">Inválida</span>}
                      </td>
                      <td className="px-4 py-3 text-xs font-mono">{d.forwarded ? <span className="text-rust-400">Sim</span> : <span className="text-dark-400">Não</span>}</td>
                      ...
                      <td className="px-4 py-3">
                        <button onClick={e => { e.stopPropagation(); replayDelivery(d.id); }}
                          className="px-2 py-1 bg-accent-500/10 text-accent-400 rounded text-xs font-mono hover:bg-accent-500/20">Reenviar</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={() => setShowRouteForm(!showRouteForm)}
              className="px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium font-mono hover:bg-rust-600">
              {showRouteForm ? "Cancelar" : "Adicionar Rota"}
            </button>
          </div>
          ...
          {showRouteForm && (
            <div className="bg-dark-800 rounded-lg border border-dark-500 p-5 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Nome da Rota</label>
                  <input type="text" value={routeForm.name} onChange={e => setRouteForm({ ...routeForm, name: e.target.value })}
                    placeholder="ex.: Encaminhar para Slack"
                    className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">URL de Destino</label>
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">JSON Path do Filtro</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Valor do Filtro</label>
                  <input type="text" value={routeForm.filter_value} onChange={e => setRouteForm({ ...routeForm, filter_value: e.target.value })}
                    placeholder="push (opcional)"
                    className="w-full px-3 py-2 bg-dark-900 border border-dark-500 rounded-lg text-sm font-mono text-dark-50 outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1 font-mono">Tentativas</label>
              ...
              <div className="flex justify-end">
                <button onClick={createRoute} className="px-4 py-2 bg-rust-500 text-white rounded-lg text-sm font-medium font-mono hover:bg-rust-600">Criar Rota</button>
              </div>
            </div>
          )}
          ...
          {routes.length === 0 ? (
            <div className="p-12 text-center">
              <p className="text-dark-200 text-sm font-mono">Nenhuma rota configurada</p>
              <p className="text-dark-300 text-xs mt-1 font-mono">Adicione uma rota para encaminhar webhooks recebidos para um destino</p>
            </div>
          ) : (
            <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="bg-dark-900 border-b border-dark-500">
                    {["Nome", "Destino", "Filtro", "Tentativas", "Encaminhados", "Último Status", ""].map(h => (
                      <th key={h} className="text-left text-xs font-medium text-dark-200 uppercase font-mono tracking-widest px-4 py-3">{h}</th>
                    ))}
                  </tr>
                </thead>
                    ...
                    <tr key={r.id} className="hover:bg-dark-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <button onClick={() => deleteRoute(r.id)}
                          className="px-2 py-1 bg-danger-500/10 text-danger-400 rounded text-xs font-mono hover:bg-danger-500/20">Excluir</button>
                      </td>
                    </tr>
                  ))}
                </tbody>

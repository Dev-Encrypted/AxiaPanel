  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-dark-600">
        <div>
          <h1 className="text-sm font-medium text-dark-300 uppercase font-mono tracking-widest">Sistema</h1>
          <p className="text-sm text-dark-200 mt-1">Atualizações do sistema, serviços e monitoramento de saúde</p>
        </div>
      </div>
      <div className="flex gap-6 mb-6 text-sm font-mono overflow-x-auto">
        <button onClick={() => setTab("updates")} className={`whitespace-nowrap ${tab === "updates" ? "border-b-2 border-rust-500 text-dark-50 pb-2" : "text-dark-300 hover:text-dark-100 pb-2"}`}>Atualizações</button>
        <button onClick={() => setTab("health")} className={`whitespace-nowrap ${tab === "health" ? "border-b-2 border-rust-500 text-dark-50 pb-2" : "text-dark-300 hover:text-dark-100 pb-2"}`}>Saúde</button>
      </div>
...
function HealthContent() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (s) setSysInfo(s);
      if (g) setGpuInfo(g);
      if (!h && !s) setError("Falha ao buscar dados de saúde");
  ...
  if (health) {
    cards.push({
      label: "Status da API",
      value: health.status === "ok" ? "Saudável" : "Degradado",
      sub: health.service,
    ...
    cards.push({
      ...
      value: health.db === "unreachable" ? "Inacessível" : "Conectado",
  ...
  if (sysInfo) {
    ...
    cards.push({
      ...
      sub: `${sysInfo.cpu_count} cores \u00b7 carga ${sysInfo.load_avg_1.toFixed(2)}`,
    ...
    <div className="bg-dark-800 rounded-lg border border-dark-600 p-5">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-xs text-dark-400 font-mono uppercase tracking-wider">
          Histórico da GPU {gpuIndex} — {gpuName}
        </h3>
      ...
      <div className="flex justify-between mt-1">
        ...
        <span className="text-xs text-dark-400">
          Média: {(values.reduce((a, b) => a + b, 0) / values.length).toFixed(1)}{unit}
          {" · "}Pico: {Math.max(...values).toFixed(1)}{unit}
        </span>
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-4">
        <h2 className="text-sm font-medium text-dark-300 uppercase font-mono tracking-widest">Monitoramento de GPU</h2>
        <span className="text-xs text-dark-400">
          {gpuInfo.gpu_count} GPU{gpuInfo.gpu_count !== 1 ? "s" : ""}
          {gpuInfo.nvidia_toolkit_installed ? " \u00b7 Container Toolkit instalado" : ""}
        </span>
      </div>
        ...
        return (
          <div key={gpu.index} className="bg-dark-800 rounded-lg border border-dark-600 p-5 mb-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              <div>
                <p className="text-xs text-dark-400 font-mono uppercase tracking-wider mb-1">Utiliza\u00e7\u00e3o da GPU</p>
              ...
              <div>
                <p className="text-xs text-dark-400 font-mono uppercase tracking-wider mb-1">Temperatura</p>
                <p className={`text-lg font-semibold ${tempColor(gpu.temperature_c)}`}>
                  {gpu.temperature_c !== null ? `${gpu.temperature_c}\u00b0C` : "N/A"}
                </p>
              </div>
              <div>
                <p className="text-xs text-dark-400 font-mono uppercase tracking-wider mb-1">Energia</p>
            ...
            <div className="flex justify-between mt-1">
              <span className="text-xs text-dark-400">{vramPct.toFixed(1)}% VRAM usada</span>
              <span className="text-xs text-dark-400">{gpu.memory_free_mb.toLocaleString()} MB livres</span>
            </div>

            {gpu.fan_speed_pct !== null && (
              <p className="text-xs text-dark-400 mt-2">Ventoinha: {gpu.fan_speed_pct}%</p>
            )}
          </div>
        );
      })}
      ...
      {gpuInfo.processes.length > 0 && (
        <div className="bg-dark-800 rounded-lg border border-dark-600 p-5">
          <h3 className="text-xs text-dark-400 font-mono uppercase tracking-wider mb-3">Processos da GPU</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-dark-600">
                  <th className="text-left text-xs text-dark-400 font-mono uppercase pb-2">PID</th>
                  <th className="text-left text-xs text-dark-400 font-mono uppercase pb-2">Processo</th>
      ...
      <div className="flex items-center gap-2 mb-4 mt-6">
        <h2 className="text-sm font-medium text-dark-300 uppercase font-mono tracking-widest">Histórico da GPU</h2>
        <span className="text-xs text-dark-400">Últimas 24 horas</span>
      </div>
      {gpuInfo.gpus.map(gpu => (

...
export default function PublicStatusPage() {
  ...
  useEffect(() => {
    fetch("/api/status-page/public")
      .then(r => r.ok ? r.json() : Promise.reject("Página de status indisponível"))
      .then(setData)
      .catch(e => setError(typeof e === "string" ? e : "Falha ao carregar status"))
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-dark-950 flex items-center justify-center">
        <p className="text-dark-400 font-mono text-sm">{error || "Página de status não configurada"}</p>
      </div>
    );
  }
  ...
  for (const comp of data.components) {
    const group = comp.group || "Serviços";
      ...
      <div className="max-w-3xl mx-auto px-4 py-10">
        ...
        <div className={`rounded-lg border px-6 py-4 mb-8 text-center font-mono ${statusBgColors[data.overall_status] || statusBgColors.operational}`}>
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-lg font-medium">{statusLabels[data.overall_status] || "Todos os Sistemas Operacionais"}</span>
          </div>
        </div>
        ...
        {activeIncidents.length > 0 && (
          <div className="mb-8 space-y-3">
            <h2 className="text-xs font-mono uppercase tracking-widest text-dark-400 mb-3">Incidentes Ativos</h2>
        ...
        {data.show_incident_history && pastIncidents.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xs font-mono uppercase tracking-widest text-dark-400 mb-3">Incidentes Anteriores</h2>
                ...
                <div key={inc.id} className="bg-dark-900 rounded-lg border border-dark-700 p-4">
                  ...
                  {inc.resolved_at && (
                    <span className="text-xs text-dark-500 font-mono ml-2">
                      Resolvido {timeAgo(inc.resolved_at)}
                    </span>
                  )}
                </div>
              ))}
        ...
        {data.auto_incidents.length > 0 && (
          <div className="mb-8">
            <h2 className="text-xs font-mono uppercase tracking-widest text-dark-400 mb-3">Eventos Recentes de Indisponibilidade</h2>
              ...
              {data.auto_incidents.map(ai => (
                <div key={ai.id} className="bg-dark-900 rounded-lg border border-dark-700 px-4 py-3 flex items-center justify-between">
                  ...
                  <div className="text-xs text-dark-400 font-mono">
                    ...
                    {ai.resolved_at ? <span className="text-rust-400 ml-2">Resolvido</span> : <span className="text-danger-400 ml-2">Em Andamento</span>}
                  </div>
                </div>
              ))}
            </div>
        ...
        {data.show_subscribe && (
          <div className="bg-dark-900 rounded-lg border border-dark-700 p-5 text-center">
            {subscribed ? (
              <p className="text-sm text-rust-400 font-mono">Inscrito! Você receberá atualizações de incidentes por email.</p>
            ) : (
              <>
                <p className="text-sm text-dark-300 font-mono mb-3">Inscreva-se para receber atualizações de incidentes</p>

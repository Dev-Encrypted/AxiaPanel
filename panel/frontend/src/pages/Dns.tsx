...
export default function Dns() {
  ...
  const selectZone = async (zone: DnsZone) => {
    ...
    try {
      ...
      setMessage({ text: e instanceof Error ? e.message : "Falha ao carregar registros", type: "error" });
      setRecords([]);
    } finally {
  ...
  const handleAddZone = async () => {
    ...
    try {
      ...
      await loadZones();
      selectZone(zone);
      setMessage({ text: "Zona adicionada com sucesso", type: "success" });
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha ao adicionar zona", type: "error" });
    } finally {
      setSavingZone(false);
    }
  };
  ...
  const handleSaveRecord = async () => {
    ...
    try {
      if (editingRecord) {
        ...
        setMessage({ text: "Registro criado", type: "success" });
      }
      setShowRecordForm(false);
      selectZone(selectedZone);
    } catch (e) {
      setMessage({ text: e instanceof Error ? e.message : "Falha ao salvar", type: "error" });
    } finally {
  ...
  const executeConfirm = async () => {
    ...
    if (type === "delete-zone" && data) {
      ...
      try {
        ...
        selectZone(selectedZone);
        setMessage({ text: "Registro excluído", type: "success" });
      } catch (e) { setMessage({ text: e instanceof Error ? e.message : "Falha ao excluir", type: "error" }); }
      ...
      setMessage({ text: `${deleted} registro(s) excluído(s)`, type: "success" });
      setSelectedRecords(new Set());
  ...
  const handleImport = async () => {
    ...
    setMessage({ text: `${created} registro${created !== 1 ? 's' : ''} importado${created !== 1 ? 's' : ''}${errors.length ? `. ${errors.length} erro${errors.length !== 1 ? 's' : ''}` : ''}`, type: errors.length ? 'error' : 'success' });
  ...
  const checkPropagation = async (name: string, type: string) => {
    ...
    try {
      ...
      setMessage({ text: 'Falha na verificação de propagação', type: 'error' });
  ...
  const applyTemplate = async (template: string) => {
    ...
    setMessage({ text: `${created} registro${created !== 1 ? 's' : ''} criado${created !== 1 ? 's' : ''} a partir do template`, type: 'success' });
  ...
  const runHealthCheck = async () => {
    ...
    try {
      ...
      setMessage({ text: "Falha na verificação de saúde", type: "error" });
    } finally {
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-header-title">Gerenciamento de DNS</h1>
          <p className="page-header-subtitle">Gerencie registros DNS via Cloudflare ou PowerDNS</p>
        </div>
      ...
      {pendingConfirm && (
        <div className="mb-4 border border-danger-500/30 bg-danger-500/5 rounded-lg px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 shrink-0 ml-4">
            <button onClick={executeConfirm} className="px-3 py-1.5 bg-danger-500 text-white text-xs font-bold uppercase tracking-wider hover:bg-danger-400 transition-colors">Confirmar</button>
            <button onClick={() => setPendingConfirm(null)} className="px-3 py-1.5 bg-dark-600 text-dark-200 text-xs font-bold uppercase tracking-wider hover:bg-dark-500 transition-colors">Cancelar</button>
          </div>
        </div>
      )}
      ...
      {showAddZone && (
        <div className="bg-dark-800 rounded-lg border border-dark-500 p-5 mb-6 space-y-4">
          {/* Provider selector */}
          <div>
            <label className="block text-xs font-medium text-dark-100 mb-2">Provedor DNS</label>
            <div className="flex gap-3">
              ...
              >
                ...
                <span className="block text-xs text-dark-300 mt-0.5">Proxy para a API DNS do Cloudflare</span>
              </button>
              <button
              ...
              >
                ...
                <span className="block text-xs text-dark-300 mt-0.5">DNS autoritativo self-hosted</span>
              </button>
              <div className="flex-1 px-4 py-3 border cursor-not-allowed opacity-60 border-dark-500 bg-dark-900">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-1.5 py-0.5 bg-dark-700 text-dark-300 rounded text-[10px] font-medium">Em Breve</span>
                </div>
                <p className="text-xs text-dark-300 mt-0.5">Gerenciamento DNS do Amazon Route53</p>
              </div>
            </div>
          </div>
          ...
          {zoneProvider === "cloudflare" ? (
            <>
              <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Conectar Zona Cloudflare</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">Domínio</label>
                  <input type="text" value={zoneDomain} onChange={(e) => setZoneDomain(e.target.value)} placeholder="exemplo.com" className="w-full px-3 py-2 border border-dark-500 rounded-lg text-sm focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none" />
                  <p className="text-[11px] text-dark-300 mt-1">Seu nome de domínio, ex.: exemplo.com</p>
                </div>
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">ID da Zona Cloudflare</label>
              ...
              <div>
                <label className="block text-xs font-medium text-dark-100 mb-2">Método de Autenticação</label>
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm text-dark-100">Token de API</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm text-dark-100">Chave de API Global</span>
                  </label>
                </div>
              </div>
              <div className={`grid grid-cols-1 ${authMethod === "key" ? "md:grid-cols-2" : ""} gap-4`}>
                {authMethod === "key" && (
                  <div>
                    <label className="block text-xs font-medium text-dark-100 mb-1">E-mail do Cloudflare</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-dark-100 mb-1">{authMethod === "token" ? "Token de API" : "Chave de API Global"}</label>
                  <input type="password" value={zoneToken} onChange={(e) => setZoneToken(e.target.value)} placeholder={authMethod === "token" ? "Token com permissão de edição de DNS" : "Chave de API Global do dashboard CF"} className="w-full px-3 py-2 border border-dark-500 rounded-lg text-sm focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none" />
                </div>
              </div>
              <p className="text-xs text-dark-300">
                {authMethod === "token"
                  ? "Crie um token de API em Cloudflare Dashboard \u2192 Meu Perfil \u2192 API Tokens \u2192 Create Token \u2192 template \"Edit zone DNS\"."
                  : "Encontre sua Chave de API Global em Cloudflare Dashboard \u2192 Meu Perfil \u2192 API Tokens \u2192 Global API Key \u2192 View."}
              </p>
              <div className="flex justify-end">
                ...
                >
                  ...
                  {savingZone ? "Verificando..." : "Conectar Zona"}
                </button>
              </div>
            </>
          ) : (
            <>
              <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Criar Zona PowerDNS</h3>
              <div>
                <label className="block text-xs font-medium text-dark-100 mb-1">Domínio</label>
                <input type="text" value={zoneDomain} onChange={(e) => setZoneDomain(e.target.value)} placeholder="exemplo.com" className="w-full px-3 py-2 border border-dark-500 rounded-lg text-sm focus:ring-2 focus:ring-accent-500 focus:border-accent-500 outline-none" />
                <p className="text-[11px] text-dark-300 mt-1">Seu nome de domínio, ex.: exemplo.com</p>
              ...
              <div className="flex items-start gap-2 bg-accent-500/5 border border-accent-500/20 px-3 py-2.5">
                ...
                <p className="text-xs text-dark-200">
                  O PowerDNS precisa estar instalado e a URL da API + chave configuradas em{" "}
                  <a href="/settings" className="text-accent-400 hover:text-accent-300 underline underline-offset-2">Configurações</a>{" "}
                  antes de criar zonas. Consulte o guia de configuração lá para os passos de instalação.
                </p>
              </div>
              <div className="flex justify-end">
                <button
          ...
          <div className="md:w-60 shrink-0">
            <div className="bg-dark-800 rounded-lg border border-dark-500 overflow-hidden">
              <div className="px-4 py-3 border-b border-dark-600">
                <h3 className="text-xs font-medium text-dark-300 uppercase font-mono tracking-widest">Zonas</h3>
              </div>
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      {zoneCounts[z.id] !== undefined && (
                        <span className="text-[10px] text-dark-300 mt-0.5 block">{zoneCounts[z.id]} registro{zoneCounts[z.id] !== 1 ? "s" : ""}</span>
                      )}
                    </div>
                    <button
          ...
          {!selectedZone ? (
            !showAddZone && (
              <div className="bg-dark-800 rounded-lg border border-dark-500 p-12 text-center">
                <p className="text-dark-300">{zones.length === 0 ? "Adicione uma zona para começar" : "Selecione uma zona"}</p>
              </div>
            )
          ) : loadingRecords ? (
            <div className="bg-dark-800 rounded-lg border border-dark-500 p-6 animate-pulse">
            ...
            <div className="bg-dark-800 rounded-lg border border-dark-500">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {dnssec && dnssec.supported && (
                        <span className={`px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded ${
                          ...
                          {dnssec.active ? "DNSSEC" : "Sem DNSSEC"}
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-dark-200">{records.length} registro{records.length !== 1 ? "s" : ""}</p>
                  </div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <button
                    ...
                    <button onClick={() => setShowImport(!showImport)}
                      className="px-3 py-1.5 bg-dark-700 text-dark-200 hover:bg-dark-600 hover:text-dark-100 border border-dark-600 rounded-lg text-xs font-medium transition-colors">
                      {showImport ? "Cancelar Importação" : "Importar"}
                    </button>
                    ...
                    <button onClick={runHealthCheck} disabled={checkingHealth}
                      className="px-3 py-1.5 bg-dark-700 text-dark-200 hover:bg-dark-600 hover:text-dark-100 border border-dark-600 rounded-lg text-xs font-medium disabled:opacity-50 transition-colors">
                      {checkingHealth ? "Verificando..." : "Verificação de Saúde"}
                    </button>
                    <button onClick={loadChangelog} disabled={loadingChangelog}
                      className="px-3 py-1.5 bg-dark-700 text-dark-200 hover:bg-dark-600 hover:text-dark-100 border border-dark-600 rounded-lg text-xs font-medium disabled:opacity-50 transition-colors">
                      {loadingChangelog ? "Carregando..." : "Histórico"}
                    </button>
                    {selectedRecords.size > 0 && (
                      <button disabled={bulkDeleting} onClick={() => {
                        setPendingConfirm({ type: "bulk-delete", data: null, label: `Excluir ${selectedRecords.size} registros selecionados?` });
                        ...
                        {bulkDeleting ? "Excluindo..." : `Excluir ${selectedRecords.size} Selecionados`}
                      </button>
                    )}
                    <button
              ...
              {showRecordForm && (
                <div className="px-5 py-4 bg-dark-900 border-b border-dark-500 space-y-3">
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    ...
                    <div>
                      <label className="block text-xs font-medium text-dark-200 mb-1">Nome</label>
                      ...
                      <p className="text-[10px] text-dark-300 mt-0.5">Subdomínio ou @ para raiz</p>
                    </div>
                    <div className="col-span-2 sm:col-span-1">
                      <label className="block text-xs font-medium text-dark-200 mb-1">Conteúdo</label>
                      <input type="text" value={recContent} onChange={(e) => setRecContent(e.target.value)} placeholder={recType === "A" ? "1.2.3.4" : recType === "CNAME" ? "destino.com" : ""} className="w-full px-2 py-1.5 border border-dark-500 rounded-md text-sm focus:ring-2 focus:ring-accent-500 outline-none" />
                      <p className="text-[10px] text-dark-300 mt-0.5">Endereço IP ou destino</p>
                    </div>
                    <div>
                      ...
                      {recTtl === "custom" && (
                        <input type="number" value={customTtl} onChange={e => setCustomTtl(e.target.value)}
                          placeholder="TTL em segundos" min="60"
                    ...
                    {recType === "MX" && (
                      <div>
                        <label className="block text-xs font-medium text-dark-200 mb-1">Prioridade</label>
                  ...
                  {isCloudflare && ["A", "AAAA", "CNAME"].includes(recType) && (
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      <span className="text-sm text-dark-100">Proxy via Cloudflare (nuvem laranja)</span>
                    </label>
                  )}
                  <div className="flex items-center gap-2">
                    <button
                    ...
                    >
                      ...
                      {savingRecord ? "Salvando..." : editingRecord ? "Atualizar" : "Criar"}
                    </button>
                    <button
              ...
              {showImport && (
                <div className="px-5 py-4 bg-dark-900 border-b border-dark-500 space-y-3">
                  <p className="text-xs text-dark-300">Cole o conteúdo do arquivo de zona BIND (um registro por linha):</p>
              ...
              {showTemplates && (
                <div className="px-5 py-4 bg-dark-900 border-b border-dark-500">
                  <p className="text-xs text-dark-300 mb-3">Aplicar um template de registro:</p>
              ...
              <div className="px-4 sm:px-5 py-3 border-b border-dark-600 flex gap-2">
                <input value={recordSearch} onChange={e => setRecordSearch(e.target.value)}
                  placeholder="Buscar registros..." className="flex-1 px-3 py-1.5 border border-dark-500 rounded text-sm bg-dark-900 text-dark-100 focus:ring-2 focus:ring-accent-500 outline-none" />
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-dark-900 text-left">
                      ...
                      <th className="px-4 py-2.5 text-xs font-medium text-dark-200 uppercase font-mono tracking-widest w-20">Tipo</th>
                      <th className="px-4 py-2.5 text-xs font-medium text-dark-200 uppercase font-mono tracking-widest">Nome</th>
                      <th className="px-4 py-2.5 text-xs font-medium text-dark-200 uppercase font-mono tracking-widest">Conteúdo</th>
                      ...
                      <th className="px-4 py-2.5 text-xs font-medium text-dark-200 uppercase font-mono tracking-widest w-20">Ações</th>
                    </tr>
                  </thead>
                    ...
                    {filteredRecords.map((r) => (
                      <tr key={r.id} className="table-row-hover">
                        ...
                        {isCloudflare && (
                          <td className="px-4 py-2.5">
                            {r.proxied !== undefined && (
                              <span className={`inline-block w-3 h-3 rounded-full ${r.proxied ? "bg-warn-400" : "bg-dark-500"}`} title={r.proxied ? "Com proxy" : "Apenas DNS"} />
                            )}
                          </td>
                        )}
                        <td className="px-4 py-2.5">
                          <div className="flex items-center gap-1">
                            <button onClick={() => checkPropagation(r.name, r.type)} disabled={checkingProp === r.name}
                              title="Verificar propagação" className="p-1 text-dark-300 hover:text-dark-100 disabled:opacity-50">
                              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418" />
                              </svg>
                            </button>
                            <button onClick={() => openRecordForm(r)} className="p-1 text-dark-300 hover:text-accent-400" title="Editar">
                              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125" />
                              </svg>
                            </button>
                            <button onClick={() => handleDeleteRecord(r)} className="p-1 text-dark-300 hover:text-danger-500" title="Excluir">
                              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
                              </svg>
                            </button>
                    ...
                    {filteredRecords.length === 0 && (
                      <tr>
                        <td colSpan={isCloudflare ? 7 : 6} className="px-4 py-8 text-center text-dark-300 text-sm">
                          {records.length === 0 ? "Nenhum registro DNS encontrado" : "Nenhum registro corresponde à sua busca"}
                        </td>
                      </tr>
                    )}
                  </tbody>
                  ...
                  <div key={r.id} className="px-4 py-3 space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-0.5 shrink-0 ml-2">
                        ...
                        <button onClick={() => checkPropagation(r.name, r.type)} disabled={checkingProp === r.name}
                          className="p-1.5 text-dark-300 hover:text-dark-100 disabled:opacity-50" title="Verificar propagação">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418" />
                          </svg>
                        </button>
                ...
                <div className="px-4 sm:px-5 py-4 border-t border-dark-600">
                  <div className="bg-dark-900 rounded-lg border border-dark-500 p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button onClick={() => setPropagation(null)} className="text-xs text-dark-300 hover:text-dark-100">Fechar</button>
                    </div>
                ...
                <div className="px-4 sm:px-5 py-4 border-t border-dark-600">
                  <div className="bg-dark-900 rounded-lg border border-dark-500 p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button onClick={() => setHealthCheck(null)} className="text-xs text-dark-300 hover:text-dark-100">Fechar</button>
                    </div>
                    <div className="space-y-2">
                ...
                <div className="px-4 sm:px-5 py-4 border-t border-dark-600">
                  <div className="bg-dark-900 rounded-lg border border-dark-500 p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button onClick={() => setShowChangelog(false)} className="text-xs text-dark-300 hover:text-dark-100">Fechar</button>
                    </div>
                    {changelog.entries?.length === 0 ? (
                      <p className="text-xs text-dark-300">Nenhuma alteração de DNS registrada ainda.</p>
                    ) : (

  ...
  return (
    <div className="px-3 pt-2" ref={ref}>
      ...
      >
        ...
        <span className="flex-1 text-left truncate">{activeServer?.name || "Selecionar servidor"}</span>
          ...
          <div className="px-3 py-3 border-t border-dark-600/50">
            ...
            <div className="flex items-center justify-between px-3 mt-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] text-dark-400">{state.apiHealthy === null ? "Verificando..." : state.apiHealthy ? "Conectado" : "Desconectado"}</span>
              </div>
              <div className="flex items-center gap-1">
                <Link to="/notifications" className="relative p-1.5 text-dark-400 hover:text-dark-200 transition-colors rounded" title="Notificações" aria-label="Notificações">
        ...
        {showHeader && (
          <div className="px-4 py-3 border-t border-dark-600/50">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] text-dark-400 flex-1">{state.apiHealthy === null ? "Verificando..." : state.apiHealthy ? "Tudo Funcionando" : "Problemas Detectados"}</span>
            </div>
          </div>
        )}
      </aside>
      ...
      <div className={`flex-1 min-w-0 flex flex-col ${showHeader ? "h-screen overflow-hidden" : "overflow-hidden"}`}>
        ...
        {showHeader && (
          <header className="h-16 border-b flex items-center justify-between px-6 shrink-0 bg-dark-900 border-dark-700">
            ...
            <div className="flex items-center gap-3">
              <Link to="/notifications" className="relative p-2 rounded-lg transition-colors text-dark-400 hover:text-dark-50 hover:bg-dark-800" title="Notificações" aria-label="Notificações">
              ...
              <button onClick={state.cycleTheme} className="p-2 rounded-lg transition-colors text-dark-400 hover:text-dark-50 hover:bg-dark-800" title={`Tema: ${state.theme}`}>
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4.098 19.902a3.75 3.75 0 0 0 5.304 0l6.401-6.402M6.75 21A3.75 3.75 0 0 1 3 17.25V4.125C3 3.504 3.504 3 4.125 3h5.25c.621 0 1.125.504 1.125 1.125v4.072M6.75 21a3.75 3.75 0 0 0 3.75-3.75V8.197M6.75 21h13.125c.621 0 1.125-.504 1.125-1.125v-5.25c0-.621-.504-1.125-1.125-1.125h-4.072M10.5 8.197l2.88-2.88c.438-.439 1.15-.439 1.59 0l3.712 3.713c.44.44.44 1.152 0 1.59l-2.88 2.88M6.75 17.25h.008v.008H6.75v-.008Z" />
                </svg>
              </button>
              ...
              <button onClick={state.logout} className="p-1.5 text-dark-400 hover:text-dark-100 rounded-lg" title="Sair"><Icon name="logout" className="w-4 h-4" /></button>
            </div>
          </header>
        )}
        ...
        {!showHeader && (
          <div className="sticky top-0 z-30 flex items-center gap-3 px-4 py-3 bg-dark-900/80 backdrop-blur-lg border-b border-dark-600 md:hidden">
            <button onClick={() => state.setSidebarOpen(true)} className="p-2 text-dark-200 hover:text-dark-50 hover:bg-dark-700 rounded-lg" aria-label="Abrir menu lateral">
        ...
        {state.twoFaEnforced && !state.twoFaEnabled && (
          <div className={`border-b px-4 py-3 flex items-center justify-between ${isLight ? "bg-warn-500/5 border-warn-500/20" : "bg-warn-500/10 border-warn-500/20"}`}>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-warn-400 font-medium">Autenticação de dois fatores é obrigatória. Habilite 2FA em Configurações &rarr; Segurança.</span>
            </div>
            <a href="/settings" className="px-3 py-1.5 bg-warn-500 text-dark-900 rounded text-xs font-bold hover:bg-warn-400 transition-colors">Configurar 2FA</a>
          </div>
        )}

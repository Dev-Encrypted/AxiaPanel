...
export default function AtlasLayout() {
  ...
  const crumbs = useMemo(() => {
    ...
    const result: { label: string; to: string }[] = [{ label: "Início", to: "/" }];
  ...
  return (
    <div className="flex flex-col h-screen bg-dark-900">
        ...
        <div className="flex items-center h-full px-4">
          ...
          <div className="flex items-center gap-2 shrink-0 ml-auto md:ml-4">
            ...
            {firingCount > 0 && (
              ...
              >
                {firingCount} alerta{firingCount > 1 ? "s" : ""}
              </Link>
            )}
            ...
            {/* Notification bell */}
            <Link to="/notifications" className="relative p-1.5 text-dark-300 hover:text-dark-100 rounded-md hover:bg-dark-700/30" title="Notificações" aria-label="Notificações">
      ...
      <div className="hidden md:block sticky top-14 z-30 bg-dark-900 border-b border-dark-700 px-4 py-2">
        <nav className="flex items-center gap-2 text-sm" aria-label="Trilha de navegação">
          {crumbs.map((crumb, i) => {
        ...
        <div className={`border-b px-4 py-3 flex items-center justify-between ${isLight ? "bg-warn-500/5 border-warn-500/20" : "bg-warn-500/10 border-warn-500/20"}`}>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-warn-400 font-medium">
              Autenticação de dois fatores é obrigatória. Habilite 2FA em Configurações &rarr; Segurança.
            </span>
          </div>
          <a
            href="/settings"

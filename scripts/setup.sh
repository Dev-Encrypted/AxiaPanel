...
#
# AxiaPanel Setup
# Installs AxiaPanel on a fresh server.
...
BANNER
    ...
    echo -e "  https://github.com/Dev-Encrypted/AxiaPanel"
    echo ""
}
...
check_source() {
    ...
    if [ ! -d "$AGENT_SRC/src" ]; then
        ...
        error "Run this script from the AxiaPanel repository root,"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}║         AxiaPanel installed successfully!            ║${NC}"
    echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════════════╝${NC}"
    echo ""
    if [ -n "$PANEL_DOMAIN" ]; then
